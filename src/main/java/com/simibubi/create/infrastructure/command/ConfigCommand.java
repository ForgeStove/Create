...
public class ConfigCommand {
	public static ArgumentBuilder<CommandSourceStack, ?> register() {
		return Commands.literal("config").executes(ctx -> {
			...
			return Command.SINGLE_SUCCESS;
				...
				.executes(ctx -> {
					ServerPlayer player = ctx.getSource().getPlayerOrException();
					AllPackets.getChannel().send(
							PacketDistributor.PLAYER.with(() -> player), new SConfigureConfigPacket(
									SConfigureConfigPacket.Actions.configScreen.name(),
				...
				.then(Commands.literal("set")
						...
						.then(Commands.argument("value", StringArgumentType.string()).executes(ctx -> {
							...
							try {
								...
								ctx.getSource()
										.sendFailure(Components.literal(
												"Config could not be set the the specified value!"));
								return 0;
							} catch (Exception e) {
								ctx.getSource()
										.sendFailure(Components.literal(
												"Something went wrong while trying to set config value. Check the "
