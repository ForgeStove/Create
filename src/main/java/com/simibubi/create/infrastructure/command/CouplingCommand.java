...
public class CouplingCommand {
	...
	public static final SimpleCommandExceptionType SAME_DIMENSION = new SimpleCommandExceptionType(Components.literal(
			"Minecarts have to be in the same Dimension"));
	public static final DynamicCommandExceptionType TWO_CARTS = new DynamicCommandExceptionType(a -> Components.literal(
		...
		return Commands.literal("coupling")
				...
				.then(Commands.literal("add")
						.then(Commands.argument("cart1", EntityArgument.entity())
								.then(Commands.argument("cart2", EntityArgument.entity()).executes(ctx -> {
									Entity cart1 = EntityArgument.getEntity(ctx, "cart1");
									if (!(cart1 instanceof AbstractMinecart)) throw ONLY_MINECARTS_ALLOWED.create();
									Entity cart2 = EntityArgument.getEntity(ctx, "cart2");
									if (!(cart2 instanceof AbstractMinecart)) throw ONLY_MINECARTS_ALLOWED.create();
						...
						.then(Commands.argument("carts", EntityArgument.entities()).executes(ctx -> {
							...
							if (entities.size() != 2) throw TWO_CARTS.create(entities.size());
				...
				.then(Commands.literal("remove")
						.then(Commands.argument("cart1", EntityArgument.entity())
								.then(Commands.argument("cart2", EntityArgument.entity()).executes(ctx -> {
									...
									Entity cart2 = EntityArgument.getEntity(ctx, "cart2");
									if (!(cart2 instanceof AbstractMinecart)) throw ONLY_MINECARTS_ALLOWED.create();
									LazyOptional<MinecartController> cart1Capability = cart1.getCapability(
											CapabilityMinecartController.MINECART_CONTROLLER_CAPABILITY);
									if (!cart1Capability.isPresent()) {
										ctx.getSource()
												.sendSuccess(
														() -> Components.literal(
																"Minecart has no Couplings Attached"), true
												);
										return 0;
									}
									...
									for (boolean bool : Iterate.trueAndFalse) {
										...
										MinecartController cart2Controller =
												CapabilityMinecartController.getIfPresent(cart1.getCommandSenderWorld(),
												coupledCart
										);
									...
									ctx.getSource()
											.sendSuccess(
													() -> Components.literal("The specified Carts are not coupled"),
													true
											);
									return 0;
								}))))
				.then(Commands.literal("removeAll")
						.then(Commands.argument("cart", EntityArgument.entity()).executes(ctx -> {
							...
							if (!(cart instanceof AbstractMinecart)) throw ONLY_MINECARTS_ALLOWED.create();
							LazyOptional<MinecartController> capability = cart.getCapability(
									CapabilityMinecartController.MINECART_CONTROLLER_CAPABILITY);
							if (!capability.isPresent()) {
								ctx.getSource()
										.sendSuccess(
												() -> Components.literal("Minecart has no Couplings Attached"),
												true
										);
								return 0;
							}
							...
							ctx.getSource().sendSuccess(
									() -> Components.literal("Removed " + couplings + " couplings from the Minecart"),
									true
							);
							return couplings;
						})));
