...
public class SConfigureConfigPacket extends SimplePacketBase {
	...
	private static void trySetConfig(String option, String value) {
		...
		try {
			...
			player.displayClientMessage(
					Components.literal(
							"Something went wrong while trying to set config value. Check the client logs for more "
									+ "information"), false
			);
	...
	public enum Actions {
		...
		@OnlyIn(Dist.CLIENT) private static void configScreen(String value) {
			if (value.equals("")) {
		...
		@OnlyIn(Dist.CLIENT) private static void rainbowDebug(String value) {
			LocalPlayer player = Minecraft.getInstance().player;
			...
			Component text = boolToText(AllConfigs.client().rainbowDebug.get()).append(Components.literal(
					" Rainbow Debug Utility").withStyle(ChatFormatting.WHITE));
			player.displayClientMessage(text, false);
		}
		@OnlyIn(Dist.CLIENT) private static void overlayReset(String value) {
			AllConfigs.client().overlayOffsetX.set(0);
			AllConfigs.client().overlayOffsetY.set(0);
		}
		@OnlyIn(Dist.CLIENT) private static void overlayScreen(String value) {
			ScreenOpener.open(new GoggleConfigScreen());
		}
		@OnlyIn(Dist.CLIENT) private static void experimentalLighting(String value) {
			ForgeConfig.CLIENT.experimentalForgeLightPipelineEnabled.set(true);
			Minecraft.getInstance().levelRenderer.allChanged();
		}
		@OnlyIn(Dist.CLIENT) private static void openPonder(String value) {
		...
		@OnlyIn(Dist.CLIENT) private static void fabulousWarning(String value) {
		...
		@OnlyIn(Dist.CLIENT) private static void zoomMultiplier(String value) {
			try {
		...
		@OnlyIn(Dist.CLIENT) private static void camAngleTarget(String value, boolean yaw) {
			try {
				float v = Float.parseFloat(value);
				if (yaw) {
		...
		@OnlyIn(Dist.CLIENT) private static void camAngleFunction(String value) {
		...
		private static MutableComponent boolToText(boolean b) {
			return b
					? Components.literal("enabled").withStyle(ChatFormatting.DARK_GREEN)
