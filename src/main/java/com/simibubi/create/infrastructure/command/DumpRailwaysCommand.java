	...
	static void fillReport(
		...
		if (graphCount > 0) {
			...
			for (TrackGraph graph : nearest) {
				chat.accept(graph.id.toString().substring(0, 5) + " with " + graph.getNodes().size() + " Nodes",
						white);
		...
		if (trainCount > 0 && !nearestTrains.isEmpty()) {
			...
			for (Train train : nearestTrains) {
				...
				} else if (train.navigation.destination != null) chat.accept(
						" -> Travelling to "
								+ train.navigation.destination.name
								+ " ("
								+ Mth.floor(train.navigation.distanceToDestination)
								+ "m away)", darkBlue
				);
				...
				if (runtime.getSchedule() != null) {
					chat.accept(
							" -> Schedule, Entry " + runtime.currentEntry + ", " + (
									runtime.paused ? "Paused" : runtime.state.name().replaceAll("_", " ")
