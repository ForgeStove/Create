...
public record InfoEntry(String name, InfoProvider provider) implements InfoElement {
	...
	@Override public void write(Player player, FriendlyByteBuf buffer) {
		buffer.writeBoolean(false);
		buffer.writeUtf(name);
		buffer.writeUtf(provider.getInfoSafe(player));
	}
	@Override public void print(int depth, @Nullable Player player, Consumer<String> lineConsumer) {
