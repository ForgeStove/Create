...
public class ChuteScenes {
	public static void downward(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ElementLink<WorldSectionElement> top = scene.world.showIndependentSection(
				util.select.fromTo(3, 3, 3, 3, 4, 3),
				Direction.DOWN
		);
		ElementLink<WorldSectionElement> bottom = scene.world.showIndependentSection(
				util.select.fromTo(
						3,
						2,
						3,
		...
		scene.idle(20);
		ElementLink<EntityElement> remove = scene.world.createItemEntity(
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						.withItem(AllBlocks.INDUSTRIAL_IRON_BLOCK.asStack()), 40
		);
		scene.idle(7);
		scene.world.modifyBlock(
				util.grid.at(3, 2, 3),
		...
		for (int i = 0; i < 3; i++) {
			remove = scene.world.createItemEntity(
					...
					util.vector.of(0, 0.1, 0).add(Vec3.atLowerCornerOf(offset.getNormal()).scale(-.1)),
					stack
			);
			scene.idle(12);
	...
	public static void upward(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						Pointing.RIGHT
				).withItem(AllItems.GOGGLES.asStack()), 50
		);
		scene.overlay.showText(70)
	...
	public static void smart(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(filter.add(0, 0.125, 0), Pointing.DOWN).rightClick()
						.withItem(copper), 40
		);
		scene.idle(7);
