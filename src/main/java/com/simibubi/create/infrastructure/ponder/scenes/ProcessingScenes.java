...
public class ProcessingScenes {
	public static void millstone(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.blockSurface(millstone, Direction.NORTH),
						Pointing.RIGHT
				).rightClick()
						.withItem(flour), 40
		);
	...
	public static void crushingWheels(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(18);
		scene.world.modifyEntity(entity1, Entity::discard);
		Emitter blockSpace = Emitter.withinBlockSpace(
		...
		scene.idle(40);
		for (int i = 0; i < 5; i++) {
			if (i < 4) scene.world.createItemOnBelt(util.grid.at(4, 4, 2), Direction.EAST, input);
			scene.idle(15);
			if (i < 3) scene.world.createItemOnBelt(util.grid.at(4, 4, 2), Direction.EAST, input);
			scene.idle(15);
			if (i > 0) {
			...
			if (i < 4) scene.effects.emitParticles(util.vector.centerOf(center).add(0, -0.2, 0), blockSpace, 3, 28);
	...
	public static void mixing(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.setBlock(
				util.grid.at(1, 1, 2),
				AllBlocks.BLAZE_BURNER.getDefaultState().setValue(BlazeBurnerBlock.HEAT_LEVEL, HeatLevel.KINDLED),
				true
		);
		scene.idle(10);
		scene.overlay.showText(80)
	...
	public static void compacting(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(40);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(basin), Pointing.DOWN).withItem(copper),
				30
		);
		scene.idle(30);
		Class<MechanicalPressBlockEntity> type = MechanicalPressBlockEntity.class;
	...
	public static void emptyBlazeBurner(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(20);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.centerOf(center.above(2)), Pointing.DOWN).rightClick()
		...
		scene.idle(20);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(center.above()), Pointing.DOWN).rightClick()
						.withItem(AllItems.EMPTY_BLAZE_BURNER.asStack()), 40
		);
		scene.idle(10);
		scene.overlay.showText(60)
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(center.east().above()),
						Pointing.DOWN
				).rightClick()
						.withItem(new ItemStack(Items.FLINT_AND_STEEL)), 40
		);
		scene.idle(7);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						.withItem(new ItemStack(Items.SOUL_SAND)), 40
		);
		scene.idle(7);
		scene.world.modifyBlock(
				util.grid.at(3, 1, 2),
	...
	public static void blazeBurner(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(burner), Pointing.DOWN).rightClick()
		...
		scene.world.modifyBlockEntityNBT(
				...
				teType,
				nbt -> nbt.put("HeldItem", AllItems.BLAZE_CAKE.asStack().serializeNBT())
		);
	...
	public static void basin(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(basinPos.below().north()),
						Pointing.RIGHT
				).withItem(new ItemStack(Items.BRICKS)), 30
		);
		scene.overlay.showText(60)
				.attachKeyFrame()
				.colored(PonderPalette.GREEN)
		...
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(basinPos), Pointing.RIGHT).withItem(nugget),
				30
		);
		scene.idle(30);
		scene.world.modifyBlockEntity(pressPos, type, pte -> pte.getPressingBehaviour().start(Mode.BASIN));
		...
		scene.idle(30);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(basinPos), Pointing.RIGHT).withItem(ingot),
				30
		);
		scene.idle(30);
		scene.world.modifyBlockEntity(pressPos, type, pte -> pte.getPressingBehaviour().start(Mode.BASIN));
		...
		scene.idle(30);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(basinPos), Pointing.RIGHT).withItem(block),
				30
		);
		scene.overlay.showText(70)
				.attachKeyFrame()
