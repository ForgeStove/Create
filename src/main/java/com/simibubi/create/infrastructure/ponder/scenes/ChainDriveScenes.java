...
public class ChainDriveScenes {
	public static void chainDriveAsRelay(SceneBuilder scene, SceneBuildingUtil util) {
		...
		for (int i = 0; i < 3; i++) {
			...
			if (i != 0) scene.world.showSection(util.select.position(3, 1, 2 + i), Direction.DOWN);
		}
		scene.idle(10);
		...
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(rotatedECD), Pointing.DOWN).rightClick()
						.withWrench(), 30
		);
		scene.idle(7);
	...
	public static void adjustableChainGearshift(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ElementLink<WorldSectionElement> lever = scene.world.showIndependentSection(
				util.select.fromTo(
						leverPos,
						leverPos.south()
				), Direction.UP
		...
		scene.world.showSection(
				util.select.fromTo(eastDrive, eastDrive.west(2))
		...
		scene.world.modifyBlockEntityNBT(
				util.select.position(analogPos), AnalogLeverBlockEntity.class, nbt -> {
					nbt.putInt("State", 8);
				}
		);
		...
		for (BlockPos gauge : new BlockPos[]{eastGauge, middleGauge, westGauge}) {
			...
			Builder builder = scene.overlay.showText(180)
					...
					.placeNearTarget()
					.pointAt(util.vector.blockSurface(gauge, Direction.NORTH));
			if (gauge == westGauge) builder.text("12 RPM");
			else builder.sharedText(gauge == eastGauge ? "rpm16_source" : "rpm16");
		}
	}
}
