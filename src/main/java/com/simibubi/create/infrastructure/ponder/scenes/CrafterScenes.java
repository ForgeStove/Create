...
public class CrafterScenes {
	public static void setup(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.multiplyKineticSpeed(
		...
		scene.world.modifyBlockEntity(
				util.grid.at(1, 3, 2),
				type,
				mct -> mct.getInventory().insertItem(0, planks.copy(), false)
		);
		scene.idle(10);
		scene.overlay.showText(50)
		...
		scene.world.modifyBlockEntity(
				util.grid.at(3, 2, 2),
				type,
				mct -> mct.getInventory().insertItem(0, cobble.copy(), false)
		);
		scene.idle(5);
		scene.world.modifyBlockEntity(
				util.grid.at(2, 2, 2),
				type,
				mct -> mct.getInventory().insertItem(0, iron.copy(), false)
		);
		scene.idle(5);
		scene.world.modifyBlockEntity(
				util.grid.at(1, 2, 2),
		...
		scene.world.modifyBlockEntity(
				util.grid.at(2, 1, 2),
				type,
				mct -> mct.getInventory().insertItem(0, redstoneDust.copy(), false)
		);
		scene.idle(5);
		scene.world.modifyBlockEntity(
				util.grid.at(3, 1, 2),
		...
		scene.world.modifyBlockEntity(
				util.grid.at(2, 2, 2),
				type,
				mct -> mct.getInventory().insertItem(0, stick.copy(), false)
		);
		scene.idle(2);
		scene.world.modifyBlockEntity(
				util.grid.at(2, 1, 2),
		...
		scene.overlay.showText(90)
				...
				.text("For recipes not fully occupying the crafter setup, the start can be forced using a Redstone "
						+ "Pulse")
	...
	public static void connect(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(80);
		scene.overlay.showControls(new InputWindowElement(v.add(0, 1, 0), Pointing.LEFT).rightClick().withWrench(),
				20);
		scene.idle(7);
		scene.world.connectCrafterInvs(util.grid.at(2, 3, 2), util.grid.at(1, 3, 2));
		scene.idle(20);
		scene.overlay.showControls(
				new InputWindowElement(v.add(0, -1, 0), Pointing.LEFT).rightClick().withWrench(),
				20
		);
		scene.idle(7);
		...
		scene.idle(20);
		scene.overlay.showControls(
				new InputWindowElement(v.add(.5, -.5, 0), Pointing.LEFT).rightClick().withWrench(),
				20
		);
		scene.idle(7);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.centerOf(util.grid.at(4, 2, 2)),
						Pointing.DOWN
				).withItem(planks),
				40
		);
	...
	public static void covers(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						Pointing.RIGHT
				).withItem(AllItems.CRAFTER_SLOT_COVER.asStack()).rightClick(), 50
		);
		scene.idle(7);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						Pointing.LEFT
				).withItem(new ItemStack(Items.BUCKET)), 40
		);
		scene.idle(50);
		...
		scene.idle(10);
		scene.overlay.showSelectionWithText(util.select.fromTo(3, 2, 2, 1, 2, 2).add(util.select.position(2, 1, 2)),
						80)
				.attachKeyFrame()
				.pointAt(util.vector.blockSurface(util.grid.at(2, 2, 2), Direction.NORTH))
				...
				.placeNearTarget();
		scene.idle(60);
		ElementLink<EntityElement> ingot = scene.world.createItemEntity(
				util.vector.centerOf(4, 4, 2),
