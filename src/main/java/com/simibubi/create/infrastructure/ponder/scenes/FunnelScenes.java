...
public class FunnelScenes {
	public static void intro(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.setBlocks(util.select.fromTo(2, -1, 4, 2, 0, 4), AllBlocks.ANDESITE_CASING.getDefaultState(),
				true);
		ElementLink<WorldSectionElement> independentSection = scene.world.showIndependentSection(
				verticalFunnel,
				Direction.UP
		);
		...
		scene.world.setBlocks(
				outputFunnel,
				AllBlocks.ANDESITE_FUNNEL.getDefaultState()
	...
	public static void brass(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.createItemOnBeltLike(
				brassFunnel.below().north(),
				Direction.SOUTH,
				ItemHandlerHelper.copyStackWithSize(itemStack, 64)
		);
		...
		for (int i = 0; i < 14; i++) {
			if (i < 12) scene.world.createItemOnBelt(
			...
			if (i == 2) {
				...
				scene.overlay.showControls(
						new InputWindowElement(filter, Pointing.DOWN).rightClick().withItem(emerald),
						60
				);
				scene.idle(10);
				scene.overlay.showText(80)
						.text("Using items on the filter slot will restrict the funnel to only transfer matching "
								+ "stacks.")
						.attachKeyFrame()
						.pointAt(filter)
						.placeNearTarget();
	...
	public static void transposer(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ElementLink<WorldSectionElement> rightChest = scene.world.showIndependentSection(
				util.select.position(0, 2, 2),
				Direction.DOWN
		);
		ElementLink<WorldSectionElement> leftChest = scene.world.showIndependentSection(
