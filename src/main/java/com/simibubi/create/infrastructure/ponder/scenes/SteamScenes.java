...
public class SteamScenes {
	public static void whistle(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(10);
		ElementLink<WorldSectionElement> whistleElement = scene.world.showIndependentSection(
				whistleArea,
				Direction.SOUTH
		);
		...
		scene.world.toggleRedstonePower(lever);
		scene.idle(40);
		InputWindowElement rightClick = new InputWindowElement(
		...
		scene.idle(20);
		rightClick = new InputWindowElement(
	...
	public static void engine(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ElementLink<WorldSectionElement> fakeGroundElement = scene.world.showIndependentSection(
				fakeGround,
				Direction.UP
		);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						Pointing.DOWN
				).withItem(AllBlocks.SHAFT.asStack()).rightClick(), 60
		);
		scene.idle(10);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						Pointing.RIGHT
				).withItem(new ItemStack(Items.OAK_LOG)).rightClick(), 60
		);
		scene.idle(10);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						target.add(0, 0, 0.5),
						Pointing.DOWN
				).withItem(AllItems.GOGGLES.asStack()), 60
		);
		scene.idle(6);
		scene.overlay.showText(80)
		...
		scene.world.setBlocks(
				burners,
				AllBlocks.BLAZE_BURNER.getDefaultState().setValue(BlazeBurnerBlock.HEAT_LEVEL, HeatLevel.SEETHING),
				false
		);
		scene.idle(15);
		...
		scene.idle(20);
		ElementLink<WorldSectionElement> cogsElement = scene.world.showIndependentSection(
				util.select.position(1, 1, 7),
				Direction.NORTH
		);
		...
		for (int y = 4; y < 9; y++) {
			if (y != 6) for (boolean left : Iterate.trueAndFalse) {
				int z = (left ^ y % 2 == 0) ? 3 : 4;
				if (previous != null) scene.world.setKineticSpeed(previous, previousForward ? 64 : -64);
