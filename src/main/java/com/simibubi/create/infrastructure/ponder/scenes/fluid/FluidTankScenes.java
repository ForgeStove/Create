...
public class FluidTankScenes {
	public static void storage(SceneBuilder scene, SceneBuildingUtil util) {
		...
		for (int i = 0; i < 4; i++) {
			scene.world.modifyBlockEntity(
					tankPos,
					FluidTankBlockEntity.class,
					be -> be.getTankInventory().drain(2000, FluidAction.EXECUTE)
			);
			scene.idle(5);
		}
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						Pointing.RIGHT
				).showing(AllIcons.I_MTD_CLOSE).withItem(bucket), 40
		);
		scene.idle(7);
		...
		for (int i = 0; i < 10; i++) {
			scene.effects.emitParticles(
					...
					Emitter.simple(FluidFX.getFluidParticle(content), VecHelper.offsetRandomly(Vec3.ZERO, random,
							.1f)),
					1,
					1
			);
	...
	public static void creative(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.modifyBlockEntity(
				...
				be -> ((CreativeSmartFluidTank) be.getTankInventory()).setContainedFluid(new FluidStack(
						Fluids.FLOWING_LAVA,
						1000
				))
		);
