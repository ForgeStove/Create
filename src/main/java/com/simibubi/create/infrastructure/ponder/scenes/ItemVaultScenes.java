...
public class ItemVaultScenes {
	public static void storage(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(util.vector.blockSurface(frontVault, Direction.NORTH), Pointing.RIGHT).showing(
						AllIcons.I_MTD_CLOSE).withItem(hand), 40
		);
		scene.idle(7);
		scene.overlay.showText(60)
		...
		for (int i = 0; i < junk.length + 2; i++) {
			...
			scene.idle(5);
			if (i < junk.length) scene.world.createItemOnBeltLike(util.grid.at(0, 1, 3), Direction.SOUTH, junk[i]);
			scene.idle(9);
		}
		...
		for (ItemStack itemStack : junk) {
			scene.world.createItemOnBeltLike(util.grid.at(2, 1, 1), Direction.EAST, itemStack);
			scene.idle(18);
		}
	}
