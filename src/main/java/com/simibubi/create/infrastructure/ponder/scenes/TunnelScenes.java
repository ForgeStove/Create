...
public class TunnelScenes {
	public static void andesite(SceneBuilder scene, SceneBuildingUtil util) {
		...
		for (int i = 0; i < 3; i++) {
			...
			scene.world.modifyBlockEntityNBT(
					...
					nbt -> NBTHelper.writeEnum(nbt, "Casing", BeltBlockEntity.CasingType.ANDESITE),
					true
			);
			scene.idle(4);
		}
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(util.grid.at(4, 1, 2)),
						Pointing.DOWN
	...
	public static void brass(SceneBuilder scene, SceneBuildingUtil util) {
		...
		for (int i = 0; i < 3; i++) {
			...
			scene.world.modifyBlockEntityNBT(
					...
					BeltBlockEntity.class,
					nbt -> NBTHelper.writeEnum(nbt, "Casing", BeltBlockEntity.CasingType.BRASS),
					true
			);
			scene.idle(4);
		}
		...
		scene.world.modifyBlockEntity(
				tunnelPos,
				tunnelClass,
				be -> be.getBehaviour(SidedFilteringBehaviour.TYPE).setFilter(Direction.EAST, copper)
		);
		...
		scene.world.modifyBlockEntity(
				tunnelPos,
				tunnelClass,
				be -> be.getBehaviour(SidedFilteringBehaviour.TYPE).setFilter(Direction.EAST, ItemStack.EMPTY)
		);
		...
		scene.world.modifyBlockEntity(
				tunnelPos,
				tunnelClass,
				be -> be.getBehaviour(SidedFilteringBehaviour.TYPE).setFilter(Direction.WEST, copper)
		);
		scene.idle(4);
		tunnelFilterVec = getTunnelFilterVec(tunnelPos, Direction.NORTH);
		...
		scene.world.modifyBlockEntity(
				tunnelPos,
				tunnelClass,
				be -> be.getBehaviour(SidedFilteringBehaviour.TYPE).setFilter(Direction.NORTH, zinc)
		);
		...
		scene.world.modifyBlockEntity(
				tunnelPos,
				tunnelClass,
				be -> be.getBehaviour(SidedFilteringBehaviour.TYPE).setFilter(Direction.NORTH, ItemStack.EMPTY)
		);
		scene.world.modifyBlockEntity(
				tunnelPos,
				tunnelClass,
				be -> be.getBehaviour(SidedFilteringBehaviour.TYPE).setFilter(Direction.WEST, ItemStack.EMPTY)
		);
		scene.idle(10);
		...
		scene.overlay.showText(120)
				...
				.pointAt(tunnelTop)
				.placeNearTarget()
				.text("Whenever a passing item has multiple valid exits, the distribution mode will decide how to "
						+ "handle it");
		for (int i = 0; i < 3; i++) {
			scene.idle(40);
		...
		scene.idle(15);
		ElementLink<WorldSectionElement> newBelt = scene.world.showIndependentSection(
				util.select.fromTo(
						3,
						3,
						2,
		...
		scene.world.modifyBlockEntity(
				newTunnelPos.north(),
				tunnelClass,
				be -> be.getBehaviour(SidedFilteringBehaviour.TYPE).setFilter(Direction.WEST, item1)
		);
		scene.idle(4);
		...
		scene.world.modifyBlockEntity(
				newTunnelPos,
				tunnelClass,
				be -> be.getBehaviour(SidedFilteringBehaviour.TYPE).setFilter(Direction.WEST, item2)
		);
		scene.idle(4);
		scene.overlay.showControls(
				new InputWindowElement(tunnelFilterVec.add(0, 0, 1), Pointing.LEFT).withItem(item3),
				20
		);
		scene.world.modifyBlockEntity(
				newTunnelPos.south(),
				tunnelClass,
				be -> be.getBehaviour(SidedFilteringBehaviour.TYPE).setFilter(Direction.WEST, item3)
		);
		scene.idle(30);
		scene.overlay.showText(80)
				.pointAt(tunnelTop)
	...
	public static void brassModes(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(130);
		Class<BrassTunnelBlockEntity> tunnelClass = BrassTunnelBlockEntity.class;
		ElementLink<WorldSectionElement> blockage = scene.world.showIndependentSection(
				util.select.position(4, 1, 0),
				Direction.UP
		);
		scene.world.moveSection(blockage, util.vector.of(-3, 0, 0), 0);
		Vec3 modeVec = util.vector.of(4, 2.5, 3);
		scene.overlay.showControls(
				new InputWindowElement(modeVec, Pointing.RIGHT).showing(AllIcons.I_TUNNEL_SPLIT),
				140
		);
			...
			if (i == 4) {
				scene.overlay.showControls(
						new InputWindowElement(
								modeVec,
								Pointing.RIGHT
						).showing(AllIcons.I_TUNNEL_FORCED_SPLIT), 140
				);
				scene.world.modifyBlockEntity(
						util.grid.at(1, 2, 3),
						tunnelClass,
			...
			if (i == 7) {
				...
				scene.overlay.showControls(
						new InputWindowElement(
								modeVec,
								Pointing.RIGHT
						).showing(AllIcons.I_TUNNEL_ROUND_ROBIN), 140
				);
				scene.overlay.showText(80)
						.attachKeyFrame()
						.pointAt(tunnelTop)
			...
			if (i == 19) {
				scene.overlay.showControls(
						new InputWindowElement(
								modeVec,
								Pointing.RIGHT
						).showing(AllIcons.I_TUNNEL_PREFER_NEAREST), 140
				);
				scene.world.modifyBlockEntity(
						util.grid.at(1, 2, 3),
						tunnelClass,
			...
			if (i == 26) {
				scene.overlay.showControls(
						new InputWindowElement(
								modeVec,
								Pointing.RIGHT
						).showing(AllIcons.I_TUNNEL_RANDOMIZE), 140
				);
				scene.world.modifyBlockEntity(
						util.grid.at(1, 2, 3),
						tunnelClass,
		...
		scene.overlay.showControls(
				new InputWindowElement(
						modeVec,
						Pointing.RIGHT
				).showing(AllIcons.I_TUNNEL_SYNCHRONIZE), 140
		);
		scene.world.modifyBlockEntity(
				util.grid.at(1, 2, 3),
				tunnelClass,
