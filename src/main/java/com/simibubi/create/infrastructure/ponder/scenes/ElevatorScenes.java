...
public class ElevatorScenes {
	public static void elevator(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(5);
		ElementLink<WorldSectionElement> elevatorLink = scene.world.showIndependentSection(
				util.select.fromTo(
						4,
						13,
						3,
		...
		scene.world.setBlock(
				midContact,
				AllBlocks.ELEVATOR_CONTACT.getDefaultState().setValue(ElevatorContactBlock.FACING, Direction.EAST),
				false
		);
		scene.idle(15);
		...
		scene.world.setBlock(
				nixiePos,
				AllBlocks.NIXIE_TUBES.get(DyeColor.GREEN)
						.getDefaultState()
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						.withItem(AllBlocks.DISPLAY_LINK.asStack()), 15
		);
