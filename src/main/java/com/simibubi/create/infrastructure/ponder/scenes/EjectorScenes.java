...
public class EjectorScenes {
	public static void ejector(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(targetPos), Pointing.DOWN).rightClick()
						.whileSneaking()
						.withItem(asStack), 50
		);
		scene.idle(7);
		...
		scene.idle(80);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(ejectorPos), Pointing.DOWN).rightClick()
						.withItem(asStack),
				50
		);
		scene.idle(7);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(ejectorPos).add(0.5, 0, 0),
						Pointing.RIGHT
				).withItem(copperBlock), 30
		);
		scene.idle(7);
		...
		scene.overlay.showText(100)
				.text("It is now limited to this stack size, and only activates when its held stack reaches this "
						+ "amount")
				.pointAt(util.vector.topOf(ejectorPos))
				.placeNearTarget();
		for (int i = 0; i < 4; i++) {
	...
	public static void splitY(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(tunnel),
						Pointing.DOWN
				).showing(AllIcons.I_TUNNEL_PREFER_NEAREST), 80
		);
		scene.idle(10);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(util.grid.at(4, 1, 3)),
						Pointing.DOWN
	...
	public static void redstone(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(30);
		ElementLink<WorldSectionElement> observer = scene.world.showIndependentSection(
