...
public class ArmScenes {
	public static void setup(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(depotSurface, Pointing.RIGHT).rightClick().withItem(armItem),
				50
		);
		scene.idle(7);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(util.grid.at(1, 1, 0)),
						Pointing.DOWN
				).leftClick()
						.withItem(armItem), 30
		);
	...
	public static void modes(SceneBuilder scene, SceneBuildingUtil util) {
		...
		for (int i = 0; i < 20; i++) {
			...
			if (i == 13) {
				...
				ElementLink<WorldSectionElement> blockage2 = scene.world.showIndependentSection(
