...
public class ChassisScenes {
	public static void linearGroup(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ElementLink<WorldSectionElement> chassis = scene.world.showIndependentSection(
				util.select.position(centralChassis), Direction.DOWN);
		scene.idle(10);
	...
	public static void linearAttachement(SceneBuilder scene, SceneBuildingUtil util) {
		...
		InputWindowElement input = new InputWindowElement(
				util.vector.blockSurface(chassisPos, Direction.WEST),
				Pointing.LEFT
		).rightClick()
		...
		scene.idle(20);
		ElementLink<WorldSectionElement> glassSection = scene.world.showIndependentSection(
				util.select.position(chassisPos.above()), Direction.DOWN);
		...
		ElementLink<WorldSectionElement> topGlassSection = scene.world.showIndependentSection(
		...
		scene.overlay.showCenteredScrollInput(chassisPos, Direction.NORTH, 50);
		scene.overlay.showControls(
				new InputWindowElement(blockSurface, Pointing.UP).whileCTRL()
						.rightClick()
						.withWrench(), 50
		);
		...
		scene.idle(20);
		ElementLink<WorldSectionElement> gluedPlank = scene.world.showIndependentSection(
		...
		scene.idle(15);
		scene.addKeyframe();
		ElementLink<WorldSectionElement> chain = scene.world.showIndependentSection(
				util.select.position(2, 7, 2),
				Direction.DOWN
		);
	...
	public static void radial(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ElementLink<WorldSectionElement> outer = scene.world.showIndependentSection(
				util.select.layer(2)
						.substract(chassis)
	...
	public static void superGlue(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(
						...
						Pointing.DOWN
				).withItem(AllItems.SUPER_GLUE.asStack()).leftClick(), 40
		);
		scene.idle(50);
		...
		scene.overlay.showOutline(
				PonderPalette.GREEN,
				cogs,
				util.select.fromTo(4, 2, 2, 1, 2, 2)
