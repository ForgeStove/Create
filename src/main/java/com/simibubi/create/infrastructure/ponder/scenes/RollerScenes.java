...
public class RollerScenes {
	public static void clearAndPave(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(5);
		ElementLink<ParrotElement> birbLink = scene.special.createBirb(
				util.vector.centerOf(8, 3, 4),
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(util.grid.at(2, 2, 4)),
						Pointing.DOWN
		...
		scene.overlay.showText(110)
				.independent()
				.attachKeyFrame()
				.text("Rollers are especially useful on Trains, but can also be used on most other types of moving "
						+ "contraptions");
		scene.idle(20);
	...
	public static void fill(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(100);
		scene.overlay.showControls(
				new InputWindowElement(filterSlot, Pointing.DOWN).showing(AllIcons.I_ROLLER_FILL),
				50
		);
		scene.idle(15);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						filterSlot.add(0, 0, -6 / 16f),
						Pointing.UP
				).withItem(paveItem), 35
		);
		scene.idle(7);
		scene.world.setFilterData(rollers, RollerBlockEntity.class, paveItem);
		scene.idle(10);
		...
		for (int j = 0; j < 5; j++) {
			...
			if (j == 2) scene.overlay.showText(90)
					.attachKeyFrame()
		...
		scene.overlay.showControls(
				new InputWindowElement(
						filterSlot,
						Pointing.DOWN
				).showing(AllIcons.I_ROLLER_WIDE_FILL), 40
		);
		scene.idle(45);
		scene.world.cycleBlockProperty(stationPos, StationBlock.ASSEMBLING);
		scene.world.animateTrainStation(stationPos, false);
		...
		scene.overlay.showText(110)
				.attachKeyFrame()
				.independent()
				.text("As opposed to 'clear & pave', neither of these modes will cause the rollers to break existing "
						+ "blocks")
				.placeNearTarget();
		for (int j = 0; j < 5; j++) {
			for (int i = 0; i < 3; i++) {
