...
public class DetectorScenes {
	public static void smartObserver(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(5);
		ElementLink<WorldSectionElement> observerLink = scene.world.makeSectionIndependent(util.select.position(
				observerPos));
	...
	public static void thresholdSwitch(SceneBuilder scene, SceneBuildingUtil util) {
		...
		for (int i = 0; i < 5; i++) {
			...
			if (i % 3 == 1) scene.world.modifyBlock(
					switchPos,
		...
		for (int i = 0; i < 3; i++) {
			scene.idle(10);
			scene.world.createItemOnBelt(util.grid.at(3, 0, 4), Direction.NORTH, ironIngot);
			if (i % 3 == 2) scene.world.modifyBlock(
					switchPos,
					s -> s.setValue(ThresholdSwitchBlock.LEVEL, s.getValue(ThresholdSwitchBlock.LEVEL) - 1),
					false
			);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.blockSurface(switchPos, Direction.UP),
						Pointing.DOWN
				).rightClick(),
				60
		);
