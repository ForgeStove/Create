...
public class DisplayScenes {
	public static void link(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(
						target,
						Pointing.RIGHT
				).withItem(AllBlocks.DISPLAY_LINK.asStack()).rightClick(),
				60
		);
		scene.idle(6);
		...
		scene.world.setDisplayBoardText(
				board,
				2,
				Components.literal(14 + " ").append(AllBlocks.COGWHEEL.asStack().getHoverName())
		);
		scene.world.flashDisplayLink(linkPos);
		scene.idle(40);
	...
	public static void board(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(new InputWindowElement(target, Pointing.RIGHT).withItem(clipboard).rightClick(),
				40);
		scene.idle(6);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						target,
						Pointing.RIGHT
				).withItem(new ItemStack(Items.PINK_DYE)).rightClick(),
				40
		);
		scene.idle(6);
	...
	public static void redstone(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(10);
		ElementLink<WorldSectionElement> redstoneBlock = scene.world.showIndependentSection(
