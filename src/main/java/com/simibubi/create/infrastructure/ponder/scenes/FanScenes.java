...
public class FanScenes {
	...
	public static void processing(SceneBuilder scene, SceneBuildingUtil util) {
		...
		// blasting start
		ElementLink<WorldSectionElement> blockInFront = scene.world.showIndependentSection(
		...
		ElementLink<EntityElement> entityLink = scene.world.createItemEntity(
				util.vector.centerOf(blockPos.west(2)
		...
		scene.effects.emitParticles(
				...
				Emitter.simple(ParticleTypes.LARGE_SMOKE, Vec3.ZERO),
				1,
				60
		);
		scene.overlay.showText(80)
		...
		entityLink = scene.world.createItemEntity(
		...
		scene.idle(15);
		ElementLink<WorldSectionElement> largeCog = scene.world.showIndependentSection(
				util.select.position(1, 2, 4),
				Direction.UP
		);
		ElementLink<WorldSectionElement> belt = scene.world.showIndependentSection(
				util.select.fromTo(3, 3, 1, 1, 3, 3),
				Direction.DOWN
		);
		scene.world.moveSection(largeCog, util.vector.of(-1, -2, 0), 0);
		scene.world.moveSection(belt, util.vector.of(-1, -2, 0), 0);
		ElementLink<BeltItemElement> transported = scene.world.createItemOnBelt(
				util.grid.at(3, 3, 3),
				Direction.SOUTH,
				sand
		);
