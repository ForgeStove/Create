...
public class GantryScenes {
	...
	private static void intro(SceneBuilder scene, SceneBuildingUtil util, boolean pinion) {
		...
		scene.idle(10);
		ElementLink<WorldSectionElement> gantry = scene.world.showIndependentSection(
				util.select.layer(2),
				Direction.DOWN
		);
		scene.idle(10);
	...
	public static void redstone(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ElementLink<WorldSectionElement> gantry = scene.world.showIndependentSection(
				util.select.position(gantryPos),
				Direction.DOWN
		);
		scene.idle(15);
	...
	public static void direction(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showText(60)
				...
				.placeNearTarget();
		scene.idle(60);
		ElementLink<WorldSectionElement> gantry1 = scene.world.showIndependentSection(
				util.select.position(0, 1, 3),
				Direction.NORTH
		);
		ElementLink<WorldSectionElement> gantry2 = scene.world.showIndependentSection(
	...
	public static void subgantry(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.moveSection(gantry, util.vector.of(0, -2, 0), 40);
		scene.idle(40);
		ElementLink<WorldSectionElement> secondGantry = scene.world.showIndependentSection(
				util.select.position(gantryPos2), Direction.DOWN);
		scene.idle(15);
		scene.overlay.showText(60)
				.attachKeyFrame()
