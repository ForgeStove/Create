...
public class MechanicalSawScenes {
	public static void processing(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(itemSpawn.add(2, 0, 0), Pointing.DOWN).withItem(strippedLog),
				20
		);
		scene.idle(30);
		...
		scene.world.moveSection(beltSection, util.vector.of(0, -100, 0), 0);
		scene.idle(3);
		ElementLink<WorldSectionElement> otherBeltSection = scene.world.showIndependentSection(
				otherBelt,
				Direction.WEST
		);
	...
	public static void treeCutting(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showSelectionWithText(util.select.position(breakingPos), 90)
				...
				.placeNearTarget()
				.text("In order to cut the tree fully, the Saw has to break the last block connecting it to the "
						+ "ground");
		scene.idle(25);
		for (int i = 0; i < 10; i++) {
			scene.idle(10);
