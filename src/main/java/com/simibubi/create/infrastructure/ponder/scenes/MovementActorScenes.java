...
public class MovementActorScenes {
	public static void psiTransfer(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ElementLink<EntityElement> entity1 = scene.world.createItemEntity(
				entitySpawn,
				util.vector.of(0, 0.2, 0),
				itemStack
		);
		scene.idle(10);
		ElementLink<EntityElement> entity2 = scene.world.createItemEntity(
				entitySpawn,
				util.vector.of(0, 0.2, 0),
				itemStack
		);
	...
	public static void harvester(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(1, 2, 5),
						Pointing.DOWN
				).withItem(wheatItem), 50
		);
		for (int i = 0; i < 3; i++)
			scene.world.modifyBlockEntity(
					util.grid.at(i, 1, 4),
	...
	public static void plough(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.showBasePlate();
		ElementLink<WorldSectionElement> cogs = scene.world.showIndependentSection(
				util.select.fromTo(4, 0, 6, 5, 1, 6),
				Direction.UP
		);
		scene.idle(5);
		scene.world.showSection(kinetics.substract(dynamic), Direction.DOWN);
		ElementLink<WorldSectionElement> pistonHead = scene.world.showIndependentSection(
				util.select.fromTo(
						5,
						1,
						1,
		...
		scene.world.modifyEntities(ItemEntity.class, Entity::discard);
		ElementLink<WorldSectionElement> chest = scene.world.showIndependentSection(
				util.select.position(4, 2, 2),
				Direction.DOWN
		);
		scene.overlay.showText(60)
				.attachKeyFrame()
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(2, 2, 2),
						Pointing.DOWN
				).withItem(new ItemStack(Items.SNOWBALL)), 40
		);
		scene.idle(40);
		...
		ElementLink<ParrotElement> birb = scene.special.createBirb(
				util.vector.topOf(3, 0, 2).add(0, 0, 0.5),
				FlappyPose::new
		);
		scene.idle(11);
		scene.world.modifyKineticSpeed(util.select.everywhere(), f -> -2 * f);
	...
	public static void contraptionControls(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.showSection(util.select.position(bearingPos), Direction.DOWN);
		scene.idle(5);
		ElementLink<WorldSectionElement> contraptionLink = scene.world.showIndependentSection(
				contraption,
				Direction.DOWN
		);
		...
		ItemStack wheatItem = new ItemStack(Items.WHEAT);
		ElementLink<EntityElement> item1 = scene.world.createItemEntity(util.vector.centerOf(cobblePos), m,
				cobbleItem);
		scene.idle(5);
		...
		scene.world.setKineticSpeed(util.select.position(drillPos), 0);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.of(1.5, 2.75, 4.5), Pointing.DOWN).rightClick(),
				15
		);
		scene.idle(7);
		scene.world.modifyBlockEntity(controlsPos1, ContraptionControlsBlockEntity.class, ccte -> ccte.disabled =
				true);
		...
		scene.idle(40);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.of(3.5, 2.75, 1), Pointing.DOWN).rightClick(),
				15
		);
		scene.idle(7);
		scene.world.modifyBlockEntity(
		...
		scene.idle(75);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(controlsPos2), Pointing.DOWN).rightClick()
						.withItem(AllBlocks.MECHANICAL_DRILL.asStack()), 30
		);
		scene.idle(5);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.centerOf(controlsPos1), Pointing.UP).rightClick()
		...
		scene.idle(90);
		scene.overlay.showControls(
				new InputWindowElement(util.vector.topOf(controlsPos2), Pointing.RIGHT).rightClick(),
				15
		);
		scene.idle(7);
		scene.world.modifyBlockEntity(controlsPos2, ContraptionControlsBlockEntity.class, ccte -> ccte.disabled =
				true);
		...
		ElementLink<EntityElement> item4 = scene.world.createItemEntity(util.vector.centerOf(cobblePos), m,
				cobbleItem);
		scene.idle(5);
		scene.world.rotateBearing(bearingPos, -60, 40);
		scene.world.rotateSection(contraptionLink, 0, -60, 0, 40);
