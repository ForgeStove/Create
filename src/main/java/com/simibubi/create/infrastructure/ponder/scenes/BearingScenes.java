...
public class BearingScenes {
	public static void windmillsAsSource(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.world.setBlock(
				util.grid.at(2, -1, 0),
				AllBlocks.SAIL.getDefaultState().setValue(SailBlock.FACING, Direction.NORTH),
				false
		);
		scene.idle(5);
		...
		scene.idle(50);
		ElementLink<WorldSectionElement> structure = scene.world.showIndependentSection(
		...
		scene.idle(5);
		for (Direction d : Iterate.directions)
			if (d.getAxis() != Axis.Z) scene.effects.superGlue(anchorPos.relative(d, 1), d.getOpposite(), false);
		scene.idle(10);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.centerOf(util.grid.at(5, 2, 0)),
						Pointing.RIGHT
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.topOf(windmill).subtract(.5, 0, 0),
						Pointing.DOWN
				).rightClick(), 60
		);
		scene.idle(7);
	...
	public static void windmillsAnyStructure(SceneBuilder scene, SceneBuildingUtil util) {
		...
		ElementLink<WorldSectionElement> contraption = scene.world.showIndependentSection(
				util.select.position(bearingPos.above()), Direction.DOWN);
		scene.idle(10);
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.blockSurface(bearingPos, Direction.WEST),
						Pointing.LEFT
				).rightClick(), 40
		);
		scene.idle(7);
	...
	public static void mechanicalBearing(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.centerOf(util.grid.at(4, 3, 3)),
						Pointing.RIGHT
				).withItem(AllItems.SUPER_GLUE.asStack()), 40
		);
		scene.idle(10);
	...
	public static void stabilizedBearings(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.overlay.showSelectionWithText(util.select.position(bearingPos.north()), 60)
				...
				.placeNearTarget();
		scene.idle(70);
		ElementLink<WorldSectionElement> subContraption = scene.world.showIndependentSection(
				util.select.fromTo(
						4,
						4,
						1,
	...
	public static void clockwork(SceneBuilder scene, SceneBuildingUtil util) {
		...
		scene.idle(50);
		ElementLink<WorldSectionElement> plank = scene.world.showIndependentSection(
		...
		scene.overlay.showText(80)
				...
				.placeNearTarget()
				.attachKeyFrame()
				.text("Upon receiving Rotational Force, the structure will be rotated according to the hour of the "
						+ "day");
		scene.idle(90);
		...
		InputWindowElement clickTheBearingSide = new InputWindowElement(
		...
		ElementLink<WorldSectionElement> hourHand = scene.world.showIndependentSection(
				util.select.fromTo(
						3,
						3,
						1,
		...
		scene.overlay.showControls(
				new InputWindowElement(
						util.vector.centerOf(util.grid.at(3, 4, 1)),
						Pointing.RIGHT
				).withItem(AllItems.SUPER_GLUE.asStack()), 40
		);
		...
		scene.world.setBlock(
				util.grid.at(3, 3, 0),
				Blocks.STONE_BRICK_WALL.defaultBlockState().setValue(WallBlock.SOUTH_WALL, WallSide.TALL),
				false
		);
		ElementLink<WorldSectionElement> minuteHand = scene.world.showIndependentSection(
				util.select.fromTo(
						3,
						3,
						0,
	...
	private static void sails(SceneBuilder scene, SceneBuildingUtil util, boolean frame) {
		...
		for (int i = 0; i < 3; i++) {
			for (Direction d : Iterate.horizontalDirections) {
				...
				if (frame) scene.world.modifyBlock(
						location,
						s -> AllBlocks.SAIL_FRAME.getDefaultState()
		...
		if (!frame) {
			...
			scene.idle(10);
			InputWindowElement input = new InputWindowElement(
					util.vector.blockSurface(
							util.grid.at(2, 3, 1),
							Direction.NORTH
			),
			...
			scene.world.setBlock(
					util.grid.at(2, 3, 1),
					AllBlocks.DYED_SAILS.get(DyeColor.BLUE)
							.getDefaultState()
			...
			scene.world.setBlock(
					util.grid.at(3, 3, 2),
					AllBlocks.SAIL_FRAME.getDefaultState().setValue(SailBlock.FACING, Direction.NORTH),
					false
			);
			scene.idle(10);
			scene.overlay.showText(40)
