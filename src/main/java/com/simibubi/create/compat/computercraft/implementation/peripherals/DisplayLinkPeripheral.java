...
public class DisplayLinkPeripheral extends SyncedPeripheral<DisplayLinkBlockEntity> {
	...
	@LuaFunction public final void setCursorPos(int x, int y) {
		cursorX.set(x - 1);
		cursorY.set(y - 1);
	}
	@LuaFunction public final Object[] getCursorPos() {
		return new Object[]{cursorX.get() + 1, cursorY.get() + 1};
	}
	@LuaFunction(mainThread = true) public final Object[] getSize() {
	...
	@LuaFunction public final boolean isColor() {
		return false;
	}
	@LuaFunction public final boolean isColour() {
		return false;
	}
	@LuaFunction public final void write(String text) {
		writeImpl(text);
	}
	...
	@LuaFunction public final void writeBytes(IArguments args) throws LuaException {
	...
	@LuaFunction public final void clearLine() {
		ListTag tag = blockEntity.getSourceConfig().getList(TAG_KEY, Tag.TAG_STRING);
	...
	@LuaFunction public final void clear() {
		synchronized (blockEntity) {
			blockEntity.getSourceConfig().put(TAG_KEY, new ListTag());
		}
	}
	@LuaFunction(mainThread = true) public final void update() {
