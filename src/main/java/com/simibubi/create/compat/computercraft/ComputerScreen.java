...
public class ComputerScreen extends AbstractSimiScreen {
	...
	@Override protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		graphics.drawString(
				font,
				displayTitle.get(),
				Math.round(x + background.width / 2.0F - font.width(displayTitle.get()) / 2.0F),
				y + 4,
				0x442000,
				false
		);
		graphics.drawWordWrap(
				font,
				Lang.translate("gui.attached_computer.controlled").component(),
				x + 55,
				y + 32,
				111,
				0x7A7A7A
		);
		if (additional != null) additional.render(graphics, mouseX, mouseY, partialTicks, x, y, background);
	}
	@FunctionalInterface public interface RenderWindowFunction {
		void render(
