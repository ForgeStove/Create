...
public class AttachedComputerPacket extends BlockEntityDataPacket<SyncedBlockEntity> {
	...
	@Override protected void writeData(FriendlyByteBuf buffer) {
		buffer.writeBoolean(hasAttachedComputer);
	}
	@Override protected void handlePacket(SyncedBlockEntity blockEntity) {
		if (blockEntity instanceof SmartBlockEntity sbe) {
			sbe.getBehaviour(AbstractComputerBehaviour.TYPE).setHasAttachedComputer(hasAttachedComputer);
		}
	}
}
