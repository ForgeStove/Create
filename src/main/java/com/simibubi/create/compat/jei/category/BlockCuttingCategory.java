...
@ParametersAreNonnullByDefault public class BlockCuttingCategory
	...
	public void setRecipe(IRecipeLayoutBuilder builder, CondensedBlockCuttingRecipe recipe, IFocusGroup focuses) {
		...
		builder.addSlot(RecipeIngredientRole.INPUT, 5, 5).setBackground(getRenderedSlot(), -1, -1)
				.addItemStacks(Arrays.asList(recipe.getIngredients().get(0).getItems()));
		int i = 0;
		for (List<ItemStack> itemStacks : results) {
			int xPos = 78 + (i % 5) * 19;
			int yPos = 48 + (i / 5) * -19;
			builder.addSlot(RecipeIngredientRole.OUTPUT, xPos, yPos).setBackground(getRenderedSlot(), -1, -1)
