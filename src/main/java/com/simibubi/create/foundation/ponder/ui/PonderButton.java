...
public class PonderButton extends BoxWidget {
	...
	@Override public <T extends ElementWidget> T showingElement(RenderElement element) {
	...
	@Override public void doRender(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (shortcut != null) {
			...
			graphics.drawCenteredString(
					...
					shortcut.getTranslatedKeyMessage(),
					getX() + width / 2 + 8,
					getY() + height - 6,
					Theme.c(Theme.Key.TEXT_DARKER).scaleAlpha(fadeValue).getRGB()
			);
			ms.popPose();
		}
	}
	...
	@Override public Key getDisabledTheme() {
		return Theme.Key.PONDER_BUTTON_DISABLE;
	}
	@Override public Key getIdleTheme() {
		return Theme.Key.PONDER_BUTTON_IDLE;
	}
	@Override public Key getHoverTheme() {
		return Theme.Key.PONDER_BUTTON_HOVER;
	}
	@Override public Key getClickTheme() {
