...
public class HideAllInstruction extends TickingInstruction {
	...
	@Override protected void firstTick(PonderScene scene) {
		...
		scene.getElements().forEach(element -> {
			if (element instanceof AnimatedSceneElement animatedSceneElement) {
				...
				animatedSceneElement.setFadeVec(fadeOutTo == null
						? null
						: Vec3.atLowerCornerOf(fadeOutTo.getNormal()).scale(.5f));
	...
	@Override public void tick(PonderScene scene) {
		...
		scene.forEach(
				AnimatedSceneElement.class, ase -> {
					ase.setFade(fade * fade);
					if (remainingTicks == 0) ase.setFade(0);
				}
		);
		scene.forEach(
				AnimatedOverlayElement.class, aoe -> {
