...
public class PonderTooltipHandler {
	...
	public static void deferredTick() {
		...
		} else holdKeyProgress.setValue(Math.max(0, value - .05f));
	...
	public static void addToTooltip(ItemTooltipEvent event) {
		...
		Component component = subject
				? Lang.translateDirect(SUBJECT).withStyle(ChatFormatting.GREEN)
	...
	protected static void updateHovered(ItemStack stack) {
		...
		if (inPonderUI) {
			...
			if (ItemHelper.sameItem(stack, ponderUI.getSubject())) subject = true;
		}
		if (stack.isEmpty()) return;
		if (!PonderRegistry.ALL.containsKey(RegisteredObjects.getKeyOrThrow(stack.getItem()))) return;
		if (prevStack.isEmpty() || !ItemHelper.sameItem(prevStack, stack)) holdKeyProgress.startWithValue(0);
		hoveredStack = stack;
		trackingStack = stack;
	}
	...
	private static int getSmoothColorForProgress(float progress) {
		if (progress < .5f) return Color.mixColors(0x5000FF, 5592575, progress * 2);
		return Color.mixColors(5592575, 0xffffff, (progress - .5f) * 2);
	}
	private static Component makeProgressBar(float progress) {
		MutableComponent holdW = Lang.translateDirect(
				HOLD_TO_PONDER,
		...
		if (progress > 0) {
			...
			if (progress < 1) bars += ChatFormatting.DARK_GRAY + Strings.repeat("|", total - current);
