...
public abstract class NavigatableSimiScreen extends AbstractSimiScreen {
	...
	@Override protected void init() {
		...
		if (!(screenHistory.get(0) instanceof NavigatableSimiScreen screen)) return;
	...
	@Override protected void renderWindowBackground(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		if (backTrack != null) {
			...
			if (x + 30 < backTrack.getX()) UIRenderHelper.breadcrumbArrow(
					graphics,
					x + 30,
					height - 51,
					0,
		...
		float scale = getScale(transitionValue);
		ms.translate(depthPointX, depthPointY, 0);
		ms.scale(scale, scale, 1);
		ms.translate(-depthPointX, -depthPointY, 0);
	}
	private float getScale(float transitionValue) {
		float scale;
	...
	protected void renderZeloBreadcrumbs(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		int spacing = 20;
		List<String> names = history.stream().map(NavigatableSimiScreen::screenTitle).toList();
		...
		if (x.getValue() < 25) x.setValue(25);
		PoseStack ms = graphics.pose();
		ms.pushPose();
	...
	private static String screenTitle(Screen screen) {
		if (screen instanceof NavigatableSimiScreen) return ((NavigatableSimiScreen) screen).getBreadcrumbTitle();
		return "<";
	}
