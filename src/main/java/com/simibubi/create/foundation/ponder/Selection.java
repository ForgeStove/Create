...
public abstract class Selection implements Predicate<BlockPos> {
	...
	private static class Compound extends Selection {
		...
		@Override public boolean test(BlockPos t) {
			return posSet.contains(t);
		}
		@Override public Selection add(Selection other) {
			other.forEach(p -> posSet.add(p.immutable()));
			center = null;
			return this;
		}
		@Override public Selection substract(Selection other) {
			other.forEach(p -> posSet.remove(p.immutable()));
			center = null;
			return this;
		}
		@Override public void forEach(Consumer<BlockPos> callback) {
			posSet.forEach(callback);
		}
		@Override public OutlineParams makeOutline(Outliner outliner, Object slot) {
			return outliner.showCluster(slot, posSet);
		}
		@Override public Vec3 getCenter() {
			return center == null ? center = evalCenter() : center;
		}
		private Vec3 evalCenter() {
			Vec3 center = Vec3.ZERO;
			if (posSet.isEmpty()) return center;
		...
		@Override public Selection copy() {
