...
public abstract class AnimatedSceneElement extends PonderSceneElement {
	...
	@Override public final void renderFirst(PonderWorld world, MultiBufferSource buffer, PoseStack ms, float pt) {
		ms.pushPose();
	...
	@Override public final void renderLast(PonderWorld world, MultiBufferSource buffer, PoseStack ms, float pt) {
		ms.pushPose();
		float currentFade = applyFade(ms, pt);
	...
	protected float applyFade(PoseStack ms, float pt) {
		...
		if (fadeVec != null) TransformStack.cast(ms).translate(fadeVec.scale(-1 + currentFade));
		return currentFade;
	}
