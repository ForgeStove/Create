...
public class DisplayWorldSectionInstruction extends FadeIntoSceneInstruction<WorldSectionElement> {
	...
	@Override protected void firstTick(PonderScene scene) {
		...
		mergeOnto.ifPresent(wse -> element.setAnimatedOffset(wse.get().getAnimatedOffset(), true));
	...
	@Override public void tick(PonderScene scene) {
		...
		if (glue != null) SuperGlueItem.spawnParticles(scene.getWorld(), glue, fadeInFrom, true);
	}
	@Override protected Class<WorldSectionElement> getElementClass() {
