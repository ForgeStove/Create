...
public class MinecartElement extends AnimatedSceneElement {
	...
	@Override public void tick(PonderScene scene) {
		super.tick(scene);
		if (entity == null) entity = constructor.create(scene.getWorld(), 0, 0, 0);
		entity.tickCount++;
	...
	protected void renderLast(PonderWorld world, MultiBufferSource buffer, PoseStack ms, float fade, float pt) {
		...
		if (entity == null) entity = constructor.create(world, 0, 0, 0);
		ms.pushPose();
		...
		TransformStack.cast(ms).rotateY(rotation.getValue(pt));
