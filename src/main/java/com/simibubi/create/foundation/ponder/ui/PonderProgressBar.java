...
public class PonderProgressBar extends AbstractSimiWidget {
	...
	@Override public void onClick(double mouseX, double mouseY) {
		...
		if (keyframeIndex == -1) ponder.seekToTime(0);
		else if (keyframeIndex == activeScene.getKeyframeCount()) ponder.seekToTime(activeScene.getTotalTime());
		else ponder.seekToTime(activeScene.getKeyframeTime(keyframeIndex));
	}
	public int getHoveredKeyframeIndex(PonderScene activeScene, double mouseX) {
		...
		for (int i = 0; i < activeScene.getKeyframeCount(); i++) {
			...
			if (keyframeTime > clickedAtTime) break;
			index = i;
		}
		return index;
	}
	...
	private void renderKeyframes(GuiGraphics graphics, int mouseX, float partialTicks) {
		...
		if (hoverIndex == -1) drawKeyframe(graphics, activeScene, true, 0, 0, hoverStartColor, hoverEndColor, 8);
		else if (hoverIndex == activeScene.getKeyframeCount()) drawKeyframe(
				graphics,
				activeScene,
				true,
