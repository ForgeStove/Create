...
public class PonderWorld extends SchematicWorld {
	...
	public void restoreBlocks(Selection selection) {
		selection.forEach(p -> {
			if (originalBlocks.containsKey(p)) blocks.put(p, originalBlocks.get(p));
	...
	private void redraw() {
		if (scene != null) scene.forEach(WorldSectionElement.class, WorldSectionElement::queueRedraw);
	}
	...
	@Override public int getBrightness(LightLayer p_226658_1_, BlockPos p_226658_2_) {
	...
	@Override public BlockState getBlockState(BlockPos globalPos) {
		if (mask != null && !mask.test(globalPos.subtract(anchor))) return Blocks.AIR.defaultBlockState();
		if (currentlyTickingEntities && globalPos.getY() < 0) return Blocks.AIR.defaultBlockState();
	...
	public void tick() {
		...
		for (Iterator<Entity> iterator = entities.iterator(); iterator.hasNext(); ) {
			...
			entity.zOld = entity.getZ();
			entity.tick();
			if (entity.getY() <= -.5f) entity.discard();
			if (!entity.isAlive()) iterator.remove();
		}
	...
	@Override public boolean setBlock(BlockPos pos, BlockState arg1, int arg2) {
		return super.setBlock(pos, arg1, arg2);
	}
	public void addParticle(Particle p) {
		if (p != null) particles.addParticle(p);
	}
	@Override protected void onBEadded(BlockEntity blockEntity, BlockPos pos) {
	...
	@Override public void fixControllerBlockEntities() {
		super.fixControllerBlockEntities();
		for (BlockEntity blockEntity : blockEntities.values()) {
			if (!(blockEntity instanceof BeltBlockEntity beltBlockEntity)) continue;
	...
	public void setBlockBreakingProgress(BlockPos pos, int damage) {
		if (damage == 0) blockBreakingProgressions.remove(pos);
		else blockBreakingProgressions.put(pos, damage - 1);
	}
	...
	@Override protected BlockState processBlockStateForPrinting(BlockState state) {
		return state;
	}
	@Override public boolean hasChunkAt(BlockPos pos) {
		return true; // fix particle lighting
	}
	@Override public boolean hasChunk(int x, int y) {
		return true; // fix particle lighting
	}
