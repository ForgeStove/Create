...
public class PonderLocalization {
	...
	public static String getSpecific(ResourceLocation sceneId, String k) {
		if (PonderIndex.editingModeActive()) return SPECIFIC.get(sceneId).get(k);
				...
				.forEach(entry -> {
					entry.getValue()
							...
							.forEach(subEntry -> consumer.accept(
									langKeyForSpecific(entry.getKey(), subEntry.getKey()),
									subEntry.getValue()
							));
				});
	}
	@Deprecated(forRemoval = true) public static void record(String namespace, JsonObject object) {
