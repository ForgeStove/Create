...
public class ContinuousOBBCollider extends OBBCollider {
	...
	static boolean separate(
		...
		if (isBestSeperation) {
			...
			mf.collisionPosition = normalizedAxis.scale(signum(TL) * (-rB) - signum(seperation) * .125f);
		}
		return false;
	}
	public static class ContinuousSeparationManifold extends SeparationManifold {
		...
		public Vec3 asSeparationVec(double obbStepHeight) {
			if (isDiscreteCollision) {
				if (stepSeparation <= obbStepHeight) return createSeparationVec(stepSeparation, stepSeparationAxis);
		...
		@Override public Vec3 asSeparationVec() {
