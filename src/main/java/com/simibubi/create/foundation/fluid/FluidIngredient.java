...
public abstract class FluidIngredient implements Predicate<FluidStack> {
	...
	@Override public boolean test(FluidStack t) {
	...
	public static class FluidStackIngredient extends FluidIngredient {
		...
		@Override protected boolean testInternal(FluidStack t) {
			if (!FluidHelper.isSame(t, fluid)) return false;
			if (tagToMatch.isEmpty()) return true;
			CompoundTag tag = t.getOrCreateTag();
			return tag.copy().merge(tagToMatch).equals(tag);
		}
		@Override protected void readInternal(FriendlyByteBuf buffer) {
			fluid = buffer.readRegistryId();
			tagToMatch = buffer.readNbt();
		}
		@Override protected void writeInternal(FriendlyByteBuf buffer) {
			buffer.writeRegistryId(ForgeRegistries.FLUIDS, fluid);
			buffer.writeNbt(tagToMatch);
		}
		@Override protected void readInternal(JsonObject json) {
			FluidStack stack = FluidHelper.deserializeFluidStack(json);
			fluid = stack.getFluid();
			tagToMatch = stack.getOrCreateTag();
		}
		@Override protected void writeInternal(JsonObject json) {
			json.addProperty("fluid", RegisteredObjects.getKeyOrThrow(fluid).toString());
			json.add("nbt", JsonParser.parseString(tagToMatch.toString()));
		}
		@Override protected List<FluidStack> determineMatchingFluidStacks() {
			return ImmutableList.of(tagToMatch.isEmpty()
					? new FluidStack(fluid, amountRequired)
	...
	public static class FluidTagIngredient extends FluidIngredient {
		...
		@Override protected boolean testInternal(FluidStack t) {
			if (tag != null) return FluidHelper.isTag(t, tag);
			for (FluidStack accepted : getMatchingFluidStacks())
				if (FluidHelper.isSame(accepted, t)) return true;
			return false;
		}
		@Override protected void readInternal(FriendlyByteBuf buffer) {
		...
		@Override protected void writeInternal(FriendlyByteBuf buffer) {
			...
			for (FluidStack matchingFluidStack : matchingFluidStacks) {
				buffer.writeFluidStack(matchingFluidStack);
			}
		}
		@Override protected void readInternal(JsonObject json) {
		...
		@Override protected List<FluidStack> determineMatchingFluidStacks() {
			return ForgeRegistries.FLUIDS.tags().getTag(tag).stream().map(f -> {
				if (f instanceof FlowingFluid) return ((FlowingFluid) f).getSource();
				return f;
			}).distinct().map(f -> new FluidStack(f, amountRequired)).collect(Collectors.toList());
		}
	}
}
