...
public class FluidHelper {
	...
	@SuppressWarnings("deprecation") public static boolean isTag(Fluid fluid, TagKey<Fluid> tag) {
		return fluid.is(tag);
	}
	...
	public static SoundEvent getFillSound(FluidStack fluid) {
		...
		if (soundevent == null) soundevent = FluidHelper.isTag(fluid, FluidTags.LAVA)
	...
	public static Fluid convertToFlowing(Fluid fluid) {
		...
		if (fluid instanceof ForgeFlowingFluid) return ((ForgeFlowingFluid) fluid).getFlowing();
		return fluid;
	}
	public static Fluid convertToStill(Fluid fluid) {
		...
		if (fluid instanceof ForgeFlowingFluid) return ((ForgeFlowingFluid) fluid).getSource();
		return fluid;
	}
	...
	public static boolean tryEmptyItemIntoBE(
		...
		if (tank == null || fluidStack.getAmount() != tank.fill(fluidStack, FluidAction.SIMULATE)) return false;
	...
	public static boolean tryFillItemFromBE(
		...
		for (int i = 0; i < tank.getTanks(); i++) {
			...
			if (player.isCreative() || be instanceof CreativeFluidTankBlockEntity) heldItem = heldItem.copy();
	...
	@Nullable private static FluidExchange exchange(
			IFluidHandler fluidTank,
			IFluidHandlerItem fluidItem,
			FluidExchange preferred,
			boolean singleOp,
		...
		for (int tankSlot = 0; tankSlot < fluidTank.getTanks(); tankSlot++) {
			for (int slot = 0; slot < fluidItem.getTanks(); slot++) {
				...
				boolean canMoveToTank =
						(undecided || lockedExchange == FluidExchange.ITEM_TO_TANK) && tankCapacity > 0;
				boolean canMoveToItem =
						(undecided || lockedExchange == FluidExchange.TANK_TO_ITEM) && itemCapacity > 0;
