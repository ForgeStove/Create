...
public class BoxWidget extends ElementWidget {
	public static final Function<BoxWidget, DelegatedStencilElement.ElementRenderer>
			gradientFactory
			= (box) -> (ms, w, h, alpha) -> UIRenderHelper.angledGradient(
			ms,
			90,
			w / 2,
			-2,
	...
	@Override protected void beforeRender(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		super.beforeRender(graphics, mouseX, mouseY, partialTicks);
		if (isHovered != wasHovered) {
			startGradientAnimation(getColorForState(true), getColorForState(false), isHovered);
		}
	...
	@Override public void doRender(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		float fadeValue = fade.getValue(partialTicks);
		if (fadeValue < .1f) return;
		box.withAlpha(fadeValue);
		box.withBackground(customBackground != null
						? customBackground
						: Theme.c(Theme.Key.PONDER_BACKGROUND_TRANSPARENT))
	...
	@Override protected boolean clicked(double pMouseX, double pMouseY) {
		if (!active || !visible) return false;
	...
	private Color getColorForState(boolean first) {
		if (!active) return Theme.p(getDisabledTheme()).get(first);
		if (isHovered) {
			if (first) return customBorderTop != null ? customBorderTop.darker() : Theme.c(getHoverTheme(), true);
			else return customBorderBot != null ? customBorderBot.darker() : Theme.c(getHoverTheme(), false);
		}
		if (first) return customBorderTop != null ? customBorderTop : Theme.c(getIdleTheme(), true);
		else return customBorderBot != null ? customBorderBot : Theme.c(getIdleTheme(), false);
	}
