...
public class ConfirmationScreen extends AbstractSimiScreen {
	...
	public ConfirmationScreen removeTextLines(int amount) {
		if (amount > text.size()) return clearText();
	...
	@Override protected void init() {
		...
		TextStencilElement confirmText = new TextStencilElement(font, tristate ? "Save" : "Confirm").centered(
				true,
				true
		);
		...
		if (tristate) {
			...
			confirmDontSave = new BoxWidget(
					buttonX,
					y + textHeight + 6,
					70,
					16
			).withCallback(() -> accept(Response.ConfirmDontSave));
			confirmDontSave.showingElement(confirmDontSaveText.withElementRenderer(BoxWidget.gradientFactory.apply(
					confirmDontSave)));
		...
		textBackground = new BoxElement().gradientBorder(Theme.p(Theme.Key.BUTTON_DISABLE))
				.withBounds(width + 10, textHeight + 35)
				.at(-5, y - 5);
		if (text.size() == 1) x = (width - font.width(text.get(0))) / 2;
	}
	...
	@Override protected void renderWindowBackground(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		endFrame();
	...
	@Override protected void prepareFrame() {
		UIRenderHelper.swapAndBlitColor(minecraft.getMainRenderTarget(), UIRenderHelper.framebuffer);
		RenderSystem.clear(GL30.GL_STENCIL_BUFFER_BIT | GL30.GL_DEPTH_BUFFER_BIT, Minecraft.ON_OSX);
	}
	@Override protected void endFrame() {
