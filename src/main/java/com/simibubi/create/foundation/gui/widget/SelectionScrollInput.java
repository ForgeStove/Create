...
public class SelectionScrollInput extends ScrollInput {
	...
	@Override protected void updateTooltip() {
		toolTip.clear();
		...
		if (min > this.min) toolTip.add(Components.literal("> ...").withStyle(ChatFormatting.GRAY));
		if (this.max - 1 == max) max++;
		for (int i = min; i < max; i++) {
			if (i == state)
				toolTip.add(Components.empty().append("-> ").append(options.get(i)).withStyle(ChatFormatting.WHITE));
			else toolTip.add(Components.empty().append("> ").append(options.get(i)).withStyle(ChatFormatting.GRAY));
		}
		if (max < this.max) toolTip.add(Components.literal("> ...").withStyle(ChatFormatting.GRAY));
		if (hint != null) toolTip.add(hint.plainCopy().withStyle(s -> s.withColor(HINT_RGB)));
