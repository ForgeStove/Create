...
public class CombinedStencilElement extends StencilElement {
	...
	public static CombinedStencilElement of(
			@Nonnull StencilElement element1,
			@Nonnull StencilElement element2,
			ElementMode mode
	) {
	...
	@Override protected void renderElement(GuiGraphics graphics) {
		if (mode.rendersFirst()) element1.<StencilElement>withBounds(width, height).renderElement(graphics);
		if (mode.rendersSecond()) element2.<StencilElement>withBounds(width, height).renderElement(graphics);
	}
