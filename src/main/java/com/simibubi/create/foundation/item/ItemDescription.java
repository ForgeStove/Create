...
public record ItemDescription(
	...
	@Nullable public static ItemDescription create(Item item, Palette palette) {
		return create(getTooltipTranslationKey(item), palette);
	}
	@Nullable public static ItemDescription create(String translationKey, Palette palette) {
		...
		public ItemDescription build() {
			...
			if (hasDescription || hasControls) {
				...
				for (List<Component> list : Arrays.asList(lines, linesOnShift, linesOnCtrl)) {
					...
					if (shift || ctrl) list.add(hasDescription && hasControls ? 2 : 1, Components.immutableEmpty());
				}
			}
			if (!hasDescription) {
				linesOnCtrl.clear();
