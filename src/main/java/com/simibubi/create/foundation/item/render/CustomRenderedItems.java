...
public class CustomRenderedItems {
	...
	public static void forEach(Consumer<Item> consumer) {
		if (!itemsFiltered) {
			ITEMS.removeIf(item -> !ForgeRegistries.ITEMS.containsValue(item) || !(
					IClientItemExtensions.of(item).getCustomRenderer() instanceof CustomRenderedItemModelRenderer
			));
			itemsFiltered = true;
		}
