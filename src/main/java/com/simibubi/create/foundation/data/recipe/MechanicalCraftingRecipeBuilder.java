...
public class MechanicalCraftingRecipeBuilder {
	...
	public void build(Consumer<FinishedRecipe> p_200467_1_, ResourceLocation p_200467_2_) {
		...
		p_200467_1_.accept(new Result(p_200467_2_, result, count, pattern, key, acceptMirrored, recipeConditions));
	}
	/**
		...
		if (pattern.isEmpty()) {
			...
			for (String s : pattern) {
				for (int i = 0; i < s.length(); ++i) {
					char c0 = s.charAt(i);
					if (!key.containsKey(c0) && c0 != ' ') throw new IllegalStateException("Pattern in recipe "
							+ p_200463_1_
	...
	public static class Result implements FinishedRecipe {
		...
		public void serializeRecipeData(JsonObject p_218610_1_) {
			...
			if (this.count > 1) jsonobject1.addProperty("count", this.count);
		...
		@Nullable public JsonObject serializeAdvancement() {
			return null;
		}
