...
@MethodsReturnNonnullByDefault public interface IPlacementHelper {
	/**
	 ...
	 * 		and call {@link PlacementOffset#withTransform(Function)} if the blocks default state has to be modified
	 * 		before it is placed
	 */
	PlacementOffset getOffset(Player player, Level world, BlockState state, BlockPos pos, BlockHitResult ray);
	//sets the offset's ghost state with the default state of the held block item, this is used in PlacementHelpers
	 ...
	 *               {@link #getOffset(Player, Level, BlockState, BlockPos, BlockHitResult)}<br>
	...
	static void renderArrow(Vec3 center, Vec3 target, Direction arrowPlane, double distanceFromCenter) {
		...
		CreateClient.OUTLINER.showLine("placementArrowA" + center + target, start.add(offset), endA.add(offset))
				.lineWidth(1 / 16f);
		CreateClient.OUTLINER.showLine("placementArrowB" + center + target, start.add(offset), endB.add(offset))
				.lineWidth(1 / 16f);
	}
	...
	static List<Direction> orderedByDistanceOnlyAxis(
		...
		return orderedByDistance(pos, hit,
