...
public class DyedBlockList<T extends Block> implements Iterable<BlockEntry<T>> {
	...
	@SuppressWarnings("unchecked") public BlockEntry<T> get(DyeColor color) {
	...
	@SuppressWarnings("unchecked") public BlockEntry<T>[] toArray() {
		return (BlockEntry<T>[]) Arrays.copyOf(values, values.length);
	}
	@Override public Iterator<BlockEntry<T>> iterator() {
		return new Iterator<>() {
			private int index = 0;
			@Override public boolean hasNext() {
				return index < values.length;
			}
			@SuppressWarnings("unchecked") @Override public BlockEntry<T> next() {
