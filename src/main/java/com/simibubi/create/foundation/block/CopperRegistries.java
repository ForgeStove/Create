...
public class CopperRegistries {
	...
	public static void inject() {
		...
		try {
			...
			@SuppressWarnings("unchecked") Supplier<BiMap<Block, Block>>
					originalWeatheringMapDelegate
					= (Supplier<BiMap<Block, Block>>) delegateField.get(WeatheringCopper.NEXT_BY_BLOCK);
			com.google.common.base.Supplier<BiMap<Block, Block>> weatheringMapDelegate = () -> {
				...
				ErrorHandlingBiConsumer<Supplier<Block>, Supplier<Block>>
						consumer
						= new ErrorHandlingBiConsumer<>((original, weathered) -> {
		...
		Supplier<BiMap<Block, Block>> waxableMapSupplier = Suppliers.memoize(() -> {
			...
			ErrorHandlingBiConsumer<Supplier<Block>, Supplier<Block>>
					consumer
					= new ErrorHandlingBiConsumer<>((original, waxed) -> {
	...
	// Create itself only ever adds BlockEntry objects to these registries, which throw if they are not populated with
	// their
	// Block object. Normally this shouldn't happen as the weathering/waxable maps shouldn't be accessed before block
	// registration is complete, but internal Forge code or other mods may cause this to happen. It is better to catch
	// the
	...
	private static class ErrorHandlingBiConsumer<T, U> implements BiConsumer<T, U> {
		...
		@Nullable private Throwable firstException;
		...
		public void reportExceptions(Logger logger, String type) {
			if (exceptionCount != 0) {
				logger.error(
						"Adding {} copper entries from CopperRegistries encountered {} exception(s)!",
						type,
						exceptionCount
				);
				if (firstException != null) {
