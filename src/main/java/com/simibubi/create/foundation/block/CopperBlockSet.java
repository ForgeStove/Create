...
public class CopperBlockSet {
	public static final Variant<?>[] DEFAULT_VARIANTS = new Variant<?>[]{
			BlockVariant.INSTANCE, SlabVariant.INSTANCE, StairVariant.INSTANCE
	};
	...
	public CopperBlockSet(AbstractRegistrate<?> registrate, String name, String endTextureName,
			Variant<?>[] variants) {
				...
				for (WeatherState state : WEATHER_STATES) {
					...
					if (waxed) {
						...
						CopperRegistries.addWeathering(
								() -> entries[getIndex(
										WEATHER_STATES[state.ordinal() - 1],
										false
								)].get(), entry::get
						);
					}
	...
	protected <T extends Block> BlockEntry<?> createEntry(
		...
		if (!suffix.isEmpty()) name = Lang.nonPluralId(name);
		name += suffix;
		...
		if (waxed) {
			builder.recipe((ctx, prov) -> {
				...
				ShapelessRecipeBuilder.shapeless(RecipeCategory.BUILDING_BLOCKS, ctx.get())
						...
						.save(
								prov, new ResourceLocation(
										ctx.getId().getNamespace(),
										"crafting/" + generalDirectory + ctx.getName() + "_from_honeycomb"
								)
						);
			});
		}
	...
	public static class BlockVariant implements Variant<Block> {
		...
		@Override
		public NonNullFunction<Properties, Block> getFactory(CopperBlockSet blocks, WeatherState state,
				boolean waxed) {
			if (waxed) {
				return Block::new;
			} else {
		...
		@Override public void generateBlockState(
	...
	public static class SlabVariant implements Variant<SlabBlock> {
		...
		@Override public void generateLootTable(
				RegistrateBlockLootTables lootTable,
				SlabBlock block,
				CopperBlockSet blocks,
				WeatherState state,
	...
	public static class StairVariant implements Variant<StairBlock> {
		...
		public NonNullFunction<Properties, StairBlock> getFactory(
			...
			if (!blocks.hasVariant(parent)) {
				throw new IllegalStateException("Cannot add StairVariant '"
						+ this
						+ "' without parent Variant '"
						+ parent.toString()
						+ "'!");
			}
			...
			if (waxed) {
				...
				return p -> {
					WeatheringCopperStairBlock block = new WeatheringCopperStairBlock(
							state,
