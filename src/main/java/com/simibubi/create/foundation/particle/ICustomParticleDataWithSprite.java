...
public interface ICustomParticleDataWithSprite<T extends ParticleOptions> extends ICustomParticleData<T> {
	...
	@Override @OnlyIn(Dist.CLIENT) default ParticleProvider<T> getFactory() {
		throw new IllegalAccessError("This particle type uses a metaFactory!");
	}
	@OnlyIn(Dist.CLIENT) SpriteParticleRegistration<T> getMetaFactory();
	@Override @OnlyIn(Dist.CLIENT) default void register(ParticleType<T> type, RegisterParticleProvidersEvent event) {
