...
public abstract class NumberEntry<T extends Number> extends ValueEntry<T> {
	...
	@Nullable
	public static NumberEntry<? extends Number> create(
			Object type,
			String label,
	...
	public NumberEntry(String label, ForgeConfigSpec.ConfigValue<T> value, ForgeConfigSpec.ValueSpec spec) {
		...
		try {
			...
			if (min.doubleValue() > getTypeMin().doubleValue()) {
				...
				minText.withElementRenderer((ms, width, height, alpha) -> UIRenderHelper.angledGradient(
						ms,
						0,
						0,
						height / 2,
			...
			if (max.doubleValue() < getTypeMax().doubleValue()) {
				...
				maxText.withElementRenderer((ms, width, height, alpha) -> UIRenderHelper.angledGradient(
						ms,
						0,
						0,
						height / 2,
		...
		textField.setResponder(s -> {
			try {
				...
				if (!spec.test(number)) throw new IllegalArgumentException();
				textField.setTextColor(Theme.i(Theme.Key.TEXT));
				setValue(number);
	...
	@Override public void onValueChange(T newValue) {
	...
	@Override public void render(
		...
		if (minText != null) minText.at(textField.getX() - minOffset, textField.getY(), 0)
				.withBounds(minOffset, textField.getHeight())
				.render(graphics);
		if (maxText != null) maxText.at(textField.getX() + textField.getWidth(), textField.getY(), 0)
	...
	public static class IntegerEntry extends NumberEntry<Integer> {
		...
		@Override protected Integer getTypeMin() {
			return Integer.MIN_VALUE;
		}
		@Override protected Integer getTypeMax() {
			return Integer.MAX_VALUE;
		}
		@Override protected Function<String, Integer> getParser() {
			return (string) -> {
	...
	public static class FloatEntry extends NumberEntry<Float> {
		...
		@Override protected Float getTypeMin() {
			return -Float.MAX_VALUE;
		}
		@Override protected Float getTypeMax() {
			return Float.MAX_VALUE;
		}
		@Override protected Function<String, Float> getParser() {
	...
	public static class DoubleEntry extends NumberEntry<Double> {
		...
		@Override protected Double getTypeMin() {
			return (double) -Float.MAX_VALUE;
		}
		@Override protected Double getTypeMax() {
			return (double) Float.MAX_VALUE;
		}
		@Override protected Function<String, Double> getParser() {
