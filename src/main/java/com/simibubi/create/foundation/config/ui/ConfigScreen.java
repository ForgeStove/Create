...
public abstract class ConfigScreen extends AbstractSimiScreen {
	...
	public static final PhysicalFloat cogSpin = PhysicalFloat.create()
			.withLimit(10f)
			.withDrag(0.3)
			.addForce(new Force.Static(.2f));
	public static final BlockState cogwheelState = AllBlocks.LARGE_COGWHEEL.getDefaultState()
	...
	@Override protected void renderWindowBackground(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		new StencilElement() {
			@Override protected void renderStencil(GuiGraphics graphics) {
				renderCog(graphics, partialTicks);
			}
			@Override protected void renderElement(GuiGraphics graphics) {
	...
	protected void renderCog(GuiGraphics graphics, float partialTicks) {
		...
		ms.scale(200, 200, 1);
		GuiGameElement.of(cogwheelState).rotateBlock(22.5, cogSpin.getValue(partialTicks), 22.5).render(graphics);
		ms.popPose();
	}
}
