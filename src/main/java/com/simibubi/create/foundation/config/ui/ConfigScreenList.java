...
public class ConfigScreenList extends ObjectSelectionList<ConfigScreenList.Entry> implements TickableGuiEventListener {
	...
	@Override protected void renderList(GuiGraphics graphics, int p_239229_, int p_239230_, float p_239231_) {
		Window window = minecraft.getWindow();
		double d0 = window.getGuiScale();
		RenderSystem.enableScissor(
				(int) (this.x0 * d0),
				(int) (window.getHeight() - (this.y1 * d0)),
				(int) (this.width * d0),
				(int) (this.height * d0)
		);
	...
	public static abstract class Entry extends ObjectSelectionList.Entry<Entry> implements TickableGuiEventListener {
		...
		@Override public boolean keyPressed(int code, int keyPressed_2_, int keyPressed_3_) {
			return getGuiListeners().stream().anyMatch(l -> l.keyPressed(code, keyPressed_2_, keyPressed_3_));
		}
		@Override public boolean charTyped(char ch, int code) {
	...
	public static class LabeledEntry extends Entry {
		...
		public LabeledEntry(String label) {
			...
			this.label.withElementRenderer((graphics, width, height, alpha) -> UIRenderHelper.angledGradient(
					graphics,
					0,
					0,
					height / 2,
					height,
					width,
					Theme.p(Theme.Key.TEXT_ACCENT_STRONG)
			));
		...
		@Override public void render(
			...
			if (unit != null) {
				...
				graphics.drawString(
						font,
						unit,
						x + getLabelWidth(width) - unitWidth - 5,
						y + height / 2 + 2,
