...
public class ConfigHelper {
	...
	private static final LoadingCache<String, EnumMap<ModConfig.Type, ModConfig>>
			configCache
			= CacheBuilder.newBuilder().expireAfterAccess(5, TimeUnit.MINUTES).build(new CacheLoader<>() {
		@Override public EnumMap<ModConfig.Type, ModConfig> load(@Nonnull String key) {
	...
	private static EnumMap<ModConfig.Type, ModConfig> findModConfigsUncached(String modID) {
		...
		EnumMap<ModConfig.Type, ModConfig> configs = ObfuscationReflectionHelper.getPrivateValue(
				ModContainer.class,
				modContainer,
				"configs"
		);
	...
	public static IConfigSpec<?> findConfigSpecFor(ModConfig.Type type, String modID) {
		if (!modID.equals(Create.ID)) return configCache.getUnchecked(modID).get(type).getSpec();
		return AllConfigs.byType(type).specification;
	}
	@Nullable public static ForgeConfigSpec findForgeConfigSpecFor(ModConfig.Type type, String modID) {
	...
	public static boolean hasAnyConfig(String modID) {
		if (!modID.equals(Create.ID)) return !configCache.getUnchecked(modID).isEmpty();
		return true;
	}
	public static boolean hasAnyForgeConfig(String modID) {
		if (!modID.equals(Create.ID)) return configCache.getUnchecked(modID)
				.values()
				.stream()
	...
	public static <T> void setConfigValue(ConfigPath path, String value) throws InvalidValueException {
		...
		if (!valueSpec.test(v)) throw new InvalidValueException();
		configValue.set(v);
	}
