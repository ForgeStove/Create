...
public class ValueSettingsPacket extends BlockEntityConfigurationPacket<SmartBlockEntity> {
	...
	@Override protected void writeSettings(FriendlyByteBuf buffer) {
		...
		if (interactHand != null) buffer.writeBoolean(interactHand == InteractionHand.MAIN_HAND);
	...
	@Override protected void applySettings(ServerPlayer player, SmartBlockEntity be) {
	...
	@Override protected void applySettings(SmartBlockEntity be) {
	}
}
