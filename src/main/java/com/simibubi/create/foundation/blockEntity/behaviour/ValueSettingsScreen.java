...
public class ValueSettingsScreen extends AbstractSimiScreen {
	...
	@Override protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (iconMode) for (int i = 0; i <= board.maxValue(); i++)
		...
		for (Component component : board.rows()) {
			...
			if (!iconMode) {
				...
				for (int w = 0; w < valueBarWidth; w += AllGuiTextures.VALUE_SETTINGS_BAR.width - 1)
					UIRenderHelper.drawCropped(
							...
							valueBarX + w,
							y + 1,
							Math.min(AllGuiTextures.VALUE_SETTINGS_BAR.width - 1, valueBarWidth - w),
							8,
							zLevel,
			...
			for (int milestone = 0; milestone < milestoneCount; milestone++) {
				if (iconMode) AllGuiTextures.VALUE_SETTINGS_WIDE_MILESTONE.render(graphics, milestoneX, y + 1);
		...
		if (board.formatter() instanceof ScrollOptionSettingsFormatter sosf) cursorIcon = sosf.getIcon(closest);
		...
		if (cursorIcon != null) {
			...
			if (fadeInWidth > fattestLabel) graphics.drawString(
					font,
					cursorText,
	...
	protected void renderBrassFrame(GuiGraphics graphics, int x, int y, int w, int h) {
		...
		if (w > 8) {
			...
			UIRenderHelper.drawCropped(graphics, x + 4, y + h - 3, w - 8, 3, zLevel,
	...
	@Override public boolean keyReleased(int pKeyCode, int pScanCode, int pModifiers) {
