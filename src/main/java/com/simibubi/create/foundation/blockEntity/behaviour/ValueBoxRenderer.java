...
public class ValueBoxRenderer {
	public static void renderItemIntoValueBox(
		...
		boolean blockItem = modelWithOverrides.isGui3d()
				&& modelWithOverrides.getRenderPasses(filter, false).size() <= 1;
	...
	public static void renderFlatItemIntoValueBox(
		...
		ms.pushPose();
		TransformStack.cast(ms).rotateX(230);
		Matrix3f copy = new Matrix3f(ms.last().normal());
		ms.popPose();
		ms.pushPose();
		TransformStack.cast(ms).translate(0, 0, -1 / 4f).translate(0, 0, 1 / 32f + .001).rotateY(180);
	...
	@SuppressWarnings("deprecation") private static float customZOffset(Item item) {
		float nudge = -.1f;
