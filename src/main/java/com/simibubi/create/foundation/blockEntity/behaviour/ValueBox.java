...
public class ValueBox extends ChasingAABBOutline {
	...
	@Override public void render(PoseStack ms, SuperRenderTypeBuffer buffer, Vec3 camera, float pt) {
	...
	public static class ItemValueBox extends ValueBox {
		...
		@Override public AllIcons getOutline() {
			if (!stack.isEmpty()) return AllIcons.VALUE_BOX_HOVER_6PX;
			return super.getOutline();
		}
		@Override public void renderContents(PoseStack ms, MultiBufferSource buffer) {
			...
			boolean blockItem = modelWithOverrides.isGui3d()
					&& modelWithOverrides.getRenderPasses(stack, false).size() <= 1;
			float scale = 1.5f;
			...
			if (wildcard) ms.translate(-1, 3f, 0);
