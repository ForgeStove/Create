...
public interface ValueSettingsBehaviour extends ClipboardCloneable {
	...
	@Override default String getClipboardKey() {
		return "Settings";
	}
	@Override default boolean writeToClipboard(CompoundTag tag, Direction side) {
	...
	@Override default boolean readFromClipboard(CompoundTag tag, Player player, Direction side, boolean simulate) {
		if (!acceptsValueSettings()) return false;
	...
	default void playFeedbackSound(BlockEntityBehaviour origin) {
		...
		origin.getWorld()
				.playSound(
						null,
						origin.getPos(),
						SoundEvents.NOTE_BLOCK_IRON_XYLOPHONE.get(),
						SoundSource.BLOCKS,
						0.03f,
						1.125f
				);
