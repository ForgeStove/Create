...
public class FilteringRenderer {
	public static void tick() {
		...
		tip.add(label.copy());
		tip.add(Lang.translateDirect(filter.isEmpty()
	...
	public static void renderOnBlockEntity(
		...
		if (behaviour == null) return;
		if (!behaviour.isActive()) return;
		if (behaviour.getFilter().isEmpty() && !(behaviour instanceof SidedFilteringBehaviour)) return;
		...
		if (slotPositioning instanceof Sided sided) {
			...
			for (Direction d : Iterate.directions) {
				...
				else ValueBoxRenderer.renderItemIntoValueBox(filter, ms, buffer, light, overlay);
				ms.popPose();
			}
			sided.fromSide(side);
