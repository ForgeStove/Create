...
public class RecipeFinder {
	private static final Cache<Object, List<Recipe<?>>> cachedSearches = CacheBuilder.newBuilder().build();
	/**
	...
	public static List<Recipe<?>> get(@Nullable Object cacheKey, Level world, Predicate<Recipe<?>> conditions) {
		if (cacheKey == null) return startSearch(world, conditions);
		try {
	...
	private static List<Recipe<?>> startSearch(Level world, Predicate<? super Recipe<?>> conditions) {
		List<Recipe<?>> list = world.getRecipeManager()
				.getRecipes()
				.stream()
				.filter(conditions)
