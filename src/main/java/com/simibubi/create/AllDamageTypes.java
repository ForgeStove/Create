...
public class AllDamageTypes {
	public static final ResourceKey<DamageType> CRUSH = key("crush"), CUCKOO_SURPRISE = key("cuckoo_surprise"),
			FAN_FIRE
					= key("fan_fire"), FAN_LAVA = key("fan_lava"), DRILL = key("mechanical_drill"), ROLLER = key(
			"mechanical_roller"), SAW = key("mechanical_saw"), POTATO_CANNON = key("potato_cannon"), RUN_OVER = key(
			"run_over");
