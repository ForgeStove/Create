...
public abstract class TrainHatInfoProvider implements DataProvider {
	...
	public TrainHatInfoProvider(PackOutput output) {
		this.path = output.createPathProvider(
				...
				TrainHatInfoReloadListener.HAT_INFO_DIRECTORY
		);
	}
	...
	@Override public CompletableFuture<?> run(CachedOutput output) {
		...
		this.createOffsets();
		return CompletableFuture.allOf(this.trainHatOffsets.entrySet().stream().map(entry -> DataProvider.saveStable(
				output,
				TrainHatInfo.CODEC.encodeStart(JsonOps.INSTANCE, entry.getValue())
