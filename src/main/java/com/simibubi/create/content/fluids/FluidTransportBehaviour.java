...
public abstract class FluidTransportBehaviour extends BlockEntityBehaviour {
	...
	@Override public void read(CompoundTag nbt, boolean clientPacket) {
		...
		for (Direction face : Iterate.directions)
			if (nbt.contains(face.getName())) interfaces.computeIfAbsent(face, PipeConnection::new);
	...
	@Override public void write(CompoundTag nbt, boolean clientPacket) {
		...
		if (interfaces == null) return;
		interfaces.values().forEach(connection -> connection.serializeNBT(nbt, clientPacket));
	}
	...
	@Nullable public PipeConnection getConnection(Direction side) {
		createConnectionData();
	...
	@Nullable public PipeConnection.Flow getFlow(Direction side) {
		createConnectionData();
	...
	public void wipePressure() {
		if (interfaces != null) for (Direction d : Iterate.directions) {
	...
	private void createConnectionData() {
		...
		for (Direction d : Iterate.directions)
			if (canHaveFlowToward(blockEntity.getBlockState(), d)) interfaces.put(d, new PipeConnection(d));
	}
	public AttachmentTypes getRenderedRimAttachment(
		...
		if (facingState.getBlock() instanceof PumpBlock
				&& facingState.getValue(PumpBlock.FACING) == direction.getOpposite()) return AttachmentTypes.NONE;
		...
		if (FluidPropagator.hasFluidCapability(world, offsetPos, direction.getOpposite()) && !AllBlocks.HOSE_PULLEY.has(
				facingState)) return AttachmentTypes.DRAIN;
		return AttachmentTypes.RIM;
	}
