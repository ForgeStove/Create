...
public class HosePulleyFluidHandler implements IFluidHandler {
	...
	@Override public int fill(FluidStack resource, FluidAction action) {
		...
		if (action.simulate()) return diff <= 0 ? resource.getAmount() : internalTank.fill(remaining, action);
		if (diff <= 0) {
	...
	private FluidStack drainInternal(int maxDrain, @Nullable FluidStack resource, FluidAction action) {
		...
		if (!predicate.get() || !drainer.pullNext(pos, action.simulate())) return internalTank.drain(maxDrain, action);
		...
		if (resource != null && !returned.isFluidEqual(resource)) return FluidStack.EMPTY;
