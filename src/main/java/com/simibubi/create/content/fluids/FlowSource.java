...
public abstract class FlowSource {
	...
	public static class FluidHandler extends FlowSource {
		...
		public void manageSource(Level world) {
			if (fluidHandler.isPresent() && world.getGameTime() % 20 != 0) return;
			BlockEntity blockEntity = world.getBlockEntity(location.getConnectedPos());
			if (blockEntity != null)
				fluidHandler = blockEntity.getCapability(ForgeCapabilities.FLUID_HANDLER, location.getOppositeFace());
		}
	...
	public static class OtherPipe extends FlowSource {
		...
		@Override public void manageSource(Level world) {
			if (cached != null && cached.get() != null && !cached.get().blockEntity.isRemoved()) return;
			cached = null;
			...
			if (fluidTransportBehaviour != null) cached = new WeakReference<>(fluidTransportBehaviour);
		}
		@Override public FluidStack provideFluid(Predicate<FluidStack> extractionPredicate) {
