...
public class PotionFluidHandler {
	...
	public static FluidIngredient potionIngredient(Potion potion, int amount) {
		return FluidIngredient.fromFluidStack(FluidHelper.copyStackWithAmount(
				PotionFluidHandler.getFluidFromPotionItem(PotionUtils.setPotion(new ItemStack(Items.POTION), potion)),
				amount
		));
	}
	...
	public static FluidStack getFluidFromPotion(Potion potion, BottleType bottleType, int amount) {
		if (potion == Potions.WATER && bottleType == BottleType.REGULAR) return new FluidStack(Fluids.WATER, amount);
	...
	@OnlyIn(Dist.CLIENT) public static void addPotionTooltip(
			FluidStack fs,
			...
			for (MobEffectInstance effectinstance : list) {
				...
				if (effectinstance.getAmplifier() > 0) {
					textcomponent.append(" ")
							.append(Components.translatable("potion.potency." + effectinstance.getAmplifier())
									.getString());
				}
