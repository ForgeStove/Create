...
public class GenericItemEmptying {
	...
	public static boolean canItemBeEmptied(Level world, ItemStack stack) {
		...
		if (AllRecipeTypes.EMPTYING.find(WRAPPER, world).isPresent()) return true;
		...
		for (int i = 0; i < tank.getTanks(); i++) {
			if (tank.getFluidInTank(i).getAmount() > 0) return true;
		}
		return false;
	}
	public static Pair<FluidStack, ItemStack> emptyItem(Level world, ItemStack stack, boolean simulate) {
		...
		if (PotionFluidHandler.isPotionItem(stack)) return PotionFluidHandler.emptyPotion(stack, simulate);
		WRAPPER.setItem(0, stack);
