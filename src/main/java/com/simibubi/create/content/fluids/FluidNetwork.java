...
public class FluidNetwork {
	...
	public void tick() {
			...
			for (Iterator<Pair<BlockFace, PipeConnection>> iterator = frontier.iterator(); iterator.hasNext(); ) {
				...
				for (Direction side : Iterate.directions) {
					...
					if (!adjacent.hasFlow()) {
						...
						if (adjacent.hasPressure() && adjacent.pressure.getSecond() > 0) canRemove = false;
						continue;
					}
		...
		//		drawDebugOutlines();
		if (!source.isPresent()) source = sourceSupplier.get();
		...
		for (boolean simulate : Iterate.trueAndFalse) {
			...
			for (int i = 0; i < handler.getTanks(); i++) {
				...
				if (!contained.isFluidEqual(fluid)) continue;
			...
			if (transfer.isEmpty()) {
				...
				if (!genericExtract.isEmpty() && genericExtract.isFluidEqual(fluid)) transfer = genericExtract;
			}
			...
			while (!availableOutputs.isEmpty() && transfer.getAmount() > 0) {
				...
				for (
					...
					if (simulate) simulatedTransfer += accumulatedFill.getOrDefault(targetHandler, 0);
	...
	private void keepPortableFluidInterfaceEngaged() {
		IFluidHandler handler = source.orElse(null);
		if (!(handler instanceof InterfaceFluidHandler)) return;
	...
	@Nullable private PipeConnection get(BlockFace location) {
	...
	@Nullable private FluidTransportBehaviour getFluidTransfer(BlockPos pos) {
