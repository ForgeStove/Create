...
public class SequencedAssemblyRecipe implements Recipe<RecipeWrapper> {
	...
	public static <C extends Container, R extends ProcessingRecipe<C>> Optional<R> getRecipe(
			Level world,
			C inv,
			RecipeType<R> type,
			Class<R> recipeClass
	...
	public static <C extends Container, R extends ProcessingRecipe<C>> Optional<R> getRecipe(
			...
			RecipeType<R> type,
			Class<R> recipeClass,
			Predicate<? super R> recipeFilter
	) {
		return getRecipes(world, inv.getItem(0), type, recipeClass).filter(recipeFilter).findFirst();
	}
	public static <R extends ProcessingRecipe<?>> Optional<R> getRecipe(
			Level world,
			ItemStack item,
			RecipeType<R> type,
			Class<R> recipeClass
		...
		for (SequencedAssemblyRecipe sequencedAssemblyRecipe : all) {
			if (!sequencedAssemblyRecipe.appliesTo(item)) continue;
	...
	public static <R extends ProcessingRecipe<?>> Stream<R> getRecipes(
			Level world,
			ItemStack item,
			RecipeType<R> type,
			Class<R> recipeClass
		...
		return all.stream()
				...
				.filter(it -> it.getSecond().getType() == type && recipeClass.isInstance(it.getSecond()))
				.map(it -> {
					it.getSecond().enforceNextResult(() -> it.getFirst().advance(item));
					return it.getSecond();
				})
	...
	@Override public boolean matches(RecipeWrapper inv, Level p_77569_2_) {
		return false;
	}
	@Override public ItemStack assemble(RecipeWrapper inv, RegistryAccess registryAccess) {
		return ItemStack.EMPTY;
	}
	@Override public boolean canCraftInDimensions(int p_194133_1_, int p_194133_2_) {
		return false;
	}
	@Override public ItemStack getResultItem(RegistryAccess registryAccess) {
		return resultPool.get(0).getStack();
	}
	...
	@OnlyIn(Dist.CLIENT) public static void addToTooltip(ItemTooltipEvent event) {
		ItemStack stack = event.getItemStack();
		if (!stack.hasTag() || !stack.getTag().contains("SequencedAssembly")) return;
		...
		for (int i = 0; i < length; i++) {
			...
			else tooltip.add(Components.literal("-> ").append(textComponent).withStyle(ChatFormatting.DARK_AQUA));
		}
	}
