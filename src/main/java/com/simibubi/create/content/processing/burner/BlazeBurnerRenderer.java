...
public class BlazeBurnerRenderer extends SafeBlockEntityRenderer<BlazeBurnerBlockEntity> {
	...
	private static void renderShared(
		...
		if (heatLevel.isAtLeast(HeatLevel.SEETHING)) {
			...
			blazeModel = blockAbove && heatLevel.isAtLeast(HeatLevel.KINDLED)
		...
		if (drawHat) {
			...
			if (blazeModel == AllPartialModels.BLAZE_INERT) {
				hatBuffer.translateY(0.5f).centre().scale(0.75f).unCentre();
			} else {
				hatBuffer.translateY(0.75f);
			}
			hatBuffer.rotateCentered(Direction.UP, horizontalAngle + Mth.PI)
	...
	private static void draw(SuperByteBuffer buffer, float horizontalAngle, PoseStack ms, VertexConsumer vc) {
		buffer.rotateCentered(Direction.UP, horizontalAngle).light(LightTexture.FULL_BRIGHT).renderInto(ms, vc);
	}
}
