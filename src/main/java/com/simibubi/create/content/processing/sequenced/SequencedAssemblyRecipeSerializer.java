...
public class SequencedAssemblyRecipeSerializer implements RecipeSerializer<SequencedAssemblyRecipe> {
	...
	protected SequencedAssemblyRecipe readFromJson(ResourceLocation recipeId, JsonObject json) {
		...
		if (GsonHelper.isValidNode(json, "loops")) recipe.loops = GsonHelper.getAsInt(json, "loops");
		return recipe;
	}
	...
	@Override public final SequencedAssemblyRecipe fromJson(ResourceLocation id, JsonObject json) {
		return readFromJson(id, json);
	}
	@Override public final void toNetwork(FriendlyByteBuf buffer, SequencedAssemblyRecipe recipe) {
		writeToBuffer(buffer, recipe);
	}
	@Override public final SequencedAssemblyRecipe fromNetwork(ResourceLocation id, FriendlyByteBuf buffer) {
