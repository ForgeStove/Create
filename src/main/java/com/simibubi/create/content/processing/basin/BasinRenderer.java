...
public class BasinRenderer extends SmartBlockEntityRenderer<BasinBlockEntity> {
	...
	@Override protected void renderSafe(
		...
		ms.translate(.5, .2f, .5);
		TransformStack.cast(ms).rotateY(basin.ingredientRotation.getValue(partialTicks));
		...
		for (int slot = 0; slot < inv.getSlots(); slot++) {
			...
			TransformStack.cast(ms).rotateY(anglePartition * itemCount + 35).rotateX(65);
