...
public abstract class ProcessingRecipe<T extends Container> implements Recipe<T> {
	...
	private void validate(ResourceLocation recipeTypeId) {
		...
		if (ingredientCount > getMaxInputCount()) logger.warn(
				"{} has more item inputs ({}) than supported ({}).",
				messageHeader,
				ingredientCount,
				getMaxInputCount()
		);
		if (outputCount > getMaxOutputCount()) logger.warn(
				"{} has more item outputs ({}) than supported ({}).",
				messageHeader,
				outputCount,
				getMaxOutputCount()
		);
		if (processingDuration > 0 && !canSpecifyDuration())
			logger.warn("{} specified a duration. Durations have no impact on this type of recipe.", messageHeader);
		if (requiredHeat != HeatCondition.NONE && !canRequireHeat()) logger.warn(
				"{} specified a heat condition. Heat conditions have no impact on this type of recipe.",
				messageHeader
		);
		ingredientCount = fluidIngredients.size();
		outputCount = fluidResults.size();
		if (ingredientCount > getMaxFluidInputCount()) logger.warn(
				"{} has more fluid inputs ({}) than supported ({}).",
				messageHeader,
				ingredientCount,
				getMaxFluidInputCount()
		);
		if (outputCount > getMaxFluidOutputCount()) logger.warn(
				"{} has more fluid outputs ({}) than supported ({}).",
				messageHeader,
				outputCount,
	...
	// IRecipe<> paperwork
	@Override public ItemStack assemble(T inv, RegistryAccess registryAccess) {
