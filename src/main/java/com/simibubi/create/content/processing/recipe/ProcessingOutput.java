...
public class ProcessingOutput {
	...
	public ItemStack rollOutput() {
		...
		for (int roll = 0; roll < stack.getCount(); roll++)
			if (r.nextFloat() > chance) outputAmount--;
	...
	public JsonElement serialize() {
		...
		ResourceLocation resourceLocation = compatDatagenOutput == null
				? RegisteredObjects.getKeyOrThrow(stack.getItem())
				: compatDatagenOutput.getFirst();
		...
		if (stack.hasTag()) json.add("nbt", JsonParser.parseString(stack.getTag().toString()));
		if (chance != 1) json.addProperty("chance", chance);
		return json;
	}
	public static ProcessingOutput deserialize(JsonElement je) {
		if (!je.isJsonObject()) throw new JsonSyntaxException("ProcessingOutput must be a json object");
		...
		if (GsonHelper.isValidNode(json, "nbt")) {
			try {
				...
				itemstack.setTag(TagParser.parseTag(element.isJsonObject()
