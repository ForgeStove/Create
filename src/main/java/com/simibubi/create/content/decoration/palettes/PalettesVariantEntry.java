...
public class PalettesVariantEntry {
	...
	public PalettesVariantEntry(String name, AllPaletteStoneTypes paletteStoneVariants) {
		...
		for (PaletteBlockPattern pattern : paletteStoneVariants.variantTypes) {
			BlockBuilder<? extends Block, CreateRegistrate> builder = REGISTRATE.block(
							pattern.createName(name),
							pattern.getBlockFactory()
					)
			...
			builder.recipe((c, p) -> {
				p.stonecutting(DataIngredient.tag(paletteStoneVariants.materialTag), RecipeCategory.BUILDING_BLOCKS,
						c);
		...
		Create.REGISTRATE.addDataGenerator(
				...
				p -> p.addTag(paletteStoneVariants.materialTag).add(baseBlock.get().asItem())
		);
