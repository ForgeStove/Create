...
public enum AllPaletteStoneTypes {
	...
	TUFF(VANILLA_RANGE, r -> () -> Blocks.TUFF), ASURINE(
			STANDARD_RANGE,
			r -> r.paletteStoneBlock("asurine", () -> Blocks.DEEPSLATE, true, true)
					.properties(p -> p.destroyTime(1.25f).mapColor(MapColor.COLOR_BLUE))
					.register()
	), CRIMSITE(
			STANDARD_RANGE,
			r -> r.paletteStoneBlock("crimsite", () -> Blocks.DEEPSLATE, true, true)
					.properties(p -> p.destroyTime(1.25f).mapColor(MapColor.COLOR_RED))
					.register()
	), LIMESTONE(
			STANDARD_RANGE,
			r -> r.paletteStoneBlock("limestone", () -> Blocks.SANDSTONE, true, false)
			...
			STANDARD_RANGE,
			r -> r.paletteStoneBlock("scoria", () -> Blocks.BLACKSTONE, true, false)
					.properties(p -> p.mapColor(MapColor.COLOR_BROWN))
					.register()
	), SCORCHIA(
			STANDARD_RANGE,
			r -> r.paletteStoneBlock("scorchia", () -> Blocks.BLACKSTONE, true, false)
					.properties(p -> p.mapColor(MapColor.TERRACOTTA_GRAY))
					.register()
	), VERIDIUM(
			STANDARD_RANGE,
			r -> r.paletteStoneBlock("veridium", () -> Blocks.TUFF, true, true)
	...
	public static void register(CreateRegistrate registrate) {
		for (AllPaletteStoneTypes paletteStoneVariants : values()) {
			...
			paletteStoneVariants.materialTag = AllTags.optionalTag(
					...
					Create.asResource("stone_types/" + id)
			);
