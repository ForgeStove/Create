...
public abstract class CopycatModel extends BakedModelWrapperWithData {
	...
	private void gatherOcclusionData(
		...
		for (Direction face : Iterate.directions) {
			...
			if (state.supportsExternalFaceHiding() && neighbourState.hidesNeighborFace(
					world,
					neighbourPos,
					state,
					face.getOpposite()
			...
			if (!Block.shouldRenderFace(material, world, pos, face, neighbourPos)) occlusionData.occlude(face);
		}
	}
	@Override
	public List<BakedQuad> getQuads(
		...
		if (renderType != null && !Minecraft.getInstance()
				...
				.contains(renderType)) return super.getQuads(state, side, rand, data, renderType);
	...
	@Nullable public static BlockState getMaterial(ModelData data) {
