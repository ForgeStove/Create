...
public class SchematicTableScreen extends AbstractSimiContainerScreen<SchematicTableMenu> {
	...
	@Override protected void renderBg(GuiGraphics graphics, float partialTicks, int mouseX, int mouseY) {
		...
		if (schematicsArea == null) graphics.drawString(font, noSchematics, x + 54, y + 26, 0xD3D3D3);
		GuiGameElement.of(renderedItem)
				.<GuiGameElement.GuiRenderBuilder>at(x + background.width, y + background.height - 40, -200)
				.scale(3)
				.render(graphics);
		int width = (int) (
				SCHEMATIC_TABLE_PROGRESS.width * Mth.lerp(partialTicks, lastChasingProgress, chasingProgress)
		);
		...
		graphics.blit(
				...
				x + 70,
				y + 57,
				SCHEMATIC_TABLE_PROGRESS.startX,
				SCHEMATIC_TABLE_PROGRESS.startY,
				width,
				height
		);
	}
