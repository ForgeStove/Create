...
public class SchematicannonRenderer extends SafeBlockEntityRenderer<SchematicannonBlockEntity> {
	...
	@Override protected void renderSafe(
		...
		if (blocksLaunching) renderLaunchedBlocks(blockEntity, partialTicks, ms, buffer, light, overlay);
	...
	public static double[] getCannonAngles(SchematicannonBlockEntity blockEntity, BlockPos pos, float partialTicks) {
		...
		if (target != null) {
			...
			if (blockEntity.previousTarget != null) {
				diff = (
						Vec3.atLowerCornerOf(blockEntity.previousTarget)
		...
		for (LaunchedItem launched : blockEntity.flyingBlocks) {
			...
			if (launched instanceof ForBlockState) {
				...
				Minecraft.getInstance()
						.getBlockRenderer()
						.renderSingleBlock(state, ms, buffer, light, overlay, ModelUtil.VIRTUAL_DATA, null);
	...
	@Override public boolean shouldRenderOffScreen(SchematicannonBlockEntity blockEntity) {
		return true;
	}
