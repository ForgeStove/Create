...
public class SchematicUploadPacket extends SimplePacketBase {
	...
	public SchematicUploadPacket(FriendlyByteBuf buffer) {
		...
		if (code == BEGIN) size = buffer.readLong();
		if (code == WRITE) data = buffer.readByteArray();
	}
	@Override public void write(FriendlyByteBuf buffer) {
		buffer.writeInt(code);
		buffer.writeUtf(schematic);
		if (code == BEGIN) buffer.writeLong(size);
		if (code == WRITE) buffer.writeByteArray(data);
	}
	@Override public boolean handle(Context context) {
		context.enqueueWork(() -> {
			...
			if (code == WRITE) Create.SCHEMATIC_RECEIVER.handleWriteRequest(player, schematic, data);
			if (code == FINISH) Create.SCHEMATIC_RECEIVER.handleFinishedUpload(player, schematic);
		});
		return true;
	}
}
