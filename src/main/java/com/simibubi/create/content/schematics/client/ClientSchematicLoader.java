...
@OnlyIn(Dist.CLIENT) public class ClientSchematicLoader {
		...
		if (size > maxSize * 1000) {
			...
			if (player != null) {
				player.displayClientMessage(
						Lang.translateDirect("schematics.uploadTooLarge")
								.append(" (" + size / 1000 + " KB)."), false
				);
				player.displayClientMessage(
						Lang.translateDirect("schematics.maxAllowedSize")
								.append(" " + maxSize + " KB"), false
				);
			}
			return false;
		}
	...
	public void refresh() {
		...
		try {
			Files.list(Paths.get("schematics/"))
					.filter(f -> !Files.isDirectory(f) && f.getFileName().toString().endsWith(".nbt"))
					.forEach(path -> {
