...
public class SailBlock extends WrenchableDirectionalBlock {
	...
	public InteractionResult use(
		...
		if (frame) return InteractionResult.PASS;
		DyeColor color = DyeColor.getColor(heldItem);
		if (color != null) {
			if (!world.isClientSide) world.playSound(
					null,
					pos,
					SoundEvents.DYE_USE,
					SoundSource.BLOCKS,
	...
	public void applyDye(BlockState state, Level world, BlockPos pos, Vec3 hit, @Nullable DyeColor color) {
		BlockState newState = (
				color == null ? AllBlocks.SAIL_FRAME : AllBlocks.DYED_SAILS.get(color)
		).getDefaultState();
	...
	public ItemStack getCloneItemStack(
		...
		if (pickBlock.isEmpty()) return AllBlocks.SAIL.get().getCloneItemStack(state, target, world, pos, player);
		return pickBlock;
	}
	@Override
