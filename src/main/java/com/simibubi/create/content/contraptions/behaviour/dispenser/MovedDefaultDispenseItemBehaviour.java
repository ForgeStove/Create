...
public class MovedDefaultDispenseItemBehaviour implements IMovedDispenseItemBehaviour {
	...
	public static void doDispense(
		...
		itementity.setDeltaMovement(
				p_82486_0_.random.nextGaussian() * (double) 0.0075F * (double) p_82486_2_
						+ facing.x() * d3
						+ context.motion.x,
				p_82486_0_.random.nextGaussian() * (double) 0.0075F * (double) p_82486_2_
						+ facing.y() * d3
	...
	@Override public ItemStack dispense(ItemStack itemStack, MovementContext context, BlockPos pos) {
		...
		if (inventory == null) {
			...
			if (HopperBlockEntity.addItem(null, inventory, itemStack.copy().split(1), closestToFacing.getOpposite())
					.isEmpty()) itemStack.shrink(1);
			return itemStack;
		}
	}
	/**
	...
	protected ItemStack placeItemInInventory(
			ItemStack consumedFrom,
			ItemStack output,
			MovementContext context,
			BlockPos pos,
			Vec3 facing
		...
		if (!remainder.isEmpty()) DEFAULT_INSTANCE.dispenseStack(output, context, pos, facing);
		return consumedFrom;
	}
}
