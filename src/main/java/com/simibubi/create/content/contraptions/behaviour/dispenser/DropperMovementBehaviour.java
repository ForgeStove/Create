...
public class DropperMovementBehaviour implements MovementBehaviour {
	...
	protected void activate(MovementContext context, BlockPos pos) {
		...
		if (location.isEmpty()) {
			...
			setItemStackAt(
					location,
					DEFAULT_BEHAVIOUR.dispense(getItemStackAt(location, context), context, pos),
					context
			);
		}
	}
	...
	private void collectItems(MovementContext context) {
		getStacks(context).stream()
				...
				.forEach(itemStack -> itemStack.grow(ItemHelper.extract(
						context.contraption.getSharedInventory(),
						(otherItemStack) -> ItemStack.isSameItemSameTags(itemStack, otherItemStack),
	...
	@SuppressWarnings("unchecked") private NonNullList<ItemStack> getStacks(MovementContext context) {
	...
	private ArrayList<DispenseItemLocation> getUseableLocations(MovementContext context) {
		...
		for (int slot = 0; slot < getInvSize(); slot++) {
			...
			if (testStack == null || testStack.isEmpty()) continue;
			if (testStack.getMaxStackSize() == 1) {
				location = new DispenseItemLocation(
						false,
						ItemHelper.findFirstMatchingSlotIndex(
								context.contraption.getSharedInventory(),
								ItemHelper.sameItemPredicate(testStack)
						)
				);
				if (!getItemStackAt(location, context).isEmpty()) useable.add(location);
			} else if (testStack.getCount() >= 2) useable.add(location);
		}
		return useable;
	}
	@Override public void writeExtraData(MovementContext context) {
	...
	protected DispenseItemLocation getDispenseLocation(MovementContext context) {
		...
		if (i < 0) return DispenseItemLocation.NONE;
		else return useableLocations.get(i);
	}
	protected ItemStack getItemStackAt(DispenseItemLocation location, MovementContext context) {
		if (location.internal()) {
			return getStacks(context).get(location.slot());
		} else {
			return context.contraption.getSharedInventory().getStackInSlot(location.slot());
		}
	}
	protected void setItemStackAt(DispenseItemLocation location, ItemStack stack, MovementContext context) {
		if (location.internal()) {
			getStacks(context).set(location.slot(), stack);
		} else {
			context.contraption.getSharedInventory().setStackInSlot(location.slot(), stack);
		}
	}
