...
public abstract class AbstractPulleyInstance<T extends KineticBlockEntity> extends ShaftInstance<T>
		implements DynamicInstance, TickingLightListener {
	...
	public AbstractPulleyInstance(MaterialManager dispatcher, T blockEntity) {
		...
		coil = getCoilModel().createInstance().setPosition(getInstancePosition());
		magnet = new SelectInstance<>(this::getMagnetModelIndex);
		magnet.addModel(getMagnetModel()).addModel(getHalfMagnetModel());
	...
	@Override public void beginFrame() {
		...
		halfRope.update().get().ifPresent(rope1 -> {
			float f = offset % 1;
	...
	@Override public boolean tickLightListener() {
		if (updateVolume()) {
			light.move(volume);
			return true;
		}
	...
	private boolean updateVolume() {
		...
		if (volume.sizeY() < length) {
			volume.assign(pos.below(length), pos).fixMinMax();
			return true;
		}
		return false;
	}
	...
	@Override public boolean decreaseFramerateWithDistance() {
		return false;
	}
	@Override public ImmutableBox getVolume() {
		return volume;
	}
