...
@EventBusSubscriber public class ContraptionHandlerClient {
	@SubscribeEvent @OnlyIn(Dist.CLIENT)
	public static void preventRemotePlayersWalkingAnimations(PlayerTickEvent event) {
		if (event.phase == Phase.START) return;
		if (!(event.player instanceof RemotePlayer remotePlayer)) return;
		...
		for (WeakReference<AbstractContraptionEntity> ref : contraptions) {
			...
			if (contraptionEntity.handlePlayerInteraction(player, pos, face, hand)) {
				AllPackets.getChannel()
						.sendToServer(new ContraptionInteractionPacket(contraptionEntity, hand, pos, face));
	...
	@OnlyIn(Dist.CLIENT) public static Couple<Vec3> getRayInputs(LocalPlayer player) {
		Minecraft mc = Minecraft.getInstance();
