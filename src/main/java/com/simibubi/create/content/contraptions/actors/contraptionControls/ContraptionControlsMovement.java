...
public class ContraptionControlsMovement implements MovementBehaviour {
	...
	@Override public void startMoving(MovementContext context) {
	...
	@Override public void stopMoving(MovementContext context) {
		...
		if (filter != null) context.blockEntityData.putBoolean(
				"Disabled",
	...
	@Override public void tick(MovementContext ctx) {
		...
		if (!(contraption instanceof ElevatorContraption ec)) {
			...
			int value = contraption.isActorTypeDisabled(filter) || contraption.isActorTypeDisabled(ItemStack.EMPTY) ? 4
					* 45 : 0;
		...
		if (!(contraption.presentBlockEntities.get(ctx.localPos) instanceof ContraptionControlsBlockEntity cbe)) return;
		cbe.tickAnimations();
