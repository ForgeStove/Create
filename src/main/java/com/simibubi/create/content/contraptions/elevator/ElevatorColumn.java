...
public class ElevatorColumn {
	public static WorldAttached<Map<ColumnCoords, ElevatorColumn>>
			LOADED_COLUMNS
	...
	@Nullable public static ElevatorColumn get(LevelAccessor level, ColumnCoords coords) {
	...
	public void floorReached(LevelAccessor level, String name) {
		for (BlockPos p : getContacts()) {
			if (level.getBlockEntity(p) instanceof ElevatorContactBlockEntity ecbe) ecbe.updateDisplayedFloor(name);
		}
	}
	...
	public void initNames(Level level) {
		...
		for (int i = 0; i < contacts.size(); i++) {
			...
			for (int peekI = i + 1; peekI < contacts.size(); peekI++) {
				...
				if (!(level.getBlockEntity(peekPos) instanceof ElevatorContactBlockEntity peekEcbe)) continue;
	...
	public void add(BlockPos contactPos) {
		...
		for (; index < contacts.size(); index++)
			if (contacts.get(index) > coord) break;
