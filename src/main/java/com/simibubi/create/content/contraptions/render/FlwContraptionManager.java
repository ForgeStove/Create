...
public class FlwContraptionManager extends ContraptionRenderingWorld<FlwContraption> {
	...
	private void renderStructures(RenderLayerEvent event) {
		...
		ContraptionProgram
				structureShader
				= CreateContexts.STRUCTURE.getProgram(ProgramContext.create(
				Materials.Names.PASSTHRU,
				Formats.BLOCK,
				RenderLayer.getLayer(event.type)
		));
	...
	@Override protected FlwContraption create(Contraption c) {
	...
	@Override public void removeDeadRenderers() {
		boolean removed = renderInfos.values().removeIf(renderer -> {
			if (renderer.isDead()) {
				renderer.invalidate();
				return true;
			}
