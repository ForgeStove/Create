...
public abstract class AbstractPulleyRenderer<T extends KineticBlockEntity> extends KineticBlockEntityRenderer<T> {
	...
	@Override public boolean shouldRenderOffScreen(T p_188185_1_) {
		return true;
	}
	@Override
	protected void renderSafe(
		...
		Axis rotationAxis = (
				(IRotate) be.getBlockState().getBlock()
		).getRotationAxis(be.getBlockState());
		kineticRotationTransform(getRotatedCoil(be), be, rotationAxis, AngleHelper.rad(offset * 180), light).renderInto(ms,
		...
		if (running || offset == 0) renderAt(world, offset > .25f ? magnet : halfMagnet, offset, pos, ms, vb);
		float f = offset % 1;
		if (offset > .75f && (f < .25f || f > .75f)) renderAt(world, halfRope, f > .75f ? f - 1 : f, pos, ms, vb);
		if (!running) return;
	...
	@Override protected BlockState getRenderedBlockState(T be) {
