...
public class RollerActorInstance extends HarvesterActorInstance {
	...
	public RollerActorInstance(
		...
		frame = material.getModel(AllPartialModels.ROLLER_FRAME, context.state).createInstance();
	...
	@Override public void beginFrame() {
		harvester.loadIdentity()
				...
				.translate(0, -.5, .5)
				.rotateY(90);
		frame.loadIdentity().translate(context.localPos).centre().rotateY(horizontalAngle + 180).unCentre();
	}
	@Override protected PartialModel getRollingPartial() {
		return AllPartialModels.ROLLER_WHEEL;
	}
	@Override protected Vec3 getRotationOffset() {
		return Vec3.ZERO;
	}
