...
public interface IMovedDispenseItemBehaviour {
		...
		final IMovedDispenseItemBehaviour spawnEggDispenseBehaviour = new MovedDefaultDispenseItemBehaviour() {
			...
			protected ItemStack dispenseStack(ItemStack itemStack, MovementContext context, BlockPos pos,
				...
				if (context.world instanceof ServerLevel) {
					...
					Entity spawnedEntity = entityType.spawn(
							(ServerLevel) context.world,
							itemStack,
							null,
							pos.offset(BlockPos.containing(facing.x + .7, facing.y + .7, facing.z + .7)),
							MobSpawnType.DISPENSER,
							facing.y < .5,
							false
					);
					if (spawnedEntity != null) spawnedEntity.setDeltaMovement(context.motion.scale(2));
				}
				itemStack.shrink(1);
				return itemStack;
			}
	...
	static void init() {
		...
		DispenserMovementBehaviour.registerMovedDispenseItemBehaviour(
				Items.SPLASH_POTION,
		...
		DispenserMovementBehaviour.registerMovedDispenseItemBehaviour(
				Items.TNT, new MovedDefaultDispenseItemBehaviour() {
					@Override
		...
		DispenserMovementBehaviour.registerMovedDispenseItemBehaviour(
				Items.FIREWORK_ROCKET, new MovedDefaultDispenseItemBehaviour() {
					@Override
					protected ItemStack dispenseStack(
							ItemStack itemStack,
							MovementContext context,
					...
					@Override protected void playDispenseSound(LevelAccessor world, BlockPos pos) {
						world.levelEvent(1004, pos, 0);
					}
				}
		);
		DispenserMovementBehaviour.registerMovedDispenseItemBehaviour(
				Items.FIRE_CHARGE, new MovedDefaultDispenseItemBehaviour() {
					@Override protected void playDispenseSound(LevelAccessor world, BlockPos pos) {
		...
		DispenserMovementBehaviour.registerMovedDispenseItemBehaviour(
				Items.GLASS_BOTTLE, new MovedOptionalDispenseBehaviour() {
					@Override
					protected ItemStack dispenseStack(
							ItemStack itemStack,
							MovementContext context,
						...
						if (state.is(BlockTags.BEEHIVES) && state.getValue(BeehiveBlock.HONEY_LEVEL) >= 5) {
							((BeehiveBlock) block).releaseBeesAndResetHoneyLevel(
									context.world,
									state,
									interactAt,
									null,
		...
		DispenserMovementBehaviour.registerMovedDispenseItemBehaviour(
				Items.BUCKET, new MovedDefaultDispenseItemBehaviour() {
					@Override
					protected ItemStack dispenseStack(
							ItemStack itemStack,
							MovementContext context,
