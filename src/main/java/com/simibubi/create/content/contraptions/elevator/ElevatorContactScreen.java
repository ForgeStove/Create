...
public class ElevatorContactScreen extends AbstractSimiScreen {
	...
	@Override public void init() {
		...
		addRenderableOnly(new TooltipArea(x + 57, y + 23, 147, 18).withTooltip(ImmutableList.of(
				Lang.translate("elevator_contact.floor_description").color(0x5391E1).component(),
				Lang.translate("crafting_blueprint.optional").style(ChatFormatting.GRAY).component(),
				rmbToEdit
		)));
	...
	@Override protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		graphics.drawString(
				...
				(float) (x + (background.width - 8) / 2 - font.width(formattedcharsequence) / 2),
				(float) y + 6,
				0x2F3738,
				false
		);
	...
	@Override public boolean keyPressed(int keyCode, int p_keyPressed_2_, int p_keyPressed_3_) {
	...
	private void confirm() {
		AllPackets.getChannel().sendToServer(new ElevatorContactEditPacket(pos, shortName, longName, doorControl));
		onClose();
	}
}
