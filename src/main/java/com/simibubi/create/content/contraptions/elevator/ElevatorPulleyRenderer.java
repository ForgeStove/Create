...
public class ElevatorPulleyRenderer extends KineticBlockEntityRenderer<ElevatorPulleyBlockEntity> {
	...
	@Override protected void renderSafe(
		...
		if (running || offset == 0) AbstractPulleyRenderer.renderAt(
				world,
				magnet.centre().rotateY(blockStateAngle).unCentre(),
				offset,
				pos,
				ms,
				vb
		...
		float f = offset % 1;
		if (f < .25f || f > .75f) {
			halfRope.centre().rotateY(blockStateAngle).unCentre();
		...
		for (int i = 0; i < offset - .25f; i++) {
			rope.centre().rotateY(blockStateAngle).unCentre();
			AbstractPulleyRenderer.renderAt(
					world,
	...
	@Override protected BlockState getRenderedBlockState(ElevatorPulleyBlockEntity be) {
	...
	@Override public boolean shouldRenderOffScreen(ElevatorPulleyBlockEntity p_188185_1_) {
		return true;
	}
}
