...
public class FunnelMovementBehaviour implements MovementBehaviour {
	...
	@Override public Vec3 getActiveAreaOffset(MovementContext context) {
		...
		if (facing != Direction.UP) return vec.scale(context.state.getValue(FunnelBlock.EXTRACTING) ? .15 : .65);
		return vec.scale(.65);
	}
	...
	private void extract(MovementContext context, BlockPos pos) {
		...
		if (context.state.getValue(FunnelBlock.FACING) != Direction.DOWN) entityPos = entityPos.add(0, -.5f, 0);
		if (!world.getBlockState(pos).getCollisionShape(world, pos).isEmpty()) return;
		if (!world.getEntitiesOfClass(ItemEntity.class, new AABB(BlockPos.containing(entityPos))).isEmpty()) return;
