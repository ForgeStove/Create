...
public class FilterScreen extends AbstractFilterScreen<FilterMenu> {
	...
	private final Component allowN = Lang.translateDirect(PREFIX + "allow_list");
	private final Component allowDESC = Lang.translateDirect(PREFIX + "allow_list.description");
	...
	private final Component respectDataN = Lang.translateDirect(PREFIX + "respect_data");
	private final Component respectDataDESC = Lang.translateDirect(PREFIX + "respect_data.description");
	private final Component ignoreDataN = Lang.translateDirect(PREFIX + "ignore_data");
	private final Component ignoreDataDESC = Lang.translateDirect(PREFIX + "ignore_data.description");
	...
	@Override protected boolean isIndicatorOn(Indicator indicator) {
		if (indicator == blacklistIndicator) return menu.blacklist;
		if (indicator == whitelistIndicator) return !menu.blacklist;
		if (indicator == respectNBTIndicator) return menu.respectNBT;
		if (indicator == ignoreNBTIndicator) return !menu.respectNBT;
		return false;
	}
}
