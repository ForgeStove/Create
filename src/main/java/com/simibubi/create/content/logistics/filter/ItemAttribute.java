...
public interface ItemAttribute {
	...
	ItemAttribute standard = register(StandardTraits.DUMMY);
	ItemAttribute inTag = register(new InTag(ItemTags.LOGS));
	ItemAttribute inItemGroup = register(InItemGroup.EMPTY);
	ItemAttribute addedBy = register(new AddedBy("dummy"));
	...
	@Nullable static ItemAttribute fromNBT(CompoundTag nbt) {
		for (ItemAttribute itemAttribute : types)
			if (itemAttribute.canRead(nbt)) return itemAttribute.readNBT(nbt.getCompound(itemAttribute.getNBTKey()));
		return null;
	}
	...
	@OnlyIn(value = Dist.CLIENT) default MutableComponent format(boolean inverted) {
		return Lang.translateDirect(
				"item_attributes." + getTranslationKey() + (inverted ? ".inverted" : ""),
				getTranslationParameters()
		);
	}
	enum StandardTraits implements ItemAttribute {
		DUMMY(s -> false),
		...
		CRUSHABLE((s, w) -> testRecipe(s, w, AllRecipeTypes.CRUSHING.getType()) || testRecipe(
				s,
				w,
		...
		StandardTraits(BiPredicate<ItemStack, Level> test) {
			this.testWithWorld = test;
		}
		@Override public boolean appliesTo(ItemStack stack, Level world) {
			if (testWithWorld != null) return testWithWorld.test(stack, world);
			return appliesTo(stack);
		}
		@Override public boolean appliesTo(ItemStack stack) {
			return test.test(stack);
		}
		@Override public List<ItemAttribute> listAttributesOf(ItemStack stack, Level world) {
			List<ItemAttribute> attributes = new ArrayList<>();
			for (StandardTraits trait : values())
				if (trait.appliesTo(stack, world)) attributes.add(trait);
			return attributes;
		}
		@Override public List<ItemAttribute> listAttributesOf(ItemStack stack) {
			return null;
		}
		@Override public String getTranslationKey() {
			return Lang.asId(name());
		}
		@Override public String getNBTKey() {
			return "standard_trait";
		}
		@Override public void writeNBT(CompoundTag nbt) {
			nbt.putBoolean(name(), true);
		}
		@Override public ItemAttribute readNBT(CompoundTag nbt) {
			for (StandardTraits trait : values())
				if (nbt.contains(trait.name())) return trait;
			return null;
		}
	}
	...
	class InItemGroup implements ItemAttribute {
		...
		@Override public ItemAttribute readNBT(CompoundTag nbt) {
			return nbt.contains("group")
					? new InItemGroup(CreativeModeTabRegistry.getTab(new ResourceLocation(nbt.getString("group"))))
					: EMPTY;
		}
	}
	class AddedBy implements ItemAttribute {
		...
		@Override public List<ItemAttribute> listAttributesOf(ItemStack stack) {
			...
			return id == null ? Collections.emptyList() : List.of(new AddedBy(id));
		}
		@Override public String getTranslationKey() {
			return "added_by";
		}
