...
public abstract class AbstractChuteBlock extends Block implements IWrenchable, IBE<ChuteBlockEntity> {
	...
	@Nullable public static Direction getChuteFacing(BlockState state) {
	...
	@Override public void updateEntityAfterFallOn(BlockGetter worldIn, Entity entityIn) {
		...
		DirectBeltInputBehaviour input = BlockEntityBehaviour.get(
				entityIn.level(),
				BlockPos.containing(entityIn.position().add(0, 0.5f, 0)).below(),
	...
	@Override public void neighborChanged(
		...
		if (pos.below().equals(neighbourPos)) withBlockEntityDo(world, pos, ChuteBlockEntity::blockBelowChanged);
	...
	@Override public Class<ChuteBlockEntity> getBlockEntityClass() {
	...
	@Override public InteractionResult use(
		...
		return onBlockEntityUse(
				world, pos, be -> {
					if (be.item.isEmpty()) return InteractionResult.PASS;
