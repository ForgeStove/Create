...
public class ShulkerFillLevelAttribute implements ItemAttribute {
	...
	@Override public void writeNBT(CompoundTag nbt) {
		if (levels != null) nbt.putString("id", levels.key);
	}
	...
	enum ShulkerLevels {
		...
		@Nullable public static ShulkerLevels fromKey(String key) {
		...
		public boolean canApply(ItemStack testStack) {
			...
			if (compoundnbt == null) return requiredSize.test(0);
			if (compoundnbt.contains("LootTable", 8)) return false;
