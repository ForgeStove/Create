...
public class AttributeFilterScreen extends AbstractFilterScreen<AttributeFilterMenu> {
	...
	@Override protected void init() {
		...
		handleIndicators();
		attributeSelectorLabel = new Label(x + 43, y + 28, Components.immutableEmpty()).colored(0xF3EBDE).withShadow();
		attributeSelector = new SelectionScrollInput(x + 39, y + 23, 137, 18);
		attributeSelector.forOptions(Collections.singletonList(Components.immutableEmpty()));
		...
		menu.selectedAttributes.forEach(at -> selectedAttributes.add(Components.literal("- ")
				.append(at.getFirst().format(at.getSecond()))
	...
	@Override protected void containerTick() {
		...
		if (!stackInSlot.equals(lastItemScanned, false)) referenceItemChanged(stackInSlot);
	}
	...
	@Override protected List<IconButton> getTooltipButtons() {
		return Arrays.asList(blacklist, whitelistCon, whitelistDis);
	}
	@Override protected List<MutableComponent> getTooltipDescriptions() {
		return Arrays.asList(denyDESC.plainCopy(), allowConDESC.plainCopy(), allowDisDESC.plainCopy());
	}
	@Override protected List<Indicator> getIndicators() {
	...
	protected boolean handleAddedAttibute(boolean inverted) {
		...
		selectedAttributes.add(Components.literal("- ")
				.append(itemAttribute.format(inverted))
				.withStyle(ChatFormatting.GRAY));
		return true;
	}
	@Override protected void contentsCleared() {
		selectedAttributes.clear();
	...
	@Override protected boolean isButtonEnabled(IconButton button) {
		if (button == blacklist) return menu.whitelistMode != WhitelistMode.BLACKLIST;
		if (button == whitelistCon) return menu.whitelistMode != WhitelistMode.WHITELIST_CONJ;
		if (button == whitelistDis) return menu.whitelistMode != WhitelistMode.WHITELIST_DISJ;
		return true;
	}
	@Override protected boolean isIndicatorOn(Indicator indicator) {
		if (indicator == blacklistIndicator) return menu.whitelistMode == WhitelistMode.BLACKLIST;
		if (indicator == whitelistConIndicator) return menu.whitelistMode == WhitelistMode.WHITELIST_CONJ;
		if (indicator == whitelistDisIndicator) return menu.whitelistMode == WhitelistMode.WHITELIST_DISJ;
		return false;
	}
}
