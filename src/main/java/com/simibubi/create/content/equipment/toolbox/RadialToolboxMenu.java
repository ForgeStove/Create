...
public class RadialToolboxMenu extends AbstractSimiScreen {
	...
	@Override protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		if (state == State.DETACH) {
			...
			if (hoveredX > -20 && hoveredX < 20 && hoveredY > -80 && hoveredY < -20) hoveredSlot = UNEQUIP;
			ms.pushPose();
			AllGuiTextures.TOOLBELT_INACTIVE_SLOT.render(graphics, -12, -12);
			GuiGameElement.of(AllBlocks.TOOLBOXES.get(DyeColor.BROWN).asStack()).at(-9, -9).render(graphics);
			...
			if (hoveredX > 60 && hoveredX < 100 && hoveredY > -20 && hoveredY < 20) hoveredSlot = DEPOSIT;
			ms.pushPose();
			...
			for (int slot = 0; slot < 8; slot++) {
				...
				if (state == State.SELECT_ITEM || state == State.SELECT_ITEM_UNEQUIP) {
					...
					if (slot < toolboxes.size()) {
						...
						GuiGameElement.of(AllBlocks.TOOLBOXES.get(toolboxBlockEntity.getColor()).asStack())
								.at(3, 3)
								.render(graphics);
			...
			if (renderCenterSlot) {
				...
				if (!scrollMode && UNEQUIP == hoveredSlot) {
					AllGuiTextures.TOOLBELT_SLOT_HIGHLIGHT.render(graphics, -13, -13);
					tip = Lang.translateDirect("toolbox.unequip", minecraft.player.getMainHandItem().getHoverName())
							.withStyle(ChatFormatting.GOLD);
				}
				ms.popPose();
			}
	...
	@Override public void removed() {
		...
		if (selected == DEPOSIT) {
			...
			else if (state == State.SELECT_BOX) toolboxes.forEach(be -> AllPackets.getChannel()
					.sendToServer(new ToolboxDisposeAllPacket(be.getBlockPos())));
			else AllPackets.getChannel().sendToServer(new ToolboxDisposeAllPacket(selectedBox.getBlockPos()));
			return;
		}
		if (state == State.SELECT_BOX) return;
		if (state == State.DETACH) {
			if (selected == UNEQUIP) AllPackets.getChannel()
					.sendToServer(new ToolboxEquipPacket(null, selected, minecraft.player.getInventory().selected));
			return;
		}
		if (selected == UNEQUIP) AllPackets.getChannel()
				.sendToServer(new ToolboxEquipPacket(
		...
		if (distance <= 150) {
			...
			for (int i = 0; i < 10; i++) {
				if (state == State.SELECT_ITEM || state == State.SELECT_ITEM_UNEQUIP) {
					...
					if (!stackInSlot.isEmpty() && !inv.getStackInSlot(scrollSlot * STACKS_PER_COMPARTMENT).isEmpty())
						break;
				}
	...
	@Override public boolean keyPressed(int code, int scanCode, int modifiers) {
		...
		for (int i = 0; i < hotbarBinds.length && i < 8; i++) {
			if (hotbarBinds[i].matches(code, scanCode)) {
				if (state == State.SELECT_ITEM || state == State.SELECT_ITEM_UNEQUIP) {
					...
					if (stackInSlot.isEmpty() || inv.getStackInSlot(i * STACKS_PER_COMPARTMENT).isEmpty()) return false;
				}
	...
	@Override public boolean keyReleased(int code, int scanCode, int modifiers) {
