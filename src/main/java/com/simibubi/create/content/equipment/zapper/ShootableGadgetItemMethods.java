...
public class ShootableGadgetItemMethods {
	public static void applyCooldown(
		...
		boolean gunInOtherHand = predicate.test(player.getItemInHand(hand == InteractionHand.MAIN_HAND
	...
	public static boolean shouldSwap(
		...
		boolean gunInOtherHand = predicate.test(player.getItemInHand(mainHand
				? InteractionHand.OFF_HAND
				: InteractionHand.MAIN_HAND));
		// Pass To Offhand
		if (mainHand && isSwap && gunInOtherHand) return true;
		if (mainHand && !isSwap && gunInOtherHand) item.getTag().putBoolean("_Swap", true);
		if (!mainHand && isSwap) item.getTag().remove("_Swap");
		if (!mainHand && gunInOtherHand) player.getItemInHand(InteractionHand.MAIN_HAND).getTag().remove("_Swap");
