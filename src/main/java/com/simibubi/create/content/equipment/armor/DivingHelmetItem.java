...
@EventBusSubscriber public class DivingHelmetItem extends BaseArmorItem {
	...
	@Override public Map<Enchantment, Integer> getAllEnchantments(ItemStack stack) {
	...
	@SubscribeEvent public static void breatheUnderwater(LivingTickEvent event) {
		...
		if (!helmet.getItem().isFireResistant() && lavaDiving) return;
		...
		if (lavaDiving) {
			...
			if (backtanks.stream().noneMatch(backtank -> backtank.getItem().isFireResistant())) return;
		}
		if (drowning) entity.setAirSupply(10);
		if (world.isClientSide) entity.getPersistentData()
				.putInt(
						"VisualBacktankAir",
						Math.round(backtanks.stream().map(BacktankUtil::getAir).reduce(0f, Float::sum))
				);
		if (!second) return;
