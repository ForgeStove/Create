...
public class ToolboxDisposeAllPacket extends SimplePacketBase {
	...
	@Override public boolean handle(Context context) {
		context.enqueueWork(() -> {
			...
			if (player.distanceToSqr(toolboxPos.getX() + 0.5, toolboxPos.getY(), toolboxPos.getZ() + 0.5)
			...
			toolbox.inventory.inLimitedMode(inventory -> {
				for (int i = 0; i < 36; i++) {
					...
					if (compound.contains(key) && NbtUtils.readBlockPos(compound.getCompound(key).getCompound("Pos"))
							.equals(toolboxPos)) {
					...
					if (remainder.getCount() != itemStack.getCount()) player.getInventory().setItem(i, remainder);
				}
			});
