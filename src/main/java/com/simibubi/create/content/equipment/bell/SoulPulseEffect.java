...
public class SoulPulseEffect {
	...
	public void spawnParticles(Level world, BlockPos at) {
		...
		if (canOverlap()) world.addAlwaysVisibleParticle(
				(
						(int) Math.round(VecHelper.getCenterOf(pos).distanceTo(VecHelper.getCenterOf(at)))
				) >= distance ? new SoulParticle.PerimeterData() : new ExpandingPerimeterData(),
				p.x + 0.5,
				p.y + 0.5,
				p.z + 0.5,
				0,
