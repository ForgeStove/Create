...
@ParametersAreNonnullByDefault public class SandPaperPolishingRecipe extends ProcessingRecipe<SandPaperInv> {
	public SandPaperPolishingRecipe(ProcessingRecipeParams params) {
		super(AllRecipeTypes.SANDPAPER_POLISHING, params);
	}
	@Override public boolean matches(SandPaperInv inv, Level worldIn) {
	...
	public static ItemStack applyPolish(Level world, Vec3 position, ItemStack stack, ItemStack sandPaperStack) {
		...
		if (!matchingRecipes.isEmpty())
			return matchingRecipes.get(0).assemble(new SandPaperInv(stack), world.registryAccess()).copy();
		return stack;
	}
