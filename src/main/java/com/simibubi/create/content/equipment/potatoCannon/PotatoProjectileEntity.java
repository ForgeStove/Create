...
public class PotatoProjectileEntity extends AbstractHurtingProjectile implements IEntityAdditionalSpawnData {
	...
	public PotatoCannonProjectileType getProjectileType() {
		if (type == null)
			type = PotatoProjectileTypeManager.getTypeForStack(stack).orElse(BuiltinPotatoProjectileTypes.FALLBACK);
		return type;
	}
	public void setEnchantmentEffectsFromCannon(ItemStack cannon) {
		...
		if (power > 0) additionalDamageMult = 1 + power * .2f;
		if (punch > 0) additionalKnockback = punch * .5f;
		if (flame > 0) setSecondsOnFire(100);
	...
	@Override protected boolean shouldBurn() {
		return false;
	}
	@Override protected void onHitEntity(EntityHitResult ray) {
		...
		if (target instanceof PotatoProjectileEntity ppe) {
			...
			if (ppe.getProjectileType() != getProjectileType()) {
				...
				if (ppe.getOwner() instanceof Player p) AllAdvancements.POTATO_CANNON_COLLIDE.awardTo(p);
			}
		}
		pop(hit);
	...
	private void recoverItem() {
		if (!stack.isEmpty()) spawnAtLocation(ItemHandlerHelper.copyStackWithSize(stack, 1));
	}
	...
	@Override protected void onHitBlock(BlockHitResult ray) {
		...
		if (!getProjectileType().onBlockHit(level(), ray) && !level().isClientSide)
			if (random.nextDouble() <= recoveryChance) recoverItem();
		super.onHitBlock(ray);
		kill();
	}
	@Override public boolean hurt(@NotNull DamageSource source, float amt) {
	...
	@SuppressWarnings("unchecked") public static EntityType.Builder<?> build(EntityType.Builder<?> builder) {
