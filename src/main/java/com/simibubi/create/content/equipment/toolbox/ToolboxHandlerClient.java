...
public class ToolboxHandlerClient {
	...
	public static void onKeyInput(int key, boolean pressed) {
		...
		if (key != AllKeys.TOOLBELT.getBoundCode()) return;
		...
		else ScreenOpener.open(new RadialToolboxMenu(toolboxes, RadialToolboxMenu.State.SELECT_BOX, null));
	}
	public static void renderOverlay(ForgeGui gui, GuiGraphics graphics, float partialTicks, int width, int height) {
		...
		for (int slot = 0; slot < 9; slot++) {
			...
			AllGuiTextures texture = ToolboxHandler.distance(player.position(), pos) < max * max ? selected
					? TOOLBELT_SELECTED_ON
					: TOOLBELT_HOTBAR_ON : selected ? TOOLBELT_SELECTED_OFF : TOOLBELT_HOTBAR_OFF;
