...
public class SoulParticle extends CustomRotationParticle {
	...
	@Override public void render(VertexConsumer builder, Camera camera, float partialTicks) {
		if (!isVisible) return;
	...
	public static class Data extends BasicParticleData<SoulParticle> {
		@Override public IBasicParticleFactory<SoulParticle> getBasicFactory() {
			return (worldIn, x, y, z, vx, vy, vz, spriteSet) -> new SoulParticle(
					worldIn,
					x,
					y,
					z,
	...
	public static class StartAnimation extends AnimationStage {
		...
		@Override public void tick() {
			super.tick();
			particle.setFrame(particle.firstStartFrame + (int) (
					getAnimAge() / (float) particle.startTicks * particle.startFrames
			));
		}
		@Override public AnimationStage getNext() {
	...
	public static class EndAnimation extends AnimationStage {
		...
		@Override public void tick() {
			super.tick();
			particle.setFrame(particle.firstEndFrame + (int) (
					(getAnimAge() / (float) particle.endTicks) * particle.endFrames
			));
		}
		@Override public AnimationStage getNext() {
