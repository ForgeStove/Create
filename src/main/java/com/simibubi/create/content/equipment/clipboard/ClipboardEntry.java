...
public class ClipboardEntry {
	...
	public static List<ClipboardEntry> getLastViewedEntries(ItemStack heldItem) {
		...
		int page = heldItem.getTag() == null
				? 0
				: Math.min(heldItem.getTag().getInt("PreviouslyOpenedPage"), pages.size() - 1);
	...
	public static void saveAll(List<List<ClipboardEntry>> entries, ItemStack clipboardItem) {
		...
		tag.put(
				"Pages", NBTHelper.writeCompoundList(
						entries, list -> {
	...
	public static ClipboardEntry readNBT(CompoundTag tag) {
		...
		if (tag.contains("Icon")) clipboardEntry.displayItem(ItemStack.of(tag.getCompound("Icon")));
