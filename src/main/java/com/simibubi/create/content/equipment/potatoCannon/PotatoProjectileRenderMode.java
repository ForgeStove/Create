...
public interface PotatoProjectileRenderMode {
	@OnlyIn(Dist.CLIENT) void transform(PoseStack ms, PotatoProjectileEntity entity, float pt);
	class Billboard implements PotatoProjectileRenderMode {
		public static final Billboard INSTANCE = new Billboard();
		@Override @OnlyIn(Dist.CLIENT) public void transform(PoseStack ms, PotatoProjectileEntity entity, float pt) {
			...
			TransformStack.cast(ms)
					...
					.rotateX(AngleHelper.deg(Mth.atan2(diff.y,
							Mth.sqrt((float) (diff.x * diff.x + diff.z * diff.z)))));
		}
	}
	class Tumble extends Billboard {
	...
	class TowardMotion implements PotatoProjectileRenderMode {
		...
		@Override @OnlyIn(Dist.CLIENT) public void transform(PoseStack ms, PotatoProjectileEntity entity, float pt) {
			...
			TransformStack.cast(ms)
					...
					.rotateX(270 + AngleHelper.deg(Mth.atan2(
							diff.y,
							-Mth.sqrt((float) (diff.x * diff.x + diff.z * diff.z))
					)));
			TransformStack.cast(ms)
	...
	class StuckToEntity implements PotatoProjectileRenderMode {
