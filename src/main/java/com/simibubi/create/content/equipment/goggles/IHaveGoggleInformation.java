...
public interface IHaveGoggleInformation {
	 ...
	 * Use Lang.[...].forGoggles(list)
	 */
	@Deprecated Component componentSpacing = Components.literal(spacing);
	/**
	...
	default boolean containedFluidTooltip(
		...
		for (int i = 0; i < tank.getTanks(); i++) {
			...
			Lang.builder()
					.add(Lang.number(fluidStack.getAmount()).add(mb).style(ChatFormatting.GOLD))
					.text(ChatFormatting.GRAY, " / ")
					.add(Lang.number(tank.getTankCapacity(i)).add(mb).style(ChatFormatting.DARK_GRAY))
		...
		Lang.translate("gui.goggles.fluid_container.capacity")
				.add(Lang.number(tank.getTankCapacity(0)).add(mb).style(ChatFormatting.GOLD))
