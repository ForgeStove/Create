...
public class ClipboardEditPacket extends SimplePacketBase {
	...
	public ClipboardEditPacket(FriendlyByteBuf buffer) {
		...
		data = buffer.readNbt();
		if (buffer.readBoolean()) targetedBlock = buffer.readBlockPos();
	}
	@Override public void write(FriendlyByteBuf buffer) {
		buffer.writeVarInt(hotbarSlot);
		buffer.writeNbt(data);
		buffer.writeBoolean(targetedBlock != null);
		if (targetedBlock != null) buffer.writeBlockPos(targetedBlock);
	}
	@Override public boolean handle(Context context) {
		context.enqueueWork(() -> {
			...
			if (targetedBlock != null) {
				...
				if (world == null || !world.isLoaded(targetedBlock)) return;
