...
public class SymmetryWandScreen extends AbstractSimiScreen {
	...
	@Override public void init() {
		...
		int x = guiLeft;
		int y = guiTop;
		labelType = new Label(x + 49, y + 28, Components.immutableEmpty()).colored(0xFFFFFFFF).withShadow();
		labelAlign = new Label(x + 49, y + 50, Components.immutableEmpty()).colored(0xFFFFFFFF).withShadow();
	...
	@Override protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		GuiGameElement.of(wand).scale(4).rotate(-70, 20, 20).at(x + 178, y + 448, -150).render(graphics);
	}
