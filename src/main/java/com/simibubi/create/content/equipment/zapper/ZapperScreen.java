...
public abstract class ZapperScreen extends AbstractSimiScreen {
	...
	protected void renderZapper(GuiGraphics graphics, int x, int y) {
		GuiGameElement.of(zapper).scale(4).at(x + background.width, y + background.height - 48, -200).render(graphics);
	}
	@SuppressWarnings("deprecation") protected void renderBlock(GuiGraphics graphics, int x, int y) {
		...
		if (zapper.hasTag() && zapper.getTag().contains("BlockUsed"))
			state = NbtUtils.readBlockState(
					...
					zapper.getTag().getCompound("BlockUsed")
			);
