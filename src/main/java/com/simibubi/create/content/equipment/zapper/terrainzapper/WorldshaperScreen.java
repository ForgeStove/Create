...
public class WorldshaperScreen extends ZapperScreen {
	...
	protected final List<Component> brushOptions = Lang.translatedOptions(
	...
	protected void initBrushParams(int x, int y) {
		...
		for (int index = 0; index < 3; index++) {
			...
			).writingTo(label).titled(currentBrush.getParamLabel(index).plainCopy()).calling(state -> {
		...
		for (int i = 0; i < toolValues.length; i++)
			if (currentTool == toolValues[i]) toolIndex = i;
		if (toolIndex == -1) {
		...
		if (currentBrush.hasPlacementOptions()) {
			...
			for (int id = 0; id < placementValues.length; id++) {
				...
				placementButton.setToolTip(Lang.translateDirect("gui.terrainzapper.placement."
	...
	@Override protected void drawOnBackground(GuiGraphics graphics, int x, int y) {
	...
	@Override protected ConfigureZapperPacket getConfigurationPacket() {
