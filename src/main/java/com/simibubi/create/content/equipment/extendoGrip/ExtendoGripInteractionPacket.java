...
public class ExtendoGripInteractionPacket extends SimplePacketBase {
	...
	@Override public void write(FriendlyByteBuf buffer) {
		...
		if (specificPoint != null) {
			VecHelper.write(specificPoint, buffer);
		}
	}
	@Override public boolean handle(Context context) {
		context.enqueueWork(() -> {
			...
			if (entityByID != null && ExtendoGripItem.isHoldingExtendoGrip(sender)) {
				double d = sender.getAttribute(ForgeMod.BLOCK_REACH.get()).getValue();
				if (!sender.hasLineOfSight(entityByID)) d -= 3;
				d *= d;
				if (sender.distanceToSqr(entityByID) > d) return;
				if (interactionHand == null) sender.attack(entityByID);
				else if (specificPoint == null) sender.interactOn(entityByID, interactionHand);
				else entityByID.interactAt(sender, specificPoint, interactionHand);
			}
		});
		return true;
	}
