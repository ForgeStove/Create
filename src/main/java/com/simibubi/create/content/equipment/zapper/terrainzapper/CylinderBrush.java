...
public class CylinderBrush extends ShapedBrush {
	...
	public CylinderBrush() {
		...
		for (int i = 0; i <= MAX_RADIUS; i++) {
			...
			List<BlockPos> positions = BlockPos.betweenClosedStream(
					...
					.map(BlockPos::new)
					.filter(p -> VecHelper.getCenterOf(p).distanceTo(VecHelper.getCenterOf(BlockPos.ZERO))
							< radius + .42f)
					.toList();
	...
	@Override public BlockPos getOffset(Vec3 ray, Direction face, PlacementOptions option) {
	...
	@Override Component getParamLabel(int paramIndex) {
		return paramIndex == 0 ? Lang.translateDirect("generic.radius") : super.getParamLabel(paramIndex);
	}
	@Override public List<BlockPos> getIncludedPositions() {
