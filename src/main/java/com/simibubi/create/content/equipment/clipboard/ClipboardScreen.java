...
public class ClipboardScreen extends AbstractSimiScreen {
	...
	public void reopenWith(ItemStack clipboard) {
		...
		if (clearBtn == null) {
			currentPage = item.getTag() == null ? 0 : item.getTag().getInt("PreviouslyOpenedPage");
		...
		if (startEmpty) currentEntries.add(new ClipboardEntry(false, Components.empty()));
		editingIndex = 0;
		...
		readonly = item.getTag() != null && item.getTag().getBoolean("Readonly");
	...
	@Override protected void init() {
		...
		clearBtn = new IconButton(x + 234, y + 153, AllIcons.I_CLEAR_CHECKED).withCallback(() -> {
			...
			if (currentEntries.isEmpty()) currentEntries.add(new ClipboardEntry(false, Components.empty()));
			sendIfEditingBlock();
		});
	...
	public void tick() {
		...
		int mx = (int) (
				this.minecraft.mouseHandler.xpos() * (double) this.minecraft.getWindow().getGuiScaledWidth()
						/ (double) this.minecraft.getWindow().getScreenWidth()
		);
		int my = (int) (
				this.minecraft.mouseHandler.ypos() * (double) this.minecraft.getWindow().getGuiScaledHeight()
	...
	@Override protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		graphics.drawString(
				font,
				Components.translatable("book.pageIndicator", currentPage + 1, getNumPages()),
				x + 150,
				y + 9,
				0x43ffffff,
				false
		...
		for (int i = 0; i < currentEntries.size(); i++) {
			...
			for (FormattedCharSequence sequence : split) {
				if (i != editingIndex) graphics.drawString(
						font,
						sequence,
						x + 58 + iconOffset,
						y + 50,
						checked ? 0x31B25D : 0x311A00,
						false
				);
				y += 9;
			}
	...
	@Override public void removed() {
		...
		for (int i = 0; i < pages.size(); i++)
			if (pages.get(i) == currentEntries) item.getOrCreateTag().putInt("PreviouslyOpenedPage", i);
		send();
		super.removed();
	}
	...
	private void send() {
		...
		if (pages.isEmpty()) item.setTag(new CompoundTag());
		AllPackets.getChannel().sendToServer(new ClipboardEditPacket(targetSlot, item.getOrCreateTag(),
				targetedBlock));
	}
	...
	@Override public boolean charTyped(char pCodePoint, int pModifiers) {
		...
		if (!SharedConstants.isAllowedChatCharacter(pCodePoint)) return false;
			...
			switch (pKeyCode) {
				...
				case 335:
					if (hasShiftDown()) {
						...
						if (currentEntries.size() <= editingIndex + 1 || !currentEntries.get(editingIndex
								+ 1).text.getString().isEmpty())
	...
	private DisplayCache rebuildDisplayCache() {
		String s = getCurrentEntryText();
		if (s.isEmpty()) return DisplayCache.EMPTY;
		int i = editContext.getCursorPos();
	...
	@OnlyIn(Dist.CLIENT) static class DisplayCache {
	...
	@OnlyIn(Dist.CLIENT) static class LineInfo {
		final Style style;
	...
	@OnlyIn(Dist.CLIENT) static class Pos2i {
		public final int x;
		public final int y;
		Pos2i(int pX, int pY) {
			x = pX;
