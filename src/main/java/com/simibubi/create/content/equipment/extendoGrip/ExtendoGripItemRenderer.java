...
public class ExtendoGripItemRenderer extends CustomRenderedItemModelRenderer {
	...
	protected static final PartialModel
			THIN_SHORT
			= new PartialModel(Create.asResource("item/extendo_grip/thin_short"));
	protected static final PartialModel
			WIDE_SHORT
			= new PartialModel(Create.asResource("item/extendo_grip/wide_short"));
	...
	@Override protected void render(
		...
		if (leftHand || rightHand) animation = Mth.lerp(
		...
		stacker.rotateX(-oppositeAngle).translate(ROTATION_OFFSET);
		...
		angle %= 360;
		stacker.translate(COG_ROTATION_OFFSET).rotateZ(angle).translateBack(COG_ROTATION_OFFSET);
