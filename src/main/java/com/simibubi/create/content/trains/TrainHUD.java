...
public class TrainHUD {
	...
	public static void tick() {
		...
		double value = Math.abs(train.speed) / (
				train.maxSpeed() * AllConfigs.server().trains.manualTrainSpeedModifier.getF()
		);
	...
	private static Carriage getCarriage() {
		if (!(ControlsHandler.getContraption() instanceof CarriageContraptionEntity cce)) return null;
	...
	public static void renderOverlay(ForgeGui gui, GuiGraphics graphics, float partialTicks, int width, int height) {
		...
		graphics.blit(
				...
				0,
				0,
				AllGuiTextures.TRAIN_HUD_SPEED.startX,
				AllGuiTextures.TRAIN_HUD_SPEED.startY,
				w,
				h,
				256,
				256
		...
		if (promptSize > 1) {
			...
			graphics.blit(
					...
					0,
					0,
					AllGuiTextures.TRAIN_PROMPT.startX + (128 - promptSize / 2f),
					AllGuiTextures.TRAIN_PROMPT.startY,
					promptSize,
					AllGuiTextures.TRAIN_PROMPT.height,
					256,
					256
			);
			poseStack.popPose();
			...
			if (currentPrompt != null && font.width(currentPrompt) < promptSize - 10) {
				...
				if (currentPromptShadow) graphics.drawString(font, currentPrompt, 9, 4, 0x544D45);
				else graphics.drawString(font, currentPrompt, 9, 4, 0x544D45, false);
				poseStack.popPose();
			}
		}
		...
		graphics.blit(
				...
				0,
				0,
				AllGuiTextures.TRAIN_HUD_THROTTLE.startX + invW,
				AllGuiTextures.TRAIN_HUD_THROTTLE.startY,
				w,
				h,
				256,
				256
		...
		AllGuiTextures.TRAIN_HUD_THROTTLE_POINTER.render(
				graphics,
				Math.max(1, AllGuiTextures.TRAIN_HUD_THROTTLE.width - w) - 3,
				-2
		);
		// Direction
