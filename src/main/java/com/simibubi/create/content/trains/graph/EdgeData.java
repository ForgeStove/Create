...
public class EdgeData {
	...
	public UUID getEffectiveEdgeGroupId(TrackGraph graph) {
		return singleSignalGroup == null ? null : singleSignalGroup.equals(passiveGroup) ? graph.id :
				singleSignalGroup;
	}
	...
	public <T extends TrackEdgePoint> void addPoint(TrackGraph graph, TrackEdgePoint point) {
		...
		int i = 0;
		for (; i < points.size(); i++)
			if (points.get(i).getLocationOn(edge) > locationOn) break;
		points.add(i, point);
	}
	@Nullable @SuppressWarnings("unchecked")
	public <T extends TrackEdgePoint> T next(EdgePointType<T> type, double minPosition) {
		for (TrackEdgePoint point : points)
			if (point.getType() == type && point.getLocationOn(edge) > minPosition) return (T) point;
		return null;
	}
	@Nullable public TrackEdgePoint next(double minPosition) {
		for (TrackEdgePoint point : points)
			if (point.getLocationOn(edge) > minPosition) return point;
		return null;
	}
	@Nullable public <T extends TrackEdgePoint> T get(EdgePointType<T> type, double exactPosition) {
	...
	public CompoundTag write(DimensionPalette dimensions) {
		...
		if (hasPoints()) nbt.put(
				"Points", NBTHelper.writeCompoundList(
						points, point -> {
	...
	public static EdgeData read(CompoundTag nbt, TrackEdge edge, TrackGraph graph, DimensionPalette dimensions) {
		...
		if (nbt.contains("Points")) NBTHelper.iterateCompoundList(
				nbt.getList("Points", Tag.TAG_COMPOUND), tag -> {
