...
public class CarriageContraptionInstance extends EntityInstance<CarriageContraptionEntity> implements DynamicInstance {
	...
	private final Couple<Boolean> bogeyHidden;
	...
	@Override public void init() {
		...
		bogeys = carriage.bogeys.mapNotNullWithParam(
				(bogey, manager) -> bogey.getStyle().createInstance(bogey, bogey.type.getSize(), manager),
				materialManager
		);
		updateLight();
	}
	...
	@Override public void beginFrame() {
		if (bogeys == null) {
			if (entity.isReadyForRender()) init();
			return;
		}
		...
		TransformStack.cast(ms).translate(instancePosition);
	...
	@Override public void updateLight() {
		...
		bogeys.forEach(instance -> {
			if (instance != null) instance.updateLight(world, entity);
		});
	}
