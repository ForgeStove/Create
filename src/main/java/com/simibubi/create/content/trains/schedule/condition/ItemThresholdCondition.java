...
public class ItemThresholdCondition extends CargoThresholdCondition {
	...
	@Override protected boolean test(Level level, Train train, CompoundTag context) {
		...
		for (Carriage carriage : train.carriages) {
			...
			for (int i = 0; i < items.getSlots(); i++) {
				...
				if (stacks) foundItems += stackInSlot.getCount() == stackInSlot.getMaxStackSize() ? 1 : 0;
	...
	@Override protected void readAdditional(CompoundTag tag) {
		...
		if (tag.contains("Item")) stack = FilterItemStack.of(tag.getCompound("Item"));
	}
		...
		return ImmutableList.of(
				Lang.translateDirect(
						...
						stack.isEmpty()
								...
								: stack.isFilterItem() ? Lang.translateDirect(
		...
		builder.addSelectionScrollInput(
				71, 50, (i, l) -> {
					i.forOptions(ImmutableList.of(
							Lang.translateDirect("schedule.condition.threshold.items"),
