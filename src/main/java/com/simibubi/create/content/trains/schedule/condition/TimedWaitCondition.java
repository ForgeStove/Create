...
public abstract class TimedWaitCondition extends ScheduleWaitCondition {
	...
	protected void requestDisplayIfNecessary(CompoundTag context, int time) {
		...
		if (ticksUntilDeparture < 20 * 60 && ticksUntilDeparture % 100 == 0) requestStatusToUpdate(context);
		if (ticksUntilDeparture >= 20 * 60 && ticksUntilDeparture % (20 * 60) == 0) requestStatusToUpdate(context);
	}
	...
	protected Component formatTime(boolean compact) {
		if (compact) return Components.literal(getValue() + getUnit().suffix);
	...
	@Override @OnlyIn(Dist.CLIENT) public void initConfigurationWidgets(ModularGuiLineBuilder builder) {
		...
		builder.addSelectionScrollInput(
				36, 85, (i, l) -> {
					i.forOptions(TimeUnit.translatedOptions()).titled(Lang.translateDirect("generic.timeUnit"));
				}, "TimeUnit"
		);
	}
	@Override public MutableComponent getWaitingStatus(Level level, Train train, CompoundTag tag) {
		...
		int num = (int) (
				showInMinutes ? Math.floor(ticksUntilDeparture / (20 * 60f)) :
