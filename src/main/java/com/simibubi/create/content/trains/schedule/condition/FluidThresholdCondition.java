...
public class FluidThresholdCondition extends CargoThresholdCondition {
	...
	@Override protected Component getUnit() {
		return Components.literal("b");
	}
	@Override protected ItemStack getIcon() {
		return compareStack.item();
	}
	@Override protected boolean test(Level level, Train train, CompoundTag context) {
	...
	@Override protected void readAdditional(CompoundTag tag) {
		...
		if (tag.contains("Bucket")) compareStack = FilterItemStack.of(tag.getCompound("Bucket"));
	}
	...
	@OnlyIn(Dist.CLIENT) private FluidStack loadFluid() {
		...
		return ImmutableList.of(
				Lang.translateDirect(
						...
						compareStack.isEmpty()
								? Lang.translateDirect("schedule.condition.threshold.anything")
								: compareStack.isFilterItem() ? Lang.translateDirect(
										"schedule.condition.threshold.matching_content") : loadFluid().getDisplayName()
	...
	@Override public MutableComponent getWaitingStatus(Level level, Train train, CompoundTag tag) {
		...
		if (lastDisplaySnapshot == -1) return Components.empty();
