...
@EventBusSubscriber(Dist.CLIENT) public class TrackBlockItem extends BlockItem {
	...
	@Override public InteractionResult useOn(UseOnContext pContext) {
		...
		if (!isFoil(stack)) {
			if (state.getBlock() instanceof TrackBlock track && track.getTrackAxes(level, pos, state).size() > 1) {
				if (!level.isClientSide) player.displayClientMessage(
						Lang.translateDirect("track.junction_start")
			...
			if (level.getBlockEntity(pos) instanceof TrackBlockEntity tbe && tbe.isTilted()) {
				if (!level.isClientSide)
					player.displayClientMessage(
							Lang.translateDirect("track.turn_start").withStyle(ChatFormatting.RED),
							true
					);
				return InteractionResult.SUCCESS;
			}
			...
			if (!level.isClientSide) {
				...
				stack.setTag(null);
			} else level.playSound(player, pos, SoundEvents.ITEM_FRAME_REMOVE_ITEM, SoundSource.BLOCKS, 0.75f, 1);
			return InteractionResult.SUCCESS;
		}
		...
		if (soundtype != null) level.playSound(
				null,
				pos,
	...
	public static void sendExtenderPacket(PlayerInteractEvent.RightClickBlock event) {
		...
		if (!AllTags.AllBlockTags.TRACKS.matches(stack) || !stack.hasTag()) return;
		if (Minecraft.getInstance().options.keySprint.isDown()) AllPackets.getChannel()
