...
public class FlapDisplayRenderer extends KineticBlockEntityRenderer<FlapDisplayBlockEntity> {
	...
	@OnlyIn(Dist.CLIENT) static class FlapDisplayRenderOutput implements FormattedCharSink {
		...
		public boolean accept(int charIndex, Style style, int glyph) {
			...
			if (section.renderCharsIndividually()) x += standardWidth - (standardWidth - glyphWidth) / 2f;
			else x += glyphWidth;
			return true;
		}
	...
	@Override protected SuperByteBuffer getRotatedModel(FlapDisplayBlockEntity be, BlockState state) {
	...
	@Override public boolean shouldRenderOffScreen(FlapDisplayBlockEntity be) {
