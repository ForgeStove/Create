...
public class CurvedTrackDestroyPacket extends BlockEntityConfigurationPacket<TrackBlockEntity> {
	...
	@Override protected void applySettings(ServerPlayer player, TrackBlockEntity be) {
		...
		if (level.getBlockEntity(targetPos) instanceof TrackBlockEntity other) other.removeConnection(pos);
		...
		if (wrench) {
			...
			if (!player.isCreative() && bezierConnection != null) bezierConnection.addItemsToPlayer(player);
		} else if (!player.isCreative() && bezierConnection != null) bezierConnection.spawnItems(level);
	...
	@Override protected void applySettings(TrackBlockEntity be) {
	}
}
