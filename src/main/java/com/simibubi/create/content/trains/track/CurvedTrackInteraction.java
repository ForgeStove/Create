...
public class CurvedTrackInteraction {
		...
		if (mc.options.keyAttack.isDown() && result != null) {
			...
			if (breakProgress >= 1) {
				AllPackets.getChannel()
						.sendToServer(new CurvedTrackDestroyPacket(
								breakPos,
	...
	private static void resetBreakProgress() {
		Minecraft mc = Minecraft.getInstance();
		ClientLevel level = mc.level;
		if (breakPos != null && level != null) level.destroyBlockProgress(mc.player.getId(), breakPos, -1);
		breakProgress = 0;
		breakTicks = 0;
		breakPos = null;
	}
	public static boolean onClickInput(InputEvent.InteractionKeyMappingTriggered event) {
		...
		if (event.isUseItem()) {
			...
			if (AllItems.WRENCH.isIn(heldItem) && player.isShiftKeyDown()) {
				AllPackets.getChannel().sendToServer(new CurvedTrackDestroyPacket(
