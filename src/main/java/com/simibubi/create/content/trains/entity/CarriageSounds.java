...
public class CarriageSounds {
	...
	public CarriageSounds(CarriageContraptionEntity entity) {
		this.entity = entity;
		bogeySounds = entity.getCarriage().bogeys.map(bogey -> bogey != null && bogey.getStyle() != null
				? bogey.getStyle().getSoundType()
	...
	public void tick(DimensionalCarriageEntity dce) {
		...
		if (entity.carriageIndex == 0) {
			...
			if ((3 + tick) % 4 == 0) AllSoundEvents.STEAM.playAt(
					entity.level(),
					soundLocation,
			...
			if (tick % 16 == 0) AllSoundEvents.STEAM.playAt(entity.level(), soundLocation, v * 1.5f, .8f, false);
		}
