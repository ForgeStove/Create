...
public class TrainPacket extends SimplePacketBase {
	...
	public TrainPacket(FriendlyByteBuf buffer) {
		...
		if (!add) return;
		UUID owner = null;
		if (buffer.readBoolean()) owner = buffer.readUUID();
		...
		for (int i = 0; i < size; i++) {
			...
			for (boolean isFirst : Iterate.trueAndFalse) {
				...
				AbstractBogeyBlock<?>
						type
						= (AbstractBogeyBlock<?>) ForgeRegistries.BLOCKS.getValue(buffer.readResourceLocation());
				boolean upsideDown = buffer.readBoolean();
				CompoundTag data = buffer.readNbt();
				bogies.set(
						isFirst,
						new CarriageBogey(type, upsideDown, data, new TravellingPoint(), new TravellingPoint())
				);
			}
	...
	@Override public void write(FriendlyByteBuf buffer) {
		...
		if (train.owner != null) buffer.writeUUID(train.owner);
	...
	@Override public boolean handle(Context context) {
		context.enqueueWork(() -> {
			...
			if (add) trains.put(train.id, train);
			else trains.remove(trainId);
		});
		return true;
	}
}
