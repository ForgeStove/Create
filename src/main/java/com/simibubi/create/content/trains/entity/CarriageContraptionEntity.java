...
public class CarriageContraptionEntity extends OrientedContraptionEntity {
	private static final EntityDataAccessor<CarriageSyncData> CARRIAGE_DATA = SynchedEntityData.defineId(CarriageContraptionEntity.class,
			AllEntityDataSerializers.CARRIAGE_DATA
	);
	...
	@OnlyIn(Dist.CLIENT) public CarriageSounds sounds;
	@OnlyIn(Dist.CLIENT) public CarriageParticles particles;
	...
	@Override public boolean isControlledByLocalInstance() {
		return true;
	}
	...
	@Override public Vec3 getPrevPositionVec() {
	...
	public static CarriageContraptionEntity create(Level world, CarriageContraption contraption) {
		CarriageContraptionEntity entity = new CarriageContraptionEntity(
	...
	@Override public void tick() {
		super.tick();
		if (contraption instanceof CarriageContraption cc) for (Entity entity : getPassengers()) {
	...
	@Override public void setBlock(BlockPos localPos, StructureBlockInfo newInfo) {
		if (carriage == null) return;
		carriage.forEachPresentEntity(cce -> {
			cce.contraption.getBlocks().put(localPos, newInfo);
			AllPackets.getChannel().send(
					PacketDistributor.TRACKING_ENTITY.with(() -> cce),
	...
	private void tickArrivalSound(CarriageContraption cc) {
		...
		if (arrivalSoundTicks == Integer.MIN_VALUE) {
			...
			for (int index = 0; index < carriageCount; index++) {
				...
				if (entity == null || !(entity.contraption instanceof CarriageContraption otherCC)) break;
		...
		for (Carriage c : carriages) {
			...
			if (entity == null || !(entity.contraption instanceof CarriageContraption otherCC)) continue;
	...
	@Override public void tickActors() {
		super.tickActors();
	}
	@Override protected boolean isActorActive(MovementContext context, MovementBehaviour actor) {
	...
	@Override protected void handleStallInformation(double x, double y, double z, float angle) {
	}
	...
	@Override protected void addPassenger(Entity pPassenger) {
	...
	@Override public void onClientRemoval() {
	...
	@Override protected void writeAdditional(CompoundTag compound, boolean spawnPacket) {
		super.writeAdditional(compound, spawnPacket);
		compound.putUUID("TrainId", trainId);
		compound.putInt("CarriageIndex", carriageIndex);
	}
	@Override protected void readAdditional(CompoundTag compound, boolean spawnPacket) {
	...
	@Override public Component getContraptionName() {
	...
	@Override public boolean startControlling(BlockPos controlsLocalPos, Player player) {
		if (player == null || player.isSpectator()) return false;
		if (carriage == null) return false;
		if (carriage.train.derailed) return false;
		Train train = carriage.train;
		if (train.runtime.getSchedule() != null && !train.runtime.paused) train.status.manualControls();
	...
	@Override public Component getDisplayName() {
		if (carriage == null) return Lang.translateDirect("train");
		return carriage.train.name;
	}
	double navDistanceTotal = 0;
	int hudPacketCooldown = 0;
	@Override public boolean control(BlockPos controlsLocalPos, Collection<Integer> heldControls, Player player) {
		if (carriage == null) return false;
		if (carriage.train.derailed) return false;
		if (level().isClientSide) return true;
		if (player.isSpectator()) return false;
		if (!toGlobalVector(VecHelper.getCenterOf(controlsLocalPos), 1).closerThan(player.position(), 8)) return false;
		...
		if (hudPacketCooldown-- <= 0 && player instanceof ServerPlayer sp) {
			AllPackets.getChannel()
					.send(PacketDistributor.PLAYER.with(() -> sp), new TrainHUDUpdatePacket(carriage.train));
			hudPacketCooldown = 5;
		}
		int targetSpeed = 0;
		...
		if (currentStation != null && spaceDown) {
			sendPrompt(
					player, Lang.translateDirect(
							"train.arrived_at",
							Components.literal(currentStation.name).withStyle(s -> s.withColor(0x704630))
					), false
			);
			return true;
		}
	...
	private void sendPrompt(Player player, MutableComponent component, boolean shadow) {
		if (player instanceof ServerPlayer sp) AllPackets.getChannel()
				.send(PacketDistributor.PLAYER.with(() -> sp), new TrainPromptPacket(component, shadow));
	}
	...
	public void setCarriage(Carriage carriage) {
		...
		if (carriage.train.graph != null) entityData.set(TRACK_GRAPH, Optional.of(carriage.train.graph.id));
		DimensionalCarriageEntity dimensional = carriage.getDimensional(level());
		dimensional.pivot = null;
		carriage.updateContraptionAnchors();
		dimensional.updateRenderedCutoff();
	}
	@OnlyIn(Dist.CLIENT) private WeakReference<CarriageContraptionInstance> instanceHolder;
	@OnlyIn(Dist.CLIENT) public void bindInstance(CarriageContraptionInstance instance) {
