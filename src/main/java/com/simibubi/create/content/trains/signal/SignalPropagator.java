...
public class SignalPropagator {
	public static void onSignalRemoved(TrackGraph graph, SignalBoundary signal) {
		...
		for (boolean front : Iterate.trueAndFalse) {
			...
			if (Create.RAILWAYS.signalEdgeGroups.remove(id) != null) Create.RAILWAYS.sync.edgeGroupRemoved(id);
			walkSignals(
					graph, signal, front, pair -> {
		...
		walkSignals(
				graph, signal, front, pair -> {
					...
					if (currentGroup != null)
						if (globalGroups.remove(currentGroup) != null) sync.edgeGroupRemoved(currentGroup);
					...
					if (singleSignalGroup != null)
						if (globalGroups.remove(singleSignalGroup) != null) sync.edgeGroupRemoved(singleSignalGroup);
	...
	private static void walkSignals(
		...
		while (!frontier.isEmpty()) {
			...
			for (Entry<TrackNode, TrackEdge> entry : graph.getConnectionsFrom(currentNode).entrySet()) {
				...
				if (forCollection && !graph.getConnectionsFrom(prevNode).get(currentNode).canTravelTo(edge)) continue;
