...
public class ScheduleRuntime {
	...
	public DiscoveredPath startCurrentInstruction() {
		...
		if (instruction instanceof DestinationInstruction destination) {
			...
			if (best == null) {
				...
				else train.status.failedNavigationNoTarget(destination.getFilter());
				cooldown = INTERVAL;
				return null;
			}
			return best;
					...
					if (predictedTime > 0) {
						predictedTime *= Mth.clamp(
								train.navigation.distanceToDestination
										/ train.navigation.distanceStartedAt, 0, 1
						);
	...
	private TrainDeparturePrediction createPrediction(int index, String destination, String currentTitle, int time) {
		...
		if (text.isBlank()) {
			for (int i = 1; i < size; i++) {
				...
				if (!(scheduleEntry.instruction instanceof DestinationInstruction instruction)) continue;
	...
	public CompoundTag write() {
		...
		if (schedule != null) tag.put("Schedule", schedule.write());
	...
	public void read(CompoundTag tag) {
		...
		if (tag.contains("Schedule")) schedule = Schedule.fromTag(tag.getCompound("Schedule"));
		...
		if (schedule != null) {
			schedule.entries.forEach($ -> predictionTicks.add(TBD));
			if (readTransits.length == schedule.entries.size()) for (int i = 0; i < readTransits.length; i++)
	...
	public MutableComponent getWaitingStatus(Level level) {
		...
		if (progress >= list.size()) return Components.empty();
