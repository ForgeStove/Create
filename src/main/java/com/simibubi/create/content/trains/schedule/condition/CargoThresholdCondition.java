...
public abstract class CargoThresholdCondition extends LazyTickedScheduleCondition {
	...
	@Override public boolean lazyTickCompletion(Level level, Train train, CompoundTag context) {
	...
	@Override public Pair<ItemStack, Component> getSummary() {
		return Pair.of(getIcon(),
				Components.literal(getOperator().formatted + " " + getThreshold()).append(getUnit()));
	}
	...
	@Override public List<Component> getSecondLineTooltip(int slot) {
		return ImmutableList.of(
				Lang.translateDirect("schedule.condition.threshold.place_item"),
				Lang.translateDirect("schedule.condition.threshold.place_item_2").withStyle(ChatFormatting.GRAY),
				Lang.translateDirect("schedule.condition.threshold.place_item_3").withStyle(ChatFormatting.GRAY)
		);
	}
