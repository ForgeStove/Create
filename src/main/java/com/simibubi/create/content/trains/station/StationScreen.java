...
public class StationScreen extends AbstractStationScreen {
	...
	@Override protected void init() {
		...
		tickTrainDisplay();
		Pair<ScrollInput, Label> doorControlWidgets = DoorControl.createWidget(
				x + 35,
				y + 102,
	...
	@Override public void tick() {
		...
		super.tick();
		updateAssemblyTooltip(blockEntity.edgePoint.isOnCurve()
				? "no_assembly_curve"
	...
	private void tickTrainDisplay() {
		...
		if (blockEntity.trainHasSchedule)
			dropScheduleButton.setToolTip(Lang.translateDirect(blockEntity.trainHasAutoSchedule
	...
	private void syncTrainName() {
		Train train = displayedTrain.get();
		if (train != null && !trainNameBox.getValue().equals(train.name.getString())) AllPackets.getChannel()
				.sendToServer(new TrainEditPacket(train.id, trainNameBox.getValue(), train.icon.getId()));
	}
	private void syncStationName() {
		if (!nameBox.getValue().equals(station.name)) AllPackets.getChannel()
				.sendToServer(StationEditPacket.configure(
	...
	@Override public void removed() {
		...
		if (!switchingToAssemblyMode) AllPackets.getChannel()
				.sendToServer(new TrainEditPacket(train.id, trainNameBox.getValue(), train.icon.getId()));
		else blockEntity.imminentTrain = null;
	}
	@Override protected PartialModel getFlag(float partialTicks) {
