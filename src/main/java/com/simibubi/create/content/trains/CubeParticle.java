...
public class CubeParticle extends Particle {
	...
	private static final ParticleRenderType RENDER_TYPE = new ParticleRenderType() {
		@Override public void begin(BufferBuilder builder, TextureManager textureManager) {
		...
		@Override public void end(Tesselator tessellator) {
			tessellator.end();
		...
		if (this.hot && this.age > 0) {
			if (this.yo == this.y) {
				...
				if (this.xd == 0 && this.zd == 0) {
					Vec3 diff = Vec3.atLowerCornerOf(BlockPos.containing(x, y, z)).add(0.5, 0.5, 0.5).subtract(x, y,
							z);
					this.xd = -diff.x * 0.1;
					this.zd = -diff.z * 0.1;
				}
	...
	@Override public void render(VertexConsumer builder, Camera renderInfo, float p_225606_3_) {
		...
		for (int i = 0; i < 6; i++) {
			...
			for (int j = 0; j < 4; j++) {
				...
				vec = vec
						/* .rotate(?) */.scale(scale * ageMultiplier).add(lerpedX, lerpedY, lerpedZ);
