...
public class TrackGraphSyncPacket extends TrackGraphPacket {
	...
	@Override protected void handle(GlobalRailwayManager manager, TrackGraph graph) {
		...
		for (Pair<Pair<Couple<Integer>, TrackMaterial>, BezierConnection> pair : addedEdges) {
			...
			if (node1 != null && node2 != null) graph.putConnection(
					node1,
					node2,
		...
		if (!splitSubGraphs.isEmpty()) graph.findDisconnectedGraphs(null, splitSubGraphs).forEach(manager::putGraph);
	}
	protected void handleEdgeData(GlobalRailwayManager manager, TrackGraph graph) {
		for (Entry<Couple<Integer>, Pair<Integer, List<UUID>>> entry : updatedEdgeData.entrySet()) {
			...
			else if (groupType == PASSIVE_GROUP) edgeData.setSingleSignalGroup(null, EdgeData.passiveGroup);
