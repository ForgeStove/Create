...
public class SignalBoundary extends TrackEdgePoint {
	...
	@Override public void invalidate(LevelAccessor level) {
		blockEntities.forEach(s -> s.keySet().forEach(p -> invalidateAt(level, p)));
		groups.forEach(uuid -> {
			if (Create.RAILWAYS.signalEdgeGroups.remove(uuid) != null) Create.RAILWAYS.sync.edgeGroupRemoved(uuid);
		});
	}
	@Override public boolean canCoexistWith(EdgePointType<?> otherType, boolean front) {
	...
	@Override public void blockEntityRemoved(BlockPos blockEntityPos, boolean front) {
		blockEntities.forEach(s -> s.remove(blockEntityPos));
		if (blockEntities.both(Map::isEmpty)) removeFromAllGraphs();
	}
	@Override public void onRemoved(TrackGraph graph) {
	...
	@Override public boolean canNavigateVia(TrackNode side) {
		...
		for (int i = 1; i <= 2; i++)
			if (nbt.contains("Tiles" + i)) {
				...
				NBTHelper.iterateCompoundList(
						nbt.getList("Tiles" + i, Tag.TAG_COMPOUND),
	...
	@Override public void write(CompoundTag nbt, DimensionPalette dimensions) {
		...
		for (int i = 1; i <= 2; i++)
			if (!blockEntities.get(i == 1).isEmpty()) nbt.put(
					"Tiles" + i, NBTHelper.writeCompoundList(
							blockEntities.get(i == 1).entrySet(), e -> {
