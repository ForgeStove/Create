...
public class TrackTargetingBehaviour<T extends TrackEdgePoint> extends BlockEntityBehaviour {
	...
	@Override public void write(CompoundTag nbt, boolean clientPacket) {
		...
		if (targetBezier != null) {
			...
			bezierNbt.put("Key", NbtUtils.writeBlockPos(targetBezier.curveTarget().subtract(getPos())));
	...
	@Override public void read(CompoundTag nbt, boolean clientPacket) {
		...
		if (nbt.contains("PrevAxis")) prevDirection = VecHelper.readNBT(nbt.getList("PrevAxis", Tag.TAG_DOUBLE));
		...
		if (nbt.contains("Bezier")) {
			...
			targetBezier = new BezierTrackPointLocation(
					bezierNbt.contains("FromStack") ? key : key.offset(getPos()),
	...
	@SuppressWarnings("unchecked") public T createEdgePoint() {
		...
		point.setLocation(reverseEdge ? loc.edge : loc.edge.swap(), reverseEdge ? loc.position :
				length - loc.position);
	...
	@OnlyIn(Dist.CLIENT) public static void render(
			LevelAccessor level,
			BlockPos pos,
		...
		if (level instanceof SchematicWorld && !(level instanceof PonderWorld)) return;
		...
		if (partial != null) CachedBufferer.partial(partial, trackState)
				.translate(.5, 0, .5)
				.scale(scale)
				.translate(-.5, 0, -.5)
	...
	public void transform(StructureTransform transform) {
		...
		if (prevDirection != null) rotatedDirection = transform.applyWithoutOffsetUncentered(prevDirection);
		if (targetBezier != null) targetBezier = new BezierTrackPointLocation(
