...
public class Navigation {
	...
	public void tick(Level level) {
		...
		if (train.graph != null) {
			...
			if (distanceToSignal > 1 / 16f) {
				...
				double scanDistance = Mth.clamp(brakingDistanceNoFlicker, preDepartureLookAhead,
				...
				signalScout.travel(
						...
						(distance, couple) -> {
							...
							if (boundary == destination
									&& ((GlobalStation) boundary).canApproachFrom(nodes.getSecond())) return true;
						...
						(distance, edge) -> {
							...
							if (current == -1 || distance < current) curveDistanceTracker.setValue(distance);
						}
				);
				if (trackingCrossSignal.getValue() != null && waitingForSignal == null) reserveChain();
	...
	@Nullable public DiscoveredPath findPathTo(ArrayList<GlobalStation> destinations, double maxCost) {
		...
		if (frontEmpty || !canDriveForward) return back;
		...
		if (initialSignalData.hasPoints()) {
			for (TrackEdgePoint point : initialSignalData.getPoints()) {
				...
				if (point instanceof GlobalStation station) {
					...
					if (presentTrain != null && !isOwnStation) initialPenalty += Train.Penalties.STATION_WITH_TRAIN;
			...
			for (Entry<TrackNode, TrackEdge> target : validTargets) {
				...
				if (signalData.hasPoints()) {
					for (TrackEdgePoint point : signalData.getPoints()) {
						...
						if (point instanceof GlobalStation station) {
							Train presentTrain = station.getPresentTrain();
							boolean isOwnStation = presentTrain == train;
							if (presentTrain != null && !isOwnStation) newPenalty += Train.Penalties.STATION_WITH_TRAIN;
				...
				if (destinations != null && !destinations.isEmpty()) {
					...
					for (GlobalStation destination : destinations) {
						...
						if (node2.getLocation().equals(destinationNode)) currentRemaining -= newEdge.getLength()
	...
	private static class FrontierEntry implements Comparable<FrontierEntry> {
		...
		@Override public int compareTo(FrontierEntry o) {
			return Double.compare(distance + penalty + remaining, o.distance + o.penalty + o.remaining);
		}
	}
	@FunctionalInterface public interface StationTest {
		boolean test(
				double distance,
				double cost,
	...
	public CompoundTag write(DimensionPalette dimensions) {
		...
		tag.put(
				"Path", NBTHelper.writeCompoundList(
						currentPath, c -> {
	...
	public void read(CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
		destination = graph != null && tag.contains("Destination") ? graph.getPoint(
		...
		removeBrokenPathEntries();
		waitingForSignal = tag.contains("BlockingSignal") ? Pair.of(
				tag.getUUID("BlockingSignal"),
				tag.getBoolean("BlockingSignalSide")
		) : null;
	...
	private void removeBrokenPathEntries() {
		...
		if (nullEntriesPresent) Create.LOGGER.error(
				"Found null values in path of train with name: {}, id: {}",
