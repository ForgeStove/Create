...
@EventBusSubscriber(Dist.CLIENT) public class TrackBlockOutline {
	public static WorldAttached<Map<BlockPos, TrackBlockEntity>>
			TRACKS_WITH_TURNS
		...
		for (TrackBlockEntity be : turns.values()) {
			for (BezierConnection bc : be.connections.values()) {
				...
				for (int i = 0; i < stepLUT.length - 2; i++) {
					...
					if (bestSegment != -1 && bestDistance < clip.get().distanceToSqr(0, 0.25f, 0)) continue;
	...
	@SubscribeEvent public static void drawCustomBlockSelection(RenderHighlightEvent.Block event) {
		Minecraft mc = Minecraft.getInstance();
		...
		walkShapes(
				shape, TransformStack.cast(ms), s -> {
