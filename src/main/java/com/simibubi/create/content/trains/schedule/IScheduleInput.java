...
public interface IScheduleInput {
	...
	@Nullable default List<Component> getSecondLineTooltip(int slot) {
		return null;
	}
	@OnlyIn(Dist.CLIENT) default void initConfigurationWidgets(ModularGuiLineBuilder builder) {
	}
	@OnlyIn(Dist.CLIENT) default boolean renderSpecialIcon(GuiGraphics graphics, int x, int y) {
		return false;
	}
}
