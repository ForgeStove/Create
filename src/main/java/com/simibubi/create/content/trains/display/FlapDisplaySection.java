...
public class FlapDisplaySection {
	...
	public void refresh(boolean transition) {
		...
		if (!singleFlap) {
			...
			if (!text.isEmpty()) for (int i = 0; i < spinning.length; i++)
	...
	public int tick(boolean instant) {
		...
		spinningTicks++;
		if (spinningTicks <= max && spinningTicks < 2) return spinningTicks == 1 ? 0 : spinning.length;
		int spinningFlaps = 0;
	...
	public CompoundTag write() {
		...
		if (component != null) tag.putString("Text", Component.Serializer.toJson(component));
	...
	public static String[] getFlapCycle(String key) {
		return LOADED_FLAP_CYCLES.computeIfAbsent(
				key,
				k -> Lang.translateDirect("flap_display.cycles." + key).getString().split(";")
		);
	}
}
