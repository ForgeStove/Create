...
/**
 * This is a port of the bogey api from Extended Bogeys, If you are looking to implement your own bogeys you can find
...
public abstract class BogeyRenderer {
	...
	public BogeyModelData[] getTransform(PartialModel model, PoseStack ms, boolean inInstancedContraption, int size) {
		return (inInstancedContraption)
				? transformContraptionModelData(keyFromModel(model), ms)
	 ...
	 */
	@OnlyIn(Dist.CLIENT) public abstract void render(
	 ...
	 * @param ms         The posestack to render to
	 */
	@OnlyIn(Dist.CLIENT) public void render(CompoundTag bogeyData, float wheelAngle, PoseStack ms) {
	 ...
	 */
	@OnlyIn(Dist.CLIENT) public abstract void initialiseContraptionModelData(
			MaterialManager materialManager,
			CarriageBogey carriageBogey
	);
	/**
	...
	public void createModelInstance(MaterialManager materialManager, PartialModel model, int count) {
		BogeyModelData[] modelData = IntStream.range(0, count)
				.mapToObj(i -> materialManager.defaultSolid()
						.material(Materials.TRANSFORMED).getModel(model).createInstance())
	...
	public void createModelInstance(MaterialManager materialManager, BlockState state, int count) {
		BogeyModelData[] modelData = IntStream.range(0, count)
				.mapToObj(i -> materialManager.defaultSolid()
						.material(Materials.TRANSFORMED).getModel(state).createInstance())
				.map(BogeyModelData::new)
				.toArray(BogeyModelData[]::new);
	 ...
	 */
	@Deprecated public static <B extends Transform<?>> void finalize(
			B b,
			PoseStack ms,
			int light,
		...
		b.scale(1 - 1 / 512f);
		if (b instanceof SuperByteBuffer byteBuf && vb != null) byteBuf.light(light).renderInto(ms, vb);
	}
	/**
	...
	public static abstract class CommonRenderer extends BogeyRenderer {
		@Override public BogeySizes.BogeySize getSize() {
			return null;
		}
	}
	public record BogeyModelData(Transform<?> transform) implements Transform<BogeyModelData> {
		...
		public void render(PoseStack ms, int light, @Nullable VertexConsumer vb) {
			...
			if (transform instanceof SuperByteBuffer byteBuf && vb != null) byteBuf.light(light).renderInto(ms, vb);
		}
		public BogeyModelData setTransform(PoseStack ms) {
			if (this.transform instanceof ModelData model) model.setTransform(ms);
			return this;
		}
		public BogeyModelData setEmptyTransform() {
			if (this.transform instanceof ModelData model) model.setEmptyTransform();
			return this;
		}
		public BogeyModelData delete() {
			if (this.transform instanceof ModelData model) model.delete();
			return this;
		}
		public BogeyModelData updateLight(int blockLight, int skyLight) {
			if (this.transform instanceof ModelData model) model.setBlockLight(blockLight).setSkyLight(skyLight);
			return this;
		}
		@Override public BogeyModelData mulPose(Matrix4f pose) {
			this.transform.mulPose(pose);
			return this;
		}
		@Override public BogeyModelData mulNormal(Matrix3f normal) {
			this.transform.mulNormal(normal);
			return this;
		}
		@Override public BogeyModelData multiply(Quaternionf quaternion) {
			this.transform.multiply(quaternion);
			return this;
		}
		@Override public BogeyModelData scale(float factorX, float factorY, float factorZ) {
			this.transform.scale(factorX, factorY, factorZ);
			return this;
		}
		@Override public BogeyModelData translate(double x, double y, double z) {
