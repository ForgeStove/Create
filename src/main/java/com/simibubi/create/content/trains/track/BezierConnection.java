...
public class BezierConnection implements Iterable<BezierConnection.Segment> {
	...
	private static boolean coupleEquals(Couple<?> a, Couple<?> b) {
		return (
				a.getFirst().equals(b.getFirst()) && a.getSecond().equals(b.getSecond())
		) || (
	...
	private boolean equalsSansMaterialInner(BezierConnection other) {
		return this == other || (
				other != null
						&& coupleEquals(this.tePositions, other.tePositions)
						&& coupleEquals(
						this.starts,
						other.starts
				)
	...
	public BezierConnection(CompoundTag compound, BlockPos localTo) {
		this(
				Couple.deserializeEach(compound.getList("Positions", Tag.TAG_COMPOUND), NbtUtils::readBlockPos)
		...
		if (compound.contains("Smoothing"))
			smoothing = Couple.deserializeEach(
					compound.getList("Smoothing", Tag.TAG_COMPOUND),
					NBTHelper::intFromCompound
			);
	}
	public CompoundTag write(BlockPos localTo) {
		...
		if (smoothing != null) compound.put("Smoothing", smoothing.serializeEach(NBTHelper::intToCompound));
		return compound;
	}
	...
	public int yOffsetAt(Vec3 end) {
		...
		if (TrackBlockEntityTilt.compareHandles(starts.getFirst(), end)) return smoothing.getFirst();
		if (TrackBlockEntityTilt.compareHandles(starts.getSecond(), end)) return smoothing.getSecond();
		return 0;
	}
	// Runtime information
	...
	public Vec3 getNormal(double t) {
		...
		Vec3 derivative = VecHelper.bezierDerivative(end1, end2, finish1, finish2, (float) t).normalize();
	...
	@Override public Iterator<Segment> iterator() {
		resolve();
		var offset = Vec3.atLowerCornerOf(tePositions.getFirst()).scale(-1).add(0, 3 / 16f, 0);
	...
	public void spawnDestroyParticles(Level level) {
		...
		BlockParticleOption girderData = new BlockParticleOption(
				ParticleTypes.BLOCK,
				AllBlocks.METAL_GIRDER.getDefaultState()
		);
	...
	private static class Bezierator implements Iterator<Segment> {
		...
		private Bezierator(BezierConnection bc, Vec3 offset) {
			...
			finish1 = bc.axes.getFirst().scale(bc.handleLength).add(end1);
			finish2 = bc.axes.getSecond().scale(bc.handleLength).add(end2);
	...
	@OnlyIn(Dist.CLIENT) public static class SegmentAngles {
		public Pose tieTransform;
		public Couple<Pose> railTransforms;
		public BlockPos lightPosition;
	}
	@OnlyIn(Dist.CLIENT) public static class GirderAngles {
		public Couple<Pose> beams;
		public Couple<Couple<Pose>> beamCaps;
		public BlockPos lightPosition;
	}
	@OnlyIn(Dist.CLIENT) public SegmentAngles[] getBakedSegments() {
		if (bakedSegments != null) return bakedSegments;
	...
	@OnlyIn(Dist.CLIENT) public GirderAngles[] getBakedGirders() {
		if (bakedGirders != null) return bakedGirders;
		int segmentCount = getSegmentCount();
		...
		for (BezierConnection.Segment segment : this) {
			...
			angles.lightPosition = BlockPos.containing(leftGirder.add(rightGirder).scale(.5));
