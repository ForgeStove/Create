...
public interface ITrackBlock {
	...
	default Collection<DiscoveredLocation> getConnected(
		...
		trackAxes.forEach(axis -> {
			...
			Function<Boolean, ResourceKey<Level>> dimensionFactory = b -> world instanceof Level l
					? l.dimension()
					: Level.OVERWORLD;
	...
	static void addToListIfConnected(
		...
		DiscoveredLocation secondLocation =
				new DiscoveredLocation(dimensionFactory.apply(false), secondOffset).viaTurn(
						viaTurn)
	...
	@OnlyIn(Dist.CLIENT) PartialModel prepareTrackOverlay(
			BlockGetter world,
			BlockPos pos,
			BlockState state,
	...
	@OnlyIn(Dist.CLIENT) PartialModel prepareAssemblyOverlay(
			BlockGetter world,
			BlockPos pos,
			BlockState state,
			Direction direction,
