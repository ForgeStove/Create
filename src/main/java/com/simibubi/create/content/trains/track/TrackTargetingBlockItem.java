...
public class TrackTargetingBlockItem extends BlockItem {
	...
	@Override public InteractionResult useOn(UseOnContext pContext) {
		...
		if (state.getBlock() instanceof ITrackBlock track) {
			...
			boolean front = track.getNearestTrackAxis(level, pos, state, lookAngle).getSecond()
			...
			if (result.getValue().feedback != null) {
				player.displayClientMessage(
						Lang.translateDirect(result.getValue().feedback)
		...
		if (!stack.hasTag()) {
			player.displayClientMessage(
					Lang.translateDirect("track_target.missing").withStyle(ChatFormatting.RED),
					true
			);
			return InteractionResult.FAIL;
		}
		...
		if (!selectedPos.closerThan(placedPos, bezier ? 64 + 16 : 16)) {
			player.displayClientMessage(
					Lang.translateDirect("track_target.too_far").withStyle(ChatFormatting.RED),
					true
			);
			return InteractionResult.FAIL;
		}
		if (bezier) teTag.put("Bezier", tag.getCompound("Bezier"));
		...
		player.displayClientMessage(Lang.translateDirect("track_target.success").withStyle(ChatFormatting.GREEN),
				true);
	...
	@OnlyIn(Dist.CLIENT) public boolean useOnCurve(BezierPointSelection selection, ItemStack stack) {
		...
		AllPackets.getChannel()
				.sendToServer(new CurvedTrackSelectionPacket(
						be.getBlockPos(),
						loc.curveTarget(),
						loc.segment(),
						front,
