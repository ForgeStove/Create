...
public class TravellingPoint {
	...
	public interface ITrackSelector
			extends BiFunction<TrackGraph, Pair<Boolean, List<Entry<TrackNode, TrackEdge>>>, Entry<TrackNode,
			TrackEdge>> {
	}
	...
	public double travel(
		...
		if (forward) {
			...
			while (position > edgeLength) {
				...
				blockedLocation = edgeTraversedFrom(graph, true, signalListener, turnListener, 0, collectedDistance);
				if (blockedLocation != null) {
					traveled -= position;
					position = blockedLocation;
					traveled += position;
	...
	protected Double edgeTraversedFrom(
		...
		for (int i = 0; i < edgePoints.size(); i++) {
			...
			if (edgePointListener.test(totalDistance + distance, Pair.of(nextBoundary, forward ? nodes :
					nodes.swap())))
				return locationOn;
		}
		return null;
	...
	public static TravellingPoint read(CompoundTag tag, TrackGraph graph, DimensionPalette dimensions) {
		...
		Couple<TrackNode> locs = tag.contains("Nodes") ? Couple.deserializeEach(
