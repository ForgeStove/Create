...
public class BeltInstance extends KineticBlockEntityInstance<BeltBlockEntity> {
	...
	private Instancer<RotatingData> getPulleyModel() {
		...
		Supplier<PoseStack> ms = () -> {
			...
			msr.centre();
			if (axis == Direction.Axis.X) msr.rotateY(90);
			if (axis == Direction.Axis.Y) msr.rotateX(90);
			msr.rotateX(90);
			msr.unCentre();
	...
	private BeltData setup(BeltData key, boolean bottom, SpriteShiftEntry spriteShift) {
		...
		float rotX = (!diagonal && beltSlope != BeltSlope.HORIZONTAL ? 90 : 0) + (downward ? 180 : 0) + (
				sideways ? 90 : 0
		) + (vertical && alongZ ? 180 : 0);
		float rotY =
				facing.toYRot() + ((diagonal ^ alongX) && !downward ? 180 : 0) + (sideways && alongZ ? 180 : 0) + (
