...
public class BeltConnectorHandler {
	...
	public static void tick() {
		...
		for (InteractionHand hand : InteractionHand.values()) {
			...
			if (!(rayTrace instanceof BlockHitResult)) {
				if (r.nextInt(50) == 0) {
					world.addParticle(
							...
							first.getX() + .5f + randomOffset(.25f),
							first.getY() + .5f + randomOffset(.25f),
							first.getZ() + .5f + randomOffset(.25f),
							0,
							0,
							0
					);
			...
			if (world.getBlockState(selected).canBeReplaced()) return;
			if (!ShaftBlock.isShaft(world.getBlockState(selected)))
				selected = selected.relative(((BlockHitResult) rayTrace).getDirection());
			if (!selected.closerThan(first, AllConfigs.server().kinetics.maxBeltLength.get())) return;
				...
				if (r.nextInt(10) == 0) {
					world.addParticle(
							new DustParticleOptions(
									new Vector3f(canConnect ? .3f : .9f, canConnect ? .9f : .3f, .5f),
									1
							),
							position.x + .5f,
							position.y + .5f,
