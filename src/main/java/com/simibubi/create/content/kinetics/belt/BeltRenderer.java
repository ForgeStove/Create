...
public class BeltRenderer extends SafeBlockEntityRenderer<BeltBlockEntity> {
	...
	@Override public boolean shouldRenderOffScreen(BeltBlockEntity be) {
	...
	protected void renderSafe(
		...
		if (!Backend.canUseInstancing(be.getLevel())) {
			...
			if (be.hasPulley()) {
				...
				SuperByteBuffer superBuffer = CachedBufferer.partialDirectional(
		...
		if (color != null) {
			return (
					diagonal
							...
							: bottom ? AllSpriteShifts.DYED_OFFSET_BELTS : AllSpriteShifts.DYED_BELTS
			).get(color);
	...
	protected void renderItems(
		...
		for (TransportedItemStack transported : be.getInventory().getTransportedItems()) {
			...
			boolean tiltForward = (
					slope == BeltSlope.DOWNWARD ^ beltFacing.getAxisDirection() == AxisDirection.POSITIVE
			) == (
			...
			Vec3 itemPos = beltStartOffset.add(
					be.getBlockPos().getX(),
					be.getBlockPos().getY(), be.getBlockPos().getZ()
			).add(offsetVec);
	...
	protected int getPackedLight(BeltBlockEntity controller, float beltPos) {
		...
		if (controller.lighter == null || segment >= controller.lighter.lightSegments() || segment < 0) return 0;
