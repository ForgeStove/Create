...
public class BracketedKineticBlockEntityInstance extends SingleRotatingInstance<BracketedKineticBlockEntity> {
	...
	@Override public void init() {
		...
		Instancer<RotatingData> half = getRotatingMaterial().getModel(
	...
	private PoseStack rotateToAxis(Direction.Axis axis) {
		...
		TransformStack.cast(poseStack).centre().rotateToFace(facing).multiply(Axis.XN.rotationDegrees(-90)).unCentre();
		return poseStack;
	}
	...
	@Override public void updateLight() {
		super.updateLight();
		if (additionalShaft != null) relight(pos, additionalShaft);
	}
