...
public interface IRotate extends IWrenchable {
	enum SpeedLevel {
		...
		public float getSpeedValue() {
			return switch (this) {
				case FAST -> AllConfigs.server().kinetics.fastSpeed.get().floatValue();
				case MEDIUM -> AllConfigs.server().kinetics.mediumSpeed.get().floatValue();
				case SLOW -> 1;
				default -> 0;
			};
		}
		...
		public static LangBuilder getFormattedSpeedText(float speed, boolean overstressed) {
			...
			builder.translate("tooltip.speedRequirement." + Lang.asId(speedLevel.name()))
					...
					.text(")")
					.space();
			if (overstressed) builder.style(ChatFormatting.DARK_GRAY).style(ChatFormatting.STRIKETHROUGH);
