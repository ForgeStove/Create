...
public abstract class GaugeInstance extends ShaftInstance<GaugeBlockEntity> implements DynamicInstance {
	...
	protected GaugeInstance(MaterialManager materialManager, GaugeBlockEntity blockEntity) {
		...
		for (Direction facing : Iterate.directions) {
			if (!gaugeBlock.shouldRenderHeadOnFace(world, pos, blockState, facing)) continue;
	...
	@Override public void beginFrame() {
		GaugeBlockEntity gaugeBlockEntity = blockEntity;
		if (Mth.equal(gaugeBlockEntity.prevDialState, gaugeBlockEntity.dialState)) return;
		float progress = Mth.lerp(
				AnimationTickHolder.getPartialTicks(),
				gaugeBlockEntity.prevDialState,
				gaugeBlockEntity.dialState
		);
		TransformStack msr = TransformStack.cast(ms);
	...
	@Override public void updateLight() {
		super.updateLight();
		relight(pos, faces.stream().flatMap(Couple::stream));
	}
	...
	private class DialFace extends Couple<ModelData> {
		...
		protected TransformStack rotateToFace(TransformStack msr) {
			return msr.centre().rotate(Direction.UP, (float) ((-face.toYRot() - 90) / 180 * Math.PI)).unCentre();
		}
		private void delete() {
			getFirst().delete();
			getSecond().delete();
	...
	public static class Speed extends GaugeInstance {
		...
		@Override protected Instancer<ModelData> getHeadModel() {
