...
public class CuckooClockRenderer extends KineticBlockEntityRenderer<CuckooClockBlockEntity> {
	...
	@Override protected void renderSafe(
		...
		if (be.animationType != null) {
			...
			for (int phase = 30; phase <= 60; phase += step) {
				...
				if (local < 0) angle = Mth.lerp(((value - (phase - 5)) / 5), 0, 135);
				else if (local < (float) step / 3) angle = 135;
				else if (local < (float) (2 * step) / 3) angle = Mth.lerp(((value - (phase + 5)) / 5), 135, 0);
			}
		}
	...
	@Override protected SuperByteBuffer getRotatedModel(CuckooClockBlockEntity be, BlockState state) {
