...
public class TransportedItemStackHandlerBehaviour extends BlockEntityBehaviour {
	...
	public static class TransportedResult {
		...
		public boolean didntChangeFrom(ItemStack stackBefore) {
			return doesNothing() || outputs.size() == 1
					&& outputs.get(0).stack.equals(stackBefore, false)
					&& !hasHeldOutput();
		}
		...
		@Nullable public TransportedItemStack getHeldOutput() {
			if (heldOutput == null)
	...
	@FunctionalInterface public interface ProcessingCallback {
		void applyToAllItems(
				float maxDistanceFromCenter,
				Function<TransportedItemStack, TransportedResult> processFunction
		);
	}
	@FunctionalInterface public interface PositionGetter {
