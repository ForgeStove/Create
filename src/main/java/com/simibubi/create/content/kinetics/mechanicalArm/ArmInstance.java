...
public class ArmInstance extends SingleRotatingInstance<ArmBlockEntity> implements DynamicInstance {
	...
	public ArmInstance(MaterialManager materialManager, ArmBlockEntity blockEntity) {
		...
		base = mat.getModel(AllPartialModels.ARM_BASE, blockState).createInstance();
		lowerBody = mat.getModel(AllPartialModels.ARM_LOWER_BODY, blockState).createInstance();
		upperBody = mat.getModel(AllPartialModels.ARM_UPPER_BODY, blockState).createInstance();
		claw = mat.getModel(
				blockEntity.goggles ? AllPartialModels.ARM_CLAW_BASE_GOGGLES : AllPartialModels.ARM_CLAW_BASE,
				blockState
		).createInstance();
		ModelData clawGrip1 = mat.getModel(AllPartialModels.ARM_CLAW_GRIP_UPPER, blockState).createInstance();
		ModelData clawGrip2 = mat.getModel(AllPartialModels.ARM_CLAW_GRIP_LOWER, blockState).createInstance();
	...
	@Override public void beginFrame() {
		...
		boolean settled =
				Mth.equal(baseAngle, baseAngleNow) && Mth.equal(lowerArmAngle, lowerArmAngleNow) && Mth.equal(upperArmAngle,
				upperArmAngleNow
		...
		if (!settled || firstRender) animateArm(false);
	...
	private void animateArm(boolean rave) {
		...
		msr.centre();
		if (ceiling) msr.rotateX(180);
		ArmRenderer.transformBase(msr, baseAngle);
		base.setTransform(msLocal);
		ArmRenderer.transformLowerArm(msr, lowerArmAngle);
		lowerBody.setTransform(msLocal).setColor(color);
		ArmRenderer.transformUpperArm(msr, upperArmAngle);
		upperBody.setTransform(msLocal).setColor(color);
		ArmRenderer.transformHead(msr, headAngle);
		if (ceiling && blockEntity.goggles) msr.rotateZ(180);
		claw.setTransform(msLocal);
		if (ceiling && blockEntity.goggles) msr.rotateZ(180);
		ItemStack item = blockEntity.heldItem;
		ItemRenderer itemRenderer = Minecraft.getInstance().getItemRenderer();
		boolean hasItem = !item.isEmpty();
		boolean isBlockItem = hasItem && (item.getItem() instanceof BlockItem) && itemRenderer.getModel(
				item,
	...
	@Override public void update() {
		...
		models.remove(claw);
		claw.delete();
		claw = getTransformMaterial().getModel(
				blockEntity.goggles ? AllPartialModels.ARM_CLAW_BASE_GOGGLES : AllPartialModels.ARM_CLAW_BASE,
				blockState
		).createInstance();
		models.add(claw);
	...
	@Override protected Instancer<RotatingData> getModel() {
