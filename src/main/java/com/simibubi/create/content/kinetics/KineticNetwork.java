...
public class KineticNetwork {
	...
	public float calculateCapacity() {
		...
		for (
				Iterator<KineticBlockEntity> iterator = sources.keySet().iterator(); iterator.hasNext(); ) {
	...
	public float calculateStress() {
		...
		for (
				Iterator<KineticBlockEntity> iterator = members.keySet().iterator(); iterator.hasNext(); ) {
			KineticBlockEntity be = iterator.next();
			if (be.getLevel().getBlockEntity(be.getBlockPos()) != be) {
				iterator.remove();
				continue;
