	...
	public void renderItems(
		...
		if (be.phase == Phase.IDLE) {
			...
			if (be.phase == Phase.CRAFTING) {
				...
				items.calcStats();
				float progress = Mth.clamp((2000 - be.countDown + be.getCountDownSpeed() * partialTicks) / 1000f, 0,
						1);
			...
			items.grid.forEach((pair, stack) -> {
				if (onlyRenderFirst && (
						pair.getLeft() != 0 || pair.getRight() != 0
				)) return;
				ms.pushPose();
			...
			if (be.phase == Phase.CRAFTING) {
				...
				float progress = Mth.clamp((1000 - be.countDown + be.getCountDownSpeed() * partialTicks) / 1000f, 0,
						1);
				float earlyProgress = Mth.clamp(progress * 2, 0, 1);
				float lateProgress = Mth.clamp(progress * 2 - 1, 0, 1);
				...
				items.grid.forEach((pair, stack) -> {
					if (pair.getLeft() != 0 || pair.getRight() != 0) return;
					ms.pushPose();
					ms.mulPose(Axis.YP.rotationDegrees(180));
					Minecraft.getInstance()
							.getItemRenderer()
	...
	public void renderFast(
		...
		if (!Backend.canUseInstancing(be.getLevel())) {
			...
			superBuffer.rotateCentered(
					Direction.UP, (float) (
							blockState.getValue(HORIZONTAL_FACING).getAxis() != Direction.Axis.X ? 0 : Math.PI / 2
					)
			);
		...
		if (MechanicalCrafterBlock.isValidTarget(be.getLevel(), pos.relative(targetDirection), blockState)) {
			...
			SuperByteBuffer beltFrameBuffer = renderAndTransform(
					AllPartialModels.MECHANICAL_CRAFTER_BELT_FRAME,
					blockState
			);
			if (be.phase == Phase.EXPORTING) {
