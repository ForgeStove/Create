...
public class SequencedGearshiftScreen extends AbstractSimiScreen {
	...
	@Override protected void init() {
		if (be.computerBehaviour.hasAttachedComputer()) minecraft.setScreen(new ComputerScreen(
				title,
	...
	public void initInputsOfRow(int row, int backgroundX, int backgroundY) {
		...
		ScrollInput type = new SelectionScrollInput(
				x,
				y + rowHeight * row,
				50,
				18
		...
		ScrollInput value = new ScrollInput(x + 58, y + rowHeight * row, 28, 18).calling(state ->
				instruction.value
						= state);
		ScrollInput direction = new SelectionScrollInput(x + 88, y + rowHeight * row, 28, 18).forOptions(
						InstructionSpeedModifiers.getOptions())
	...
	public void updateParamsOfRow(int row) {
		...
		if (hasValue) value.withRange(1, def.maxValue + 1)
	...
	@Override protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		for (int row = 0; row < instructions.capacity(); row++) {
			...
			if (def.hasSpeedParameter) label(graphics, 127, yOffset - 1, instruction.speedModifier.label);
		}
		graphics.drawString(
				font,
				title,
	...
	private void renderAdditional(
		...
		GuiGameElement.of(renderedItem)
				.<GuiGameElement.GuiRenderBuilder>at(
						guiLeft + background.width + 6,
						guiTop + background.height - 56,
						100
				)
				.scale(5)
				.render(graphics);
	...
	public void sendPacket() {
		...
		if (serialized.equals(compareTag)) return;
		AllPackets.getChannel().sendToServer(new ConfigureSequencedGearshiftPacket(be.getBlockPos(), serialized));
	}
