...
public abstract class ArmInteractionPointType {
	...
	public static void register(ArmInteractionPointType type) {
		...
		if (TYPES.containsKey(id))
			throw new IllegalArgumentException("Tried to override ArmInteractionPointType registration for id '"
					+ id
	...
	@Nullable public static ArmInteractionPointType get(ResourceLocation id) {
		return TYPES.get(id);
	}
	public static void forEach(Consumer<ArmInteractionPointType> action) {
		SORTED_TYPES.forEach(action);
	}
	@Nullable public static ArmInteractionPointType getPrimaryType(Level level, BlockPos pos, BlockState state) {
	...
	@Nullable public abstract ArmInteractionPoint createPoint(Level level, BlockPos pos, BlockState state);
