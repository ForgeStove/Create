...
public class EncasedCogCTBehaviour extends EncasedCTBehaviour {
	...
	@Override public boolean connectsTo(
		...
		if (large || axis == face.getAxis()) return super.connectsTo(state, other, reader, pos, otherPos, face);
		if (other.getBlock() == state.getBlock() && other.getValue(AXIS) == state.getValue(AXIS)) return true;
	...
	@Override protected boolean reverseUVs(BlockState state, Direction face) {
	...
	@Override protected boolean reverseUVsVertically(BlockState state, Direction face) {
		if (!large && state.getValue(AXIS) == Axis.X && face.getAxis() == Axis.Z) return face != Direction.SOUTH;
	...
	public CTSpriteShiftEntry getShift(BlockState state, Direction direction, @Nullable TextureAtlasSprite sprite) {
		...
		if (large || axis == direction.getAxis()) {
			if (axis == direction.getAxis() && state.getValue(direction.getAxisDirection() == AxisDirection.POSITIVE
