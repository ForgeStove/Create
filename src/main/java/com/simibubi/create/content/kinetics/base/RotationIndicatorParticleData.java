...
public class RotationIndicatorParticleData
	...
	public static final PrimitiveCodec<Character> CHAR = new PrimitiveCodec<>() {
		@Override public <T> DataResult<Character> read(final DynamicOps<T> ops, final T input) {
			return ops.getNumberValue(input).map(n -> (char) n.shortValue());
		}
		@Override public <T> T write(final DynamicOps<T> ops, final Character value) {
	...
	public static final Codec<RotationIndicatorParticleData> CODEC = RecordCodecBuilder.create(i -> i.group(
	...
	public static final ParticleOptions.Deserializer<RotationIndicatorParticleData>
			DESERIALIZER
			= new ParticleOptions.Deserializer<>() {
		public RotationIndicatorParticleData fromCommand(
				ParticleType<RotationIndicatorParticleData> particleTypeIn,
				StringReader reader
		) throws
		...
		public RotationIndicatorParticleData fromNetwork(
				ParticleType<RotationIndicatorParticleData> particleTypeIn,
				FriendlyByteBuf buffer
		) {
			return new RotationIndicatorParticleData(
					buffer.readInt(),
					buffer.readFloat(),
					buffer.readFloat(),
					buffer.readFloat(),
	...
	@Override public String writeToString() {
		return String.format(
				Locale.ROOT,
				"%s %d %.2f %.2f %.2f %d %c",
				AllParticleTypes.ROTATION_INDICATOR.parameter(),
				color,
				speed,
				radius1,
				radius2,
	...
	@Override public Deserializer<RotationIndicatorParticleData> getDeserializer() {
		return DESERIALIZER;
	}
	@Override public Codec<RotationIndicatorParticleData> getCodec(ParticleType<RotationIndicatorParticleData> type) {
		return CODEC;
	}
	@Override @OnlyIn(Dist.CLIENT) public SpriteParticleRegistration<RotationIndicatorParticleData> getMetaFactory() {
