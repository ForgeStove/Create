...
public class MechanicalCraftingRecipe extends ShapedRecipe {
	...
	@Override public boolean matches(CraftingContainer inv, Level worldIn) {
		if (!(inv instanceof MechanicalCraftingInventory)) return false;
		if (acceptsMirrored()) return super.matches(inv, worldIn);
		...
		for (int i = 0; i <= inv.getWidth() - this.getWidth(); ++i)
			for (int j = 0; j <= inv.getHeight() - this.getHeight(); ++j)
				if (this.matchesSpecific(inv, i, j)) return true;
		return false;
	}
	// From ShapedRecipe
	private boolean matchesSpecific(CraftingContainer inv, int p_77573_2_, int p_77573_3_) {
		...
		for (int i = 0; i < inv.getWidth(); ++i) {
			for (int j = 0; j < inv.getHeight(); ++j) {
				...
				if (k >= 0 && l >= 0 && k < width && l < height) ingredient = ingredients.get(k + l * width);
				if (!ingredient.test(inv.getItem(i + j * inv.getWidth()))) return false;
			}
		}
		return true;
	}
	...
	public static class Serializer extends ShapedRecipe.Serializer {
		@Override public ShapedRecipe fromJson(ResourceLocation recipeId, JsonObject json) {
			return fromShaped(super.fromJson(recipeId, json), GsonHelper.getAsBoolean(json, "acceptMirrored", true));
		}
		@Override public ShapedRecipe fromNetwork(ResourceLocation recipeId, FriendlyByteBuf buffer) {
			return fromShaped(super.fromNetwork(recipeId, buffer), buffer.readBoolean() && buffer.readBoolean());
		}
		@Override public void toNetwork(FriendlyByteBuf p_199427_1_, ShapedRecipe p_199427_2_) {
			...
			} else p_199427_1_.writeBoolean(false);
		}
	}
}
