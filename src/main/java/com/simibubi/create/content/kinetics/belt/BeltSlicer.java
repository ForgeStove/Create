...
public class BeltSlicer {
	...
	public static InteractionResult useWrench(
		...
		if (!creative) {
			...
			if (AllBlocks.BELT.has(other) && other.getValue(BeltBlock.PART) == BeltPart.MIDDLE) requiredShafts++;
			...
			while (true) {
				for (int i = 0; i < player.getInventory().getContainerSize(); ++i) {
					...
					if (AllBlocks.SHAFT.isIn(itemstack)) {
						...
						if (!world.isClientSide) if (taken == count) player.getInventory().setItem(i, ItemStack.EMPTY);
	...
	public static InteractionResult useConnector(
		...
		if (!nextState.canBeReplaced()) {
			if (!AllBlocks.BELT.has(nextState)) return InteractionResult.FAIL;
	...
	@OnlyIn(Dist.CLIENT) public static void tickHoveringInformation() {
		...
		if (feedback.langKey != null)
			mc.player.displayClientMessage(Lang.translateDirect(feedback.langKey).withStyle(feedback.formatting),
					true);
		else mc.player.displayClientMessage(Components.immutableEmpty(), true);
		if (feedback.bb != null)
			CreateClient.OUTLINER.chaseAABB("BeltSlicer", feedback.bb).lineWidth(1 / 16f).colored(feedback.color);
	}
}
