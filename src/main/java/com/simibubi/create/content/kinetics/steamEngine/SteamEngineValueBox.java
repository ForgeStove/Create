...
public class SteamEngineValueBox extends ValueBoxTransform.Sided {
	@Override protected boolean isSideActive(BlockState state, Direction side) {
		...
		if (engineFacing.getAxis() == Axis.Y) recessed ^= state.getValue(SteamEngineBlock.FACING).getAxis() == Axis.X;
		return !recessed;
	}
	...
	@Override public void rotate(BlockState state, PoseStack ms) {
		...
		TransformStack.cast(ms).rotateY(yRot).rotateX(facing == Direction.DOWN ? -90 : 90).rotateY(roll);
	}
