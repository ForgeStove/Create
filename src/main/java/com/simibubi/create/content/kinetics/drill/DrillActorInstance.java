...
public class DrillActorInstance extends ActorInstance {
	...
	public DrillActorInstance(
		...
		float eulerY;
		if (axis == Direction.Axis.Y) eulerY = 0;
		else eulerY = facing.toYRot() + ((axis == Direction.Axis.X) ? 180 : 0);
	...
	protected float getSpeed(Direction facing) {
		if (context.contraption.stalled || !VecHelper.isVecPointingTowards(
