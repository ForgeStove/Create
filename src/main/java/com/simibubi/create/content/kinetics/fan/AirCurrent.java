...
public class AirCurrent {
	...
	protected void tickAffectedEntities(Level world) {
		for (Iterator<Entity> iterator = caughtEntities.iterator(); iterator.hasNext(); ) {
			...
			// entityDistanceOld should be removed eventually. Remember that entityDistanceOld cannot be 0 while
			// entityDistance can,
			...
			if (entity instanceof ServerPlayer) ((ServerPlayer) entity).connection.aboveGroundTickCount = 0;
			...
			if (entity instanceof ItemEntity itemEntity) {
				...
				if (FanProcessing.canProcess(itemEntity, processingType))
					if (FanProcessing.applyProcessing(itemEntity, processingType)
							&& source instanceof EncasedFanBlockEntity fan) fan.award(AllAdvancements.FAN_PROCESSING);
				continue;
			}
	...
	@OnlyIn(Dist.CLIENT) private static AirCurrentSound flyingSound;
	@OnlyIn(Dist.CLIENT) private static void enableClientPlayerSound(Entity e, float maxVolume) {
	...
	@OnlyIn(Dist.CLIENT) public static void tickClientPlayerSounds() {
		if (!isClientPlayerInAirCurrent && flyingSound != null) if (flyingSound.isFaded()) flyingSound.stopSound();
