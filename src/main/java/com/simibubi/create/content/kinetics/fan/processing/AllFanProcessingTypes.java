...
public class AllFanProcessingTypes {
	...
	@Nullable public static FanProcessingType ofLegacyName(String name) {
	...
	public static class NoneType implements FanProcessingType {
		@Override public boolean isValidAt(Level level, BlockPos pos) {
			return true;
		}
		@Override public int getPriority() {
			return -1000000;
		}
		@Override public boolean canProcess(ItemStack stack, Level level) {
			return false;
		}
		@Override @Nullable public List<ItemStack> process(ItemStack stack, Level level) {
			return null;
		}
		@Override public void spawnProcessingParticles(Level level, Vec3 pos) {
		}
		@Override public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
		}
		@Override public void affectEntity(Entity entity, Level level) {
		}
	}
	public static class BlastingType implements FanProcessingType {
		...
		@Override @Nullable public List<ItemStack> process(ItemStack stack, Level level) {
			...
			if (smeltingRecipe.isPresent()) {
				...
				if (smokingRecipe.isEmpty() || !ItemStack.isSameItem(
						smokingRecipe.get().getResultItem(registryAccess),
						smeltingRecipe.get().getResultItem(registryAccess)
				)) {
		...
		@Override public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
			...
			if (random.nextFloat() < 1 / 32f) particleAccess.spawnExtraParticle(ParticleTypes.FLAME, .25f);
	...
	public static class HauntingType implements FanProcessingType {
		...
		@Override @Nullable public List<ItemStack> process(ItemStack stack, Level level) {
			HAUNTING_WRAPPER.setItem(0, stack);
			Optional<HauntingRecipe> recipe = AllRecipeTypes.HAUNTING.find(HAUNTING_WRAPPER, level);
			if (recipe.isPresent()) return RecipeApplier.applyRecipeOn(level, stack, recipe.get());
			return null;
		}
		@Override public void spawnProcessingParticles(Level level, Vec3 pos) {
			if (level.random.nextInt(8) != 0) return;
		...
		@Override public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
			...
			if (random.nextFloat() < 1 / 128f) particleAccess.spawnExtraParticle(ParticleTypes.SOUL_FIRE_FLAME, .125f);
			if (random.nextFloat() < 1 / 32f) particleAccess.spawnExtraParticle(ParticleTypes.SMOKE, .125f);
		}
		@Override public void affectEntity(Entity entity, Level level) {
			if (level.isClientSide) {
				if (entity instanceof Horse) {
					...
					if (level.random.nextInt(3) == 0) level.addParticle(
			...
			if (entity instanceof Horse horse) {
				...
				if (!horse.getArmor().isEmpty()) horse.spawnAtLocation(horse.getArmor());
	...
	public static class SmokingType implements FanProcessingType {
		...
		@Override @Nullable public List<ItemStack> process(ItemStack stack, Level level) {
			...
			Optional<SmokingRecipe> smokingRecipe = level.getRecipeManager()
					.getRecipeFor(RecipeType.SMOKING, RECIPE_WRAPPER, level)
					.filter(AllRecipeTypes.CAN_BE_AUTOMATED);
			if (smokingRecipe.isPresent()) return RecipeApplier.applyRecipeOn(level, stack, smokingRecipe.get());
			return null;
		}
		@Override public void spawnProcessingParticles(Level level, Vec3 pos) {
			if (level.random.nextInt(8) != 0) return;
		...
		@Override public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
			...
			particleAccess.setAlpha(1f);
			if (random.nextFloat() < 1 / 32f) particleAccess.spawnExtraParticle(ParticleTypes.SMOKE, .125f);
			if (random.nextFloat() < 1 / 32f) particleAccess.spawnExtraParticle(ParticleTypes.LARGE_SMOKE, .125f);
		}
		@Override public void affectEntity(Entity entity, Level level) {
			if (level.isClientSide) return;
			if (!entity.fireImmune()) {
	...
	public static class SplashingType implements FanProcessingType {
		...
		@Override public void morphAirFlow(AirFlowParticleAccess particleAccess, RandomSource random) {
			...
			particleAccess.setAlpha(1f);
			if (random.nextFloat() < 1 / 32f) particleAccess.spawnExtraParticle(ParticleTypes.BUBBLE, .125f);
