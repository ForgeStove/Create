...
public class SawRenderer extends SafeBlockEntityRenderer<SawBlockEntity> {
	...
	public static void renderInContraption(
		...
		if (SawBlock.isHorizontal(state)) {
			...
			else superBuffer = CachedBufferer.partial(AllPartialModels.SAW_BLADE_HORIZONTAL_INACTIVE, state);
		} else {
			if (shouldAnimate) superBuffer = CachedBufferer.partial(AllPartialModels.SAW_BLADE_VERTICAL_ACTIVE, state);
			else superBuffer = CachedBufferer.partial(AllPartialModels.SAW_BLADE_VERTICAL_INACTIVE, state);
		}
