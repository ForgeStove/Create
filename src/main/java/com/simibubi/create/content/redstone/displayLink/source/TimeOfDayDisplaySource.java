...
public class TimeOfDayDisplaySource extends SingleLineDisplaySource {
	...
	@Override protected MutableComponent provideLine(DisplayLinkContext context, DisplayTargetStats stats) {
		...
		if (!(context.getSourceBlockEntity() instanceof CuckooClockBlockEntity ccbe)) return EMPTY_TIME;
		...
		MutableComponent component = Components.literal((hours < 10 ? " " : "") + hours + ":" + (
				minutes < 10 ? "0" : ""
		) + minutes + (c12 ? " " : ""));
	...
	@Override protected boolean allowsLabeling(DisplayLinkContext context) {
		return true;
	}
}
