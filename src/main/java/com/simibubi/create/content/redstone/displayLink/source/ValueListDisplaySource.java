...
public abstract class ValueListDisplaySource extends DisplaySource {
	...
	@Override public List<MutableComponent> provideText(DisplayLinkContext context, DisplayTargetStats stats) {
		...
		).map(e -> createComponentsFromEntry(
				context,
				e
		)).map(l -> {
	...
	private Couple<MutableComponent> shorten(int number) {
		if (number >= 1000000) return Couple.create(
				Components.literal(String.valueOf(number / 1000000)),
				Lang.translateDirect("display_source.value_list.million").append(WHITESPACE)
		);
		if (number >= 1000) return Couple.create(
				Components.literal(String.valueOf(number / 1000)),
	...
	@OnlyIn(Dist.CLIENT) protected void addFullNumberConfig(ModularGuiLineBuilder builder) {
