...
public class TrainStatusDisplaySource extends SingleLineDisplaySource {
	@Override protected MutableComponent provideLine(DisplayLinkContext context, DisplayTargetStats stats) {
		if (!(context.getSourceBlockEntity() instanceof StationBlockEntity observerBE)) return EMPTY_LINE;
		...
		if (runtime.currentEntry == schedule.entries.size() - 1 && !schedule.cyclic) return EMPTY_LINE;
