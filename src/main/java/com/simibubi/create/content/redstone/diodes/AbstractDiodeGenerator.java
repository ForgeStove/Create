...
public abstract class AbstractDiodeGenerator extends SpecialBlockStateGen {
	...
	@Override protected final int getXRotation(BlockState state) {
		return 0;
	}
	@Override protected final int getYRotation(BlockState state) {
		return horizontalAngle(state.getValue(AbstractDiodeBlock.FACING));
	}
	protected abstract <T extends Block> Vector<ModelFile> createModels(
