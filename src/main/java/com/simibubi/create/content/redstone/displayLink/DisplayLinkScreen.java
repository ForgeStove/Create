...
public class DisplayLinkScreen extends AbstractSimiScreen {
	...
	@Override public void tick() {
		super.tick();
		if (sourceState != null
				&& sourceState.getBlock() != minecraft.level.getBlockState(blockEntity.getSourcePosition()).getBlock()
				|| targetState != null
				&& targetState.getBlock() != minecraft.level.getBlockState(blockEntity.getTargetPosition()).getBlock())
			initGathererOptions();
	}
	@SuppressWarnings("deprecation") private void initGathererOptions() {
		...
		sourceWidget.getToolTip().addAll(List.of(
				...
				Lang.translateDirect("display_link.view_compatible").withStyle(ChatFormatting.GRAY)
		));
	...
	private void initGathererSourceSubOptions(int i) {
		...
		if (targetLineSelector != null)
			targetLineSelector.titled(source instanceof SingleLineDisplaySource ? Lang.translateDirect(
					"display_link.display_on") : Lang.translateDirect("display_link.display_on_multiline"));
	...
	@Override public void onClose() {
		...
		AllPackets.getChannel()
				.sendToServer(new DisplayLinkConfigurationPacket(
	...
	@Override protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		TransformStack.cast(ms)
				...
				.rotateX(-22)
				.rotateY(63);
		GuiGameElement.of(blockEntity.getBlockState().setValue(DisplayLinkBlock.FACING, Direction.UP)).render(graphics);
		ms.popPose();
	}
	@Override protected void removeWidget(GuiEventListener p_169412_) {
