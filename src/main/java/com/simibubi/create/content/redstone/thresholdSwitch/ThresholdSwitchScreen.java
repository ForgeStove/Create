...
public class ThresholdSwitchScreen extends AbstractSimiScreen {
	...
	@Override protected void init() {
		...
		int x = guiLeft;
		int y = guiTop;
		cursor = LerpedFloat.linear().startWithValue(blockEntity.getLevelForDisplay());
		cursorLane = LerpedFloat.linear().startWithValue(blockEntity.getState() ? 1 : 0);
	...
	@Override protected void renderWindow(GuiGraphics graphics, int mouseX, int mouseY, float partialTicks) {
		...
		graphics.blit(
				...
				(int) (x + upperBound) + 37,
				y + 20,
				(int) (sprite.startX + upperBound),
				sprite.startY,
				(int) (sprite.width - upperBound),
				sprite.height
		);
		graphics.blit(sprite.location, x + 37, y + 42, sprite.startX, sprite.startY, (int) (lowerBound),
				sprite.height);
		...
		if (blockEntity.currentLevel != -1) {
			...
			ms.translate(
					...
					cursorLane.getValue(partialTicks) * 22,
					0
			);
	...
	protected void send(boolean invert) {
		AllPackets.getChannel()
				.sendToServer(new ConfigureThresholdSwitchPacket(
						blockEntity.getBlockPos(),
						offBelow.getState() / 100f,
						onAbove.getState() / 100f,
						invert
				));
	}
}
