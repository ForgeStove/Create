...
public class LinkBehaviour extends BlockEntityBehaviour implements IRedstoneLinkable, ClipboardCloneable {
	...
	@Override public int getTransmittedStrength() {
		return mode == Mode.TRANSMIT ? transmission.getAsInt() : 0;
	}
	@Override public void setReceivedStrength(int networkPower) {
	...
	@Override public Couple<Frequency> getNetworkKey() {
	...
	@Override public BlockPos getLocation() {
		return getPos();
	}
	...
	@Override public boolean writeToClipboard(CompoundTag tag, Direction side) {
		tag.put("First", frequencyFirst.getStack().save(new CompoundTag()));
		tag.put("Last", frequencyLast.getStack().save(new CompoundTag()));
		return true;
	}
