...
public abstract class SingleLineDisplaySource extends DisplaySource {
	...
	@OnlyIn(Dist.CLIENT) protected void addLabelingTextBox(ModularGuiLineBuilder builder) {
		builder.addTextInput(
				0, 137, (e, t) -> {
					e.setValue("");
					t.withTooltip(ImmutableList.of(
							Lang.translateDirect("display_source.label").withStyle(s -> s.withColor(0x5391E1)),
	...
	@Override public List<MutableComponent> provideText(DisplayLinkContext context, DisplayTargetStats stats) {
		...
		if (allowsLabeling(context)) {
			...
			if (!label.isEmpty()) line = Components.literal(label + " ").append(line);
		}
	...
	public List<List<MutableComponent>> provideFlapDisplayText(DisplayLinkContext context, DisplayTargetStats stats) {
		if (allowsLabeling(context)) {
			String label = context.sourceConfig().getString("Label");
			if (!label.isEmpty())
				return ImmutableList.of(ImmutableList.of(Components.literal(label + " "), provideLine(context,
						stats)));
		}
	...
	public void loadFlapDisplayLayout(
		...
		if (!allowsLabeling(context)) {
			if (!layout.isLayout(layoutKey)) layout.configure(
					layoutKey,
		...
		FlapDisplaySection labelSection = new FlapDisplaySection(
				Math.min(maxCharCount, label.length() + 1)
						* FlapDisplaySection.MONOSPACE, "alphabet", false, false
		);
		if (label.length() + 1 < maxCharCount) layout.configure(
				layoutName,
