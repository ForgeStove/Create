...
public class RedstoneLinkNetworkHandler {
	static final Map<LevelAccessor, Map<Couple<Frequency>, Set<IRedstoneLinkable>>>
			connections
			= new IdentityHashMap<>();
	...
	public static class Frequency {
		...
		public static Frequency of(ItemStack stack) {
			...
			if (!stack.hasTag()) return simpleFrequencies.computeIfAbsent(stack.getItem(), $ -> new Frequency(stack));
	...
	public void updateNetworkOf(LevelAccessor world, IRedstoneLinkable actor) {
		...
		for (IRedstoneLinkable other : network) {
			if (other != actor && other.isListening() && withinRange(actor, other)) other.setReceivedStrength(power);
		}
	}
