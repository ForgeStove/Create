...
@EventBusSubscriber public class DisplayLinkBlockItem extends BlockItem {
	public DisplayLinkBlockItem(Block pBlock, Properties pProperties) {
		super(pBlock, pProperties);
	}
	@SubscribeEvent public static void gathererItemAlwaysPlacesWhenUsed(PlayerInteractEvent.RightClickBlock event) {
		ItemStack usedItem = event.getItemStack();
		if (usedItem.getItem() instanceof DisplayLinkBlockItem) {
			if (AllBlocks.DISPLAY_LINK.has(event.getLevel().getBlockState(event.getPos()))) return;
	...
	@Override public InteractionResult useOn(UseOnContext pContext) {
		...
		if (!selectedPos.closerThan(placedPos, AllConfigs.server().logistics.displayLinkRange.get())) {
			player.displayClientMessage(
					Lang.translateDirect("display_link.too_far").withStyle(ChatFormatting.RED),
					true
			);
		...
		player.displayClientMessage(Lang.translateDirect("display_link.success").withStyle(ChatFormatting.GREEN),
				true);
		return useOn;
	}
	...
	@OnlyIn(Dist.CLIENT) public static void clientTick() {
		...
		if (!(heldItemMainhand.getItem() instanceof DisplayLinkBlockItem)) return;
		if (!heldItemMainhand.hasTag()) return;
		CompoundTag stackTag = heldItemMainhand.getOrCreateTag();
		if (!stackTag.contains("SelectedPos")) return;
		...
		CreateClient.OUTLINER.showAABB("target", lastShownAABB).colored(0xffcb74).lineWidth(1 / 16f);
	}
	@OnlyIn(Dist.CLIENT) private static AABB getBounds(BlockPos pos) {
