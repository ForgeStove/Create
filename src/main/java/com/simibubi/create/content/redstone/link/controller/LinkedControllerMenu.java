...
public class LinkedControllerMenu extends GhostItemMenu<ItemStack> {
	...
	@Override protected ItemStack createOnClient(FriendlyByteBuf extraData) {
	...
	@Override protected void saveData(ItemStack contentHolder) {
		contentHolder.getOrCreateTag().put("Items", ghostInventory.serializeNBT());
	}
	...
	@Override public void clicked(int slotId, int dragType, ClickType clickTypeIn, Player player) {
		if (slotId == playerInventory.selected && clickTypeIn != ClickType.THROW) return;
